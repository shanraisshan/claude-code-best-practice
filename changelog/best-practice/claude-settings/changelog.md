...
## [2026-09-17 10:37 AM PKT] Claude Code v2.1.274

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update version badge v2.1.252 → v2.1.274; update header "As of v2.1.252" → "As of v2.1.274"; update counts from "140+ settings / 315+ env vars" to "150+ settings / 315+ env vars" | ✅ COMPLETE (badge, header, and counts updated) — NEW |
| 2 | HIGH | Missing Settings | Add 4 new managed-only policy keys: `managedSourcesBehavior` (string, controls merge behavior across admin sources, v2.1.253+), `disableDesktopLocalSessions` (boolean, prevents local desktop sessions, v2.1.257+), `sshHostAllowlist` (array, SSH host glob allowlist, v2.1.261+), `gatewayInternalNetworks` (array, CIDR/hostname patterns for on-premises routing, v2.1.265+). Confirmed via research agents and official settings-reference page | ✅ COMPLETE (all 4 added to Managed-only policy keys table) — NEW |
| 3 | HIGH | Missing Settings | Add 2 new Permission Keys: `permissions.blockReadsOutsideWorkingDirectories` (boolean, read-level containment boundary, v2.1.258+) and `skipAutoPermissionPrompt` (boolean, skip auto-mode confirmation dialog, v2.1.263+). Confirmed via research agents and official settings-reference page | ✅ COMPLETE (both added to Permission Keys table) — NEW |
| 4 | HIGH | Missing Setting | Add `managedMcpServers` (object, Managed only) to MCP Settings table — admin-deployed MCP servers available to all users without project `.mcp.json`. Not disabled by `disabledMcpjsonServers`. Confirmed via research agents and official settings-reference page (v2.1.268+) | ✅ COMPLETE (added to MCP Settings table) — NEW |
| 5 | HIGH | Wrong Description | Fix `enableArtifact`: semantics were inverted — report described as "user-level opt-in when set to true" but official docs confirm this is a managed-only key for admins to enable Artifact at org level, and `disableArtifact: true` cannot be overridden once set. Version floor also wrong (v2.1.196 → v2.1.242). Confirmed on official settings-reference page | ✅ COMPLETE (semantics corrected, version updated, scope set to managed-only) — NEW |
| 6 | HIGH | Admin-Source Union Keys | Extend admin-source union keys list at line 53: add 9 missing keys: `disableClaudeAiConnectors`, `enableArtifact`, `isolatePeerMachines`, `remoteControlAtStartup`, `crossSessionInbound`, `useAutoModeDuringPlan`, `syncClaudeAiSkills`, `syncClaudeAiPlugins`, `maxEffortLevel`. Confirmed via research agents and official settings-reference page | ✅ COMPLETE (all 9 keys added to exception paragraph) — NEW |
| 7 | HIGH | Hook Count | Update hooks redirect section from "28 hook events" to "33 hook events" — 5 new hook events added between v2.1.252 and v2.1.274. Confirmed via research agents cross-checking official hooks docs | ✅ COMPLETE (count updated in hooks redirect section) — NEW |
| 8 | MED | Missing Setting | Add `promptSuggestionEnabled` (boolean, default `true`, v2.1.260+) to Display Settings table — controls AI-generated prompt suggestion chips. Confirmed via research agents and official settings-reference page | ✅ COMPLETE (added to Display Settings table) — NEW |
| 9 | MED | Missing Setting | Add `copyOnSelect` (boolean, default `false`, v2.1.271+) to Global Config Settings (`~/.claude.json`) — auto-copy selected text to clipboard in fullscreen TUI mode. Confirmed via research agents and official settings-reference page | ✅ COMPLETE (added to Global Config Settings table) — NEW |
| 10 | MED | Wrong Description | Fix Sources section: `claude-settings-reference` description said "~180 keys with Scope/Type/Default columns" but the page columns are Key/Description/Topic/Scope (no Type or Default axis). Confirmed by fetching the official page | ✅ COMPLETE (description corrected) — NEW |
| 11 | LOW | Missing Env Vars | 15 env vars confirmed absent via grep from v2.1.252–v2.1.274 window. Context-window compaction occurred mid-session before exact list was written to changelog — specific vars require re-verification in a follow-up run | ✋ ON HOLD (context loss — re-verify in next run) — NEW |
