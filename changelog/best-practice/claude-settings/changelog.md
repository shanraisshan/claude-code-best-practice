...
| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update version badge v2.1.252 → v2.1.268; update header counts from "140+ settings" to "145+ settings" to account for 6 new settings keys added this run | ✅ COMPLETE (badge and header updated) — NEW |
| 2 | HIGH | Missing Settings | Add `bashOutputMaxChars` (number, default 8000, up to 128K, v2.1.268) and `taskOutputMaxChars` (number, default 8000, up to 128K, v2.1.268) to General Settings — raise inline output limits for bash commands and background tasks respectively. Confirmed in v2.1.268 changelog | ✅ COMPLETE (both added to General Settings table) — NEW |
...
| 5 | HIGH | Missing Settings | Add `managedMcpServers` (array, Managed only, v2.1.265+) to MCP Settings — organization-deployed HTTP/SSE MCP servers pushed to all users; load regardless of `allowedMcpServers` allowlist. Confirmed in v2.1.265 changelog | ✅ COMPLETE (added to MCP Settings table) — NEW |
