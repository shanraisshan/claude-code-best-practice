...
## [2026-09-19 10:45 AM PKT] Claude Code v2.1.278

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update version badge v2.1.252 → v2.1.278; update header counts from "140+ settings / 315+ env vars" to "231+ settings / 363+ env vars". Confirmed via official settings-reference (231 key entries) and env-vars pages (363 vars) | ✅ COMPLETE (badge and header updated) — NEW |
| 2 | HIGH | Correctness Fix | Fix `ultracode` scope description — report incorrectly stated "Session-only — not persisted...set via /effort ultracode...rather than written to settings.json". Official docs confirm `ultracode` IS a valid settings.json key (scope: Any file, boolean); Claude Code reads it but never writes it. Confidence 0.95 | ✅ COMPLETE (description corrected) — NEW |
| 3 | HIGH | Correctness Fix | Fix `allowedMcpServers` and `deniedMcpServers` scope from "Managed only" → "Any file". Entries from every scope merge into one combined list. Confidence 0.95 | ✅ COMPLETE (scopes updated) — NEW |
| 4 | HIGH | Correctness Fix | Fix `permissions.skipDangerousModePermissionPrompt` — this is a top-level key, NOT nested under `permissions`. Official docs confirm the key is `skipDangerousModePermissionPrompt` (scope: User, local, or managed). Confidence 0.9 | ✅ COMPLETE (de-nested and scope corrected) — NEW |
| 5 | HIGH | Correctness Fix | Fix `permissions.defaultMode` — add missing v2.1.257 note: `"bypassPermissions"` is also ignored when set in project or local settings. Security-relevant omission. Confidence 0.95 | ✅ COMPLETE (restriction added) — NEW |
| 6 | HIGH | Correctness Fix | Fix `askUserQuestionTimeout` scope description — report said "Only honored from project and local settings" but official docs say scope is "User or managed". Confidence 0.9 | ✅ COMPLETE (scope corrected) — NEW |
| 7 | HIGH | Correctness Fix | Fix `permissions.blockReadsOutsideWorkingDirectories` (boolean, Any file, v2.1.257+) — completely absent from report. Restricts read operations to working directories even in bypassPermissions mode (restrictive-value exception). Confidence 1.0 | ✅ COMPLETE (added to Permission Keys table) — NEW |
| 8 | HIGH | Missing Settings | Add `maxEffortLevel` (string, Any file, v2.1.267) to Model Configuration — the single most impactful omission; caps all effort-level sources including agent/skill frontmatter. Restrictive-value exception. Confidence 1.0 | ✅ COMPLETE (added to Model Overrides table) — NEW |
| 9 | HIGH | Missing Settings | Add `syncClaudeAiPlugins` (boolean, User/local/managed, v2.1.273) and `syncClaudeAiSkills` (boolean, User/local/managed, v2.1.275) to Plugin Settings. Both are restrictive-value exceptions (only `false` honored). Confidence 1.0 | ✅ COMPLETE (both added to Plugin Settings table) — NEW |
| 10 | HIGH | Missing Settings | Add `managedMcpServers` (object, Managed, v2.1.259) and `managedSourcesBehavior` (string, Managed, v2.1.242) to MCP Settings. Both govern org-wide MCP server delivery. Confidence 1.0 | ✅ COMPLETE (both added to MCP Settings table) — NEW |
| 11 | HIGH | Missing Settings | Add `gatewayInternalNetworks` (array, Managed, v2.1.268) to MCP Settings. Governs internal network CIDR ranges for the Claude gateway. Confidence 1.0 | ✅ COMPLETE (added to MCP Settings table) — NEW |
| 12 | HIGH | Stale Annotation | Remove "(not in official settings page — unverified)" from `thinkingBudgetTokens` — confirmed present in official settings-reference. Confidence 0.85 | ✅ COMPLETE (annotation removed) — RESOLVED |
| 13 | HIGH | Deprecated Key | Mark `TASK_MAX_OUTPUT_LENGTH` as no-op (v2.1.278) — TaskOutput tool removed; the variable no longer has any effect. Confidence 0.95 | ✅ COMPLETE (marked as no-op) — NEW |
| 14 | HIGH | Correctness Fix | Fix `modelPicker` — report example was a bare array `[{id,label}]` but official shape is `{options: [{id,label}], replaceBuiltInOptions: boolean}`. Also add missing scope restriction: "User or managed only". Confidence 0.9 | ✅ COMPLETE (example and scope corrected) — NEW |
| 15 | HIGH | Wrong Version | Fix `teammateDefaultModel` removal version — report said "Removed in v2.1.251" but official docs say "Removed in v2.1.234". Confidence 0.9 | ✅ COMPLETE (version corrected) — NEW |
| 16 | MED | Correctness Fix | Fix `extraKnownMarketplaces` scope from "Project" → "Any file". Confidence 0.75 | ✅ COMPLETE (scope updated) — NEW |
| 17 | MED | Hook Count | Update hooks redirect from "28 hook events" → "30 hook events" (PostToolBatch and MessageDisplay added after v2.1.252). Confidence from agent findings | ✅ COMPLETE (count updated) — NEW |
| 18 | MED | Missing Settings | Add `isolatePeerMachines` (boolean, Any file), `enableWorkflows` (boolean, Any file), `promptSuggestionEnabled` (boolean, default true), `skipAutoPermissionPrompt` (boolean, User or managed), `bashOutputMaxChars` (number, v2.1.261), `bashEditDiffEnabled` (boolean, v2.1.269) to General Settings. Confidence 1.0 | ✅ COMPLETE (all added to General Settings table) — NEW |
| 19 | MED | Missing Settings | Add `subagentStatusLine` (object), `terminalTitleFromRename` (boolean, default true), `timeFormat` (string, default "auto", v2.1.257), `timeZone` (string, v2.1.257) to Display Settings. Confidence 1.0 | ✅ COMPLETE (all added to Display Settings table) — NEW |
| 20 | MED | Missing Settings | Add `copyOnSelect` (boolean, default true) to Global Config Settings (`~/.claude.json`). Confidence 1.0 | ✅ COMPLETE (added to Global Config table) — NEW |
| 21 | MED | Missing Settings | Add `sandbox.ripgrep` (object, User or managed) to Sandbox Settings. Confidence 1.0 | ✅ COMPLETE (added to Sandbox Settings table) — NEW |
| 22 | MED | Cross-Reference | Add cross-reference to `BASH_MAX_OUTPUT_LENGTH` → superseded by `bashOutputMaxChars` when set. Confidence 0.9 | ✅ COMPLETE (cross-reference added) — NEW |
| 23 | MED | Missing Env Vars | Add 13 missing env vars: `CLAUDE_CODE_SUBAGENT_MODEL_FORCE`, `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS`, `CLAUDE_CODE_AUTO_MODE_SERVER` (v2.1.278), `CLAUDE_CODE_MCP_STARTUP_WAIT_MS` (v2.1.274), `MCP_SDK_GENERATION` (v2.1.274), `MCP_PROTOCOL_NEGOTIATION` (v2.1.274), `OTEL_METRICS_INCLUDE_REPOSITORY`, `OTEL_LOG_MANAGED_SETTINGS` (v2.1.274), `CLAUDE_CODE_GATEWAY_HINT_HEADERS` (v2.1.273), `CLAUDE_GATEWAY_PROXY_IS_EGRESS_BOUNDARY` (v2.1.277), `ANTHROPIC_ORGANIZATION_ID`, `ANTHROPIC_FEDERATION_RULE_ID`, `CLAUDE_CODE_PROMPT_CACHE_TTL`, `CLAUDE_CODE_SUBAGENT_PROMPT_CACHE_TTL`. Confirmed in official env-vars page or changelog | ✅ COMPLETE (all added) — NEW |
| 24 | MED | `workflowSizeGuideline` Update | Add concrete agent-count semantics (small <5, medium <10, large <50) and note Pro plan default changes to "small" in v2.1.271. Confirmed in official settings-reference | ✅ COMPLETE (description updated) — NEW |
| 25 | LOW | Sources Update | Update Settings Reference source note from "~180 keys" to "231 documented keys"; add clarification that `docs/en/settings` is now precedence/scope narrative and `settings-reference` is the key index | ✅ COMPLETE (source note updated) — NEW |
