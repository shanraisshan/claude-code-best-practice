...
## [2026-08-16 10:47 AM PKT] Claude Code v2.1.233

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Bump | Update report version badge from v2.1.224 → v2.1.233; update header "As of v2.1.224" → "As of v2.1.233"; update env var count 311 → 335+ | ✅ COMPLETE (badge updated in Phase 2.6; header updated) — RECURRING |
| 2 | HIGH | Inverted Scope | Fix `askUserQuestionTimeout` description: scope was inverted — "Only honored from project and local settings — managed and user settings values are ignored" is wrong; correct is "Not read from project or local settings; only honored from user settings" | ✅ COMPLETE (description fixed) — NEW |
| 3 | HIGH | Wrong Type | Fix `strictKnownMarketplaces` type: `boolean` → `array`. Report contradicts itself — line 539 said boolean but line 551 mentioned owner wildcard entries (impossible for a boolean). JSON schema confirms array. Update description to allowlist semantics | ✅ COMPLETE (type and description updated) — RECURRING (first "fixed" to boolean 2026-07-13; re-correcting to array based on schema + self-contradiction) |
| 4 | HIGH | Nesting Error | Fix `skipDangerousModePermissionPrompt` key: remove `permissions.` prefix — JSON schema places this at top level (`permissions` sub-keys are exactly `allow`, `deny`, `ask`, `defaultMode`, `disableBypassPermissionsMode`, `disableAutoMode`, `additionalDirectories`) | ✅ COMPLETE (key moved to top level) — NEW |
| 5 | HIGH | New Setting | Add `crossSessionInbound` (string, `"accept" \| "hold" \| "refuse"`, v2.1.224) to Display Settings — controls how the desktop handles inbound messages from other active sessions. Confirmed on official settings page | ✅ COMPLETE (added to Display Settings table) — RESOLVED (first seen 2026-08-07 #21 ON HOLD) |
| 6 | HIGH | New Setting | Add `dialogExpiry` (string, `"60s" \| "5m" \| "10m" \| "never"`, default `"5m"`, v2.1.224) to General Settings — timeout before an inactive AskUserQuestion dialog auto-expires. Confirmed on official settings page | ✅ COMPLETE (added to General Settings table) — RESOLVED (first seen 2026-08-07 #21 ON HOLD) |
| 7 | HIGH | New Setting | Add `disableCommandPluginSources` (boolean, Managed only, v2.1.229) to Plugin Settings — blocks plugins from sourcing slash-commands through the plugin command pipeline. Confirmed in v2.1.229 changelog | ✅ COMPLETE (added to Plugin Settings table) — NEW |
| 8 | HIGH | New Setting | Add `subagentStatusLine` (object `{type, command}`) to Status Line section — status line configuration for subagent sessions. Schema-confirmed (`json.schemastore.org/claude-code-settings.json`) | ✅ COMPLETE (added to Status Line Configuration section) — NEW |
| 9 | HIGH | New Setting | Add `managedMcpServers` (object, Managed/third-party Desktop deployments only) to MCP Settings — managed MCP server configurations pushed to all users. Schema-confirmed | ✅ COMPLETE (added to MCP Settings table) — NEW |
| 10 | HIGH | New Setting | Add `sshHostAllowlist` (array, Managed only) to Workspace & Teams — allowlist restricting Desktop SSH sessions to approved hosts. Case-insensitive patterns; `*` matches any host; `*.example.com` matches subdomains. Schema-confirmed | ✅ COMPLETE (added to Workspace & Teams table) — NEW |
| 11 | HIGH | New Setting | Add `requireCoworkFullVmSandbox` (boolean, Managed only) to Sandbox Settings — run Claude's tools inside an isolated VM; disables desktop iOS Simulator pane and simulator tools. Schema-confirmed | ✅ COMPLETE (added to Sandbox Settings table) — NEW |
| 12 | HIGH | New Setting | Add `sandbox.enabledPlatforms` (array enum `"macos"\|"linux"\|"wsl"\|"windows"`, Managed only) to Sandbox Settings — limit entire sandbox config to listed platforms; sandbox is inert on unlisted platforms. Schema-confirmed | ✅ COMPLETE (added to Sandbox Settings table) — NEW |
| 13 | HIGH | New Setting | Add `sandbox.ripgrep` (object `{command, args}`, user/managed/`--settings` only as of v2.1.232) to Sandbox Settings — override bundled ripgrep binary and arguments for Claude Code's ripgrep-based Grep tool. Schema-confirmed | ✅ COMPLETE (added to Sandbox Settings table) — NEW |
| 14 | MED | New Permission Mode | Add `delegate` permission mode to Permission Modes table — coordination-only mode for agent-team leads; gated behind `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS`. Schema-confirmed (marked UNDOCUMENTED in schema description) | ✅ COMPLETE (added to Permission Modes table with experimental annotation) — NEW |
| 15 | MED | Wrong Default | Fix `diffTool` default: `-` (none) → `"auto"`. JSON schema confirms `"auto"` is the default value | ✅ COMPLETE (default corrected) — NEW |
| 16 | MED | Wrong Type | Fix `teammateDefaultModel` type: `string` → `string \| null`. JSON schema confirms nullable | ✅ COMPLETE (type corrected) — NEW |
| 17 | MED | New Env Vars (Changelog) | Add `CLAUDE_CODE_TOOL_MEMORY_LIMIT`, `CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS`, `CLAUDE_CODE_ENABLE_TODO_TOOLS` — all v2.1.233 changelog; not yet on official env-vars page | ✅ COMPLETE (added with changelog-only annotations) — NEW |
| 18 | LOW | Suspect Key | `OTEL_LOG_TOOL_DETAILS` — still NOT on official /en/env-vars page after 59+ consecutive runs | ✋ ON HOLD (recurring from 2026-04-14 v2.1.107) |
| 19 | LOW | Suspect Key | `CLAUDE_CODE_RETRY_WATCHDOG` — still NOT on official /en/env-vars page | ✋ ON HOLD (recurring from 2026-07-03 v2.1.199; 15 consecutive runs) |
