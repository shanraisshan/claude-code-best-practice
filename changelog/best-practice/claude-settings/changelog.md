...
## [2026-09-22 10:44 AM PKT] Claude Code v2.1.278

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update version badge v2.1.252 → v2.1.278; update header counts from "140+ settings / 315+ env vars" to "145+ settings / 325+ env vars". Latest version as of 2026-09-19. Confirmed via official settings-reference page and changelog | ✅ COMPLETE (badge, version, and header updated) — NEW |
| 2 | HIGH | Wrong Description | Fix `fable` model alias — as of v2.1.257 the alias resolves to Fable 5.1 (not Fable 5) on the Anthropic API. Claude apps gateway still resolves to Fable 5. `ANTHROPIC_DEFAULT_FABLE_MODEL` pins a specific version; existing `claude-fable-5` settings are auto-migrated. Confirmed in v2.1.257 changelog | ✅ COMPLETE (fable alias description updated) — NEW |
| 3 | HIGH | Hook Count | Update hooks redirect from "28 hook events" to "30+ hook events" — additional events added through v2.1.278. Confirmed in changelog | ✅ COMPLETE (count updated) — NEW |
| 4 | HIGH | Missing Settings | Add `maxEffortLevel` (Managed only, string) to Managed-only policy keys — caps effort level across all providers. Confirmed in changelog | ✅ COMPLETE (added to Managed-only table) — NEW |
| 5 | HIGH | Missing Settings | Add `bashEditDiffEnabled` (boolean, default `true`, v2.1.269) to General Settings — includes diff of Bash-changed files in tool result. Confirmed in changelog | ✅ COMPLETE (added to General Settings) — NEW |
| 6 | HIGH | Missing Settings | Add `timeFormat` and `timeZone` (string, User/Project, v2.1.257) to Display Settings — customizable timestamp format and timezone for turn clock and transcripts. Confirmed in changelog | ✅ COMPLETE (both added to Display Settings) — NEW |
| 7 | HIGH | Missing Settings | Add `syncClaudeAiSkills` and `syncClaudeAiPlugins` (boolean, Session/Project, v2.1.275) to Plugin Settings — sync claude.ai account skills/plugins to terminal sessions. Confirmed in changelog | ✅ COMPLETE (both added to Plugin Settings) — NEW |
| 8 | HIGH | Missing Settings | Add `gatewayInternalNetworks` (array, Managed only, v2.1.277) to MCP Settings — allow `/login` on org IPv4 blocks. Confirmed in changelog | ✅ COMPLETE (added to MCP Settings) — NEW |
| 9 | HIGH | New Feature Note | Add AGENTS.md support note to General Settings — projects without CLAUDE.md can use AGENTS.md (v2.1.277); CLAUDE.md takes priority if both exist. Confirmed in changelog | ✅ COMPLETE (callout added) — NEW |
| 10 | HIGH | Missing Env Vars | Add 12 new env vars: `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS` (v2.1.269, workflow agent concurrency 1–256), `CLAUDE_CODE_AUTO_MODE_SERVER` (v2.1.278, opt-out of server-side classifier), `CLAUDE_CODE_WEBFETCH_DEADLINE_MS` (v2.1.278, WebFetch timeout override), `CLAUDE_GATEWAY_PROXY_IS_EGRESS_BOUNDARY` (v2.1.277, gateway egress proxy), `CLAUDE_GATEWAY_ALLOW_LOOPBACK` (v2.1.252+, loopback gateway), `CLAUDE_GATEWAY_DRAIN_TIMEOUT_MS` (v2.1.252+, gateway drain timeout), `CLAUDE_CODE_GATEWAY_HINT_HEADERS` (v2.1.278, gateway hint headers), `MCP_SDK_GENERATION` (v2.1.277, v1/v2 MCP client), `MCP_PROTOCOL_NEGOTIATION` (v2.1.277, MCP protocol version), `OTEL_LOG_MANAGED_SETTINGS` (v2.1.252+, log managed settings digest), `OTEL_METRICS_INCLUDE_REPOSITORY` (v2.1.252+, tag metrics with repo). Confirmed in changelog | ✅ COMPLETE (all 11 new env vars added) — NEW |
| 11 | MED | Wrong Description | Update `OTEL_LOG_TOOL_DETAILS` — as of v2.1.278, also includes agent/skill names in cost metrics and logging (not just tool_parameters). Confirmed in changelog | ✅ COMPLETE (description updated) — NEW |
| 12 | MED | Stale Annotation | Mark `TASK_MAX_OUTPUT_LENGTH` as DEPRECATED in v2.1.269 (TaskOutput tool removed). Confirmed in changelog | ✅ COMPLETE (deprecation note added) — NEW |
| 13 | MED | Stale Annotation | Mark `CLAUDE_CODE_ENABLE_TODO_TOOLS` as REMOVED in v2.1.269 (previously enabled legacy TodoWrite tools, now a no-op). Confirmed in changelog | ✅ COMPLETE (removal note added) — NEW |
