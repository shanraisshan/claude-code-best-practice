...
## [2026-08-30 10:55 AM PKT] Claude Code v2.1.251

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update version badge v2.1.224 → v2.1.251; update header counts from "127+ settings / 311 env vars" to "219+ settings / 346+ env vars" to match official settings-reference index (219 keys) and env-vars page (346 vars). Confirmed on official settings-reference and env-vars pages | ✅ COMPLETE (badge, header updated) — NEW |
| 2 | HIGH | Wrong Description | Fix `ultracode`: was described as "(Session-only — not persisted)" but official settings-reference confirms it is a valid `Any file` boolean setting. `ultracode: true` in `settings.json` starts sessions with ultracode on and takes precedence over `effortLevel`. Claude Code reads but never writes it. Confirmed on official settings-reference page | ✅ COMPLETE (characterization corrected) — NEW |
| 3 | HIGH | Wrong Type/Values | Fix `dialogExpiry` (ON HOLD from v2.1.224): type corrected from `number/30000ms` to `string/"5m"`. Values: `"60s"`, `"5m"` (default), `"10m"`, `"never"`. Added env override `CLAUDE_CODE_USER_DIALOG_TIMEOUT_MS`. Confirmed on official settings-reference page | ✅ COMPLETE (type, values, and env override corrected) — NEW |
| 4 | HIGH | Wrong Values | Fix `feedbackDrafts`: was documented as `"queue"` to enable. Official values are `"notify"` (default), `"quiet"`, `"off"`. Added `CLAUDE_CODE_SEND_FEEDBACK` env override. Confirmed on official settings-reference page | ✅ COMPLETE (values corrected) — NEW |
| 5 | HIGH | Wrong Type | Fix `spellcheck`: was documented as `boolean/false`. Official type is object `{enabled, checker, language, color}` with `checker` values `aspell`/`hunspell`/`ispell`/`auto`. User or managed only. Confirmed on official settings-reference page | ✅ COMPLETE (type and shape corrected) — NEW |
| 6 | HIGH | Wrong Type | Fix `modelPicker`: was documented as `array`. Official type is object `{options[], replaceBuiltInOptions}`. Confirmed on official settings-reference page | ✅ COMPLETE (type corrected) — NEW |
| 7 | HIGH | Missing Settings | Add `crossSessionInbound` (ON HOLD from v2.1.224): now confirmed on official settings-reference with scope `Any file`, values `"accept"/"hold"/"refuse"`, default `"hold"`, and managed-precedence exception (stricter value from project/local wins) | ✅ COMPLETE (added with full semantics) — NEW |
| 8 | HIGH | Missing Settings | Add 9 new settings keys confirmed on official settings-reference: `isolatePeerMachines` (Any file — cross-machine SendMessage approval), `enableWorkflows` (Any file — Pro plan opt-in), `managedSourcesBehavior` (Managed — `"first-wins"`/`"merge"` for admin sources, v2.1.242), `disableDesktopLocalSessions` (Managed), `sshHostAllowlist` (Managed), `skipAutoPermissionPrompt` (User/managed), `syncClaudeAiSkills` (User/local/managed), `disableCommandPluginSources` (Managed, v2.1.229), `autoContinueAtUsageLimit` (User/managed, unusual scope rule) | ✅ COMPLETE (all 9 added) — NEW |
| 9 | HIGH | Missing Settings | Add 3 new Display settings confirmed on official settings-reference: `promptSuggestionEnabled` (Any file), `terminalTitleFromRename` (Any file), and subagent status line key (`autoContinueAtUsageLimit` scope correction) | ✅ COMPLETE (added) — NEW |
| 10 | HIGH | Wrong Bash Pattern | Fix `Bash(git * main)` recommendation: v2.1.246 added a startup warning for wildcard-before-subcommand patterns because they match options inserted before the subcommand. Updated table example and wildcard notes. Confirmed in official permissions docs | ✅ COMPLETE (warning added, example fixed to `Bash(git push *)`) — NEW |
| 11 | HIGH | Missing Env Vars | Add 28 missing environment variables confirmed on official env-vars page: `ANTHROPIC_FEDERATION_RULE_ID`, `ANTHROPIC_ORGANIZATION_ID`, `ANTHROPIC_PROFILE`, `CLAUDE_CODE_PROMPT_CACHE_TTL`, `CLAUDE_CODE_SUBAGENT_PROMPT_CACHE_TTL`, `CLAUDE_CODE_RESTRICTED`, `CLAUDE_CODE_MESSAGING_SOCKET`, `CLAUDE_CODE_MESSAGING_TOKEN`, `CLAUDE_CODE_USER_DIALOG_TIMEOUT_MS`, `CLAUDE_CODE_SEND_FEEDBACK`, `CLAUDE_CODE_ARTIFACT_COMMENTS`, `CLAUDE_CODE_ARTIFACT_COMMENTS_AUTOREACT`, `CLAUDE_CODE_DISABLE_UNKNOWN_MODEL_WINDOW_ENFORCEMENT`, `CLAUDE_CODE_DISABLE_BEDROCK_CONTENT_TYPE_DEFAULT`, `CLAUDE_CODE_DISABLE_MTLS_RELOAD_ON_STALE_CONNECTION`, `CLAUDE_CODE_DISABLE_PERMISSION_PROMPT_NOTIFY_HOOKS`, `CLAUDE_CODE_TOOL_MEMORY_CGROUP_EXCLUDE`, `CLAUDE_STREAM_FIRST_BYTE_TIMEOUT_MS`, `CLAUDE_SUBAGENT_BG_SHELL_MAX_MS`, `CLAUDE_AX_STARTUP_QUIET_MS`, `CLAUDE_AX_PREPARK_MS`, `CLAUDE_CODE_BS_AS_CTRL_BACKSPACE`, `MCP_DISCOVERY_CACHE_TTL_S`, `MCP_DISCOVERY_CACHE_STRIKES`, `MCP_DISCOVERY_CACHE_MAX_STALE_S`, `MCP_PROTOCOL_NEGOTIATION`, `MCP_SDK_GENERATION`. Env vars from prior session: `ANTHROPIC_DEFAULT_MODEL`, `CLAUDE_CODE_WORKFLOW_PREFIX_STAGGER_MS`, `CLAUDE_CODE_GOAL_CHECKIN_MINUTES`, `CLAUDE_CODE_TOOL_MEMORY_LIMIT`, `CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS`, `SELF_HOSTED_RUNNER_CLIENT_LABEL`, `CLAUDE_CODE_PROJECT_DIR_NAME`, `CLAUDE_CODE_ENABLE_TODO_TOOLS` | ✅ COMPLETE (all added) — NEW |
| 12 | HIGH | Wrong Count | Fix hook events count from "26" to "28" — v2.1.251 added `PreModelSwitch` and `PostModelSwitch` hook events. Confirmed in v2.1.251 changelog | ✅ COMPLETE (count corrected) — NEW |
| 13 | HIGH | Missing Source | Add `https://code.claude.com/docs/en/settings-reference` to Sources as the new authoritative per-key index page (supersedes `/docs/en/settings` for key listings as of v2.1.242+). Without this source, future audits that only fetch `/docs/en/settings` see zero keys | ✅ COMPLETE (added as first source) — NEW |
| 14 | HIGH | Wrong Scope | Fix `extraKnownMarketplaces` scope: was listed as "Project" — official settings-reference confirms scope is `Any file`. Corrected in plugin settings table | ✅ COMPLETE (scope corrected) — NEW |
| 15 | MED | Missing Settings | Add `promptCacheTtl` and `subagentPromptCacheTtl` (Any file, v2.1.243) to Model Configuration table. Both set cache TTL to `"5m"` or `"1h"`. Confirmed on official settings-reference | ✅ COMPLETE (added) — NEW |
| 16 | MED | Missing Settings | Add `modelPricing` (Managed only, v2.1.243) to Model Configuration table — contracted per-model pricing for `/cost`, status line, OTel metrics. Confirmed on official settings-reference | ✅ COMPLETE (added) — NEW |
| 17 | MED | Missing Settings | Add `keybindingFlavor` (Any file, v2.1.238, `"classic"`/`"readline"`) to Display Settings table. Confirmed on official settings-reference | ✅ COMPLETE (added) — NEW |
| 18 | MED | Version Behavior | Add per-model effort persistence note: v2.1.251 changed `/effort` to save the default effort level per model, so switching models preserves each model's own setting | ✅ COMPLETE (added to Effort Level section) — NEW |
| 19 | MED | Env Restriction | Add v2.1.251 restriction to `CLAUDE_CONFIG_DIR` and `CLAUDE_CODE_TMPDIR`: project `.claude/settings.json` `env` block can no longer set these variables. Shell or user/managed settings only | ✅ COMPLETE (restriction notes added) — NEW |
| 20 | LOW | Unverified Keys | v2.1.224 JWT masking options (`decode`, `maskClaims`, `awsPairs`/`sigv4`) for `sandbox.credentials.files[]` — now on official settings-reference (`sandbox.credentials.awsPairs`, `sandbox.credentials.sigv4`) but confidence on exact values still moderate. Run count for on-hold: 2 consecutive runs | ✋ ON HOLD (on official page but complex shape — deferred to dedicated sandbox audit run) — CONTINUED |
| 21 | LOW | Major Drift Deferred | Many additional HIGH/MED items from research agent deferred to next run: `spinnerTipsOverride` v2.1.247 object/tipsFile expansion, `/` path-prefix semantics correction (source-relative not project-root), `permissions.defaultMode` remote environment update (adds `"default"` and `"auto"` to allowed set), `managedSourcesBehavior` exceptions table, `statusLine` v2.1.251 new input fields (`rate_limits.spend_limit`, `prompt_cache`), 6+ more missing sandbox sub-keys, full `env` block "ignored variables" list, `subagentStatusLine` key, `CLAUDE_CODE_SUBAGENT_MODEL` v2.1.251 behavior change, `ANTHROPIC_CUSTOM_HEADERS` approval requirement. All confirmed on official settings-reference — held due to session context limits | ✋ ON HOLD (confirmed official — deferred to next scheduled run) — NEW |
