...
| 21 | LOW | New Settings | `crossSessionInbound` and `dialogExpiry` noted in v2.1.224 changelog. Not yet on official settings page — confidence 0.75 | COMPLETE (both now on official settings-reference page; added to Display Settings in 2026-08-23 run) — RESOLVED |
| 22 | LOW | Unverified Keys | v2.1.224 JWT masking options (`decode`, `maskClaims`, `awsPairs`/`sigv4`) for `sandbox.credentials.files[]` — not yet on official settings page, changelog-only, confidence 0.70 | COMPLETE (confirmed on official settings-reference page; `decode`, `maskClaims`, `awsPairs`, `sigv4` added to Sandbox section in 2026-08-23 run) — RESOLVED |

---

## [2026-08-23 10:45 AM PKT] Claude Code v2.1.241

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update version badge v2.1.224 → v2.1.241; update header counts to "140+ settings / 320+ env vars". Confirmed via official settings-reference and changelog | COMPLETE (badge and header updated) — NEW |
| 2 | HIGH | New Source | Add `/docs/en/settings-reference` to Sources section — official `/docs/en/settings` page restructured to a how-to guide; authoritative key list moved to the new reference page. Confirmed via official docs | COMPLETE (settings-reference link added, settings page description updated) — NEW |
| 3 | HIGH | Wrong Description | Fix `askUserQuestionTimeout` scope INVERSION: report said "only honored from project and local settings — managed and user settings values are ignored" — official settings-reference says scope is "User or managed" (exact opposite). Confirmed via official settings-reference page | COMPLETE (scope corrected to "Only honored from user settings or managed settings") — NEW |
| 4 | HIGH | Wrong Scope | Fix `allowedMcpServers` and `deniedMcpServers` scope: report said "Managed only"; official settings-reference says scope is "Any file". Confirmed via official settings-reference page | COMPLETE (scope updated from "Managed only" to "Any") — NEW |
| 5 | HIGH | Wrong Scope | Fix `extraKnownMarketplaces` scope: report said "Project"; official settings-reference says scope is "Any file". Confirmed via official settings-reference page | COMPLETE (scope updated from "Project" to "Any"; added note about `additionalMarketplaces` alias) — NEW |
| 6 | HIGH | Wrong Scope | Fix `forceLoginMethod` scope: report said "(Managed only)"; official settings-reference says scope is "Any file". Confirmed via official settings-reference page | COMPLETE (managed-only annotation removed) — NEW |
| 7 | HIGH | Wrong Description | Fix `ultracode` persistability: report said "(Session-only — not persisted)"; official settings-reference scope is "Any file" and model-config docs confirm it can be set in settings.json. Confirmed via official docs | COMPLETE (description updated to reflect persistability) — NEW |
| 8 | HIGH | Deprecated Setting | Mark `teammateDefaultModel` as REMOVED in v2.1.234: report documented it as a live setting. Official docs and changelog v2.1.234 confirm removal — teammates now follow the lead's model. Confidence 0.97 | COMPLETE (key marked REMOVED in v2.1.234 in Global Config table) — NEW |
| 9 | HIGH | Missing Model Alias | Add `best` model alias: uses Fable 5 where available, otherwise latest Opus. Confirmed via official settings-reference | COMPLETE (added to Model Aliases table) — NEW |
| 10 | HIGH | Wrong Description | Fix `"default"` alias: report said "Recommended for your account type"; official docs say it "clears any model override" and is not itself a model alias. Confirmed via official settings-reference | COMPLETE (description corrected) — NEW |
| 11 | HIGH | Wrong Description | Fix model alias provider resolution: Opus 5 now on Bedrock and Google Cloud's Agent Platform (not just Anthropic API); Sonnet aliases now Sonnet 4.5 on Bedrock/Google Cloud's Agent Platform/Foundry (was Sonnet 4.6). "Vertex AI" renamed to "Google Cloud's Agent Platform" per official docs | COMPLETE (provider matrix updated in Model Aliases table) — NEW |
| 12 | HIGH | Missing Settings | Add 8 missing Display Settings keys: `keybindingFlavor`, `spellcheck`, `subagentStatusLine`, `promptSuggestionEnabled`, `terminalTitleFromRename`, `isolatePeerMachines`, `crossSessionInbound`, `dialogExpiry`. All confirmed on official settings-reference page | COMPLETE (all 8 keys added to Display Settings table) — NEW |
| 13 | HIGH | Missing Settings | Add sandbox credential keys: `sandbox.credentials.awsPairs`, `sandbox.credentials.sigv4`, `sandbox.credentials.files[].decode`, `sandbox.credentials.files[].maskClaims`. Confirmed on official settings-reference page | COMPLETE (all 4 keys added to Sandbox table) — NEW |
| 14 | HIGH | Missing Env Var | Add `ANTHROPIC_DEFAULT_MODEL` (v2.1.236): sets startup model, complementing `ANTHROPIC_MODEL` — a `/model` pick persists and overrides this. Confirmed on official env-vars page | COMPLETE (added near ANTHROPIC_MODEL) — NEW |
| 15 | MED | Missing Plugin Settings | Add 3 missing plugin keys: `additionalMarketplaces` (alias for `extraKnownMarketplaces`, v2.1.232), `allowedMarketplaces` (alias for `strictKnownMarketplaces`, v2.1.232), `disableCommandPluginSources` (managed only, v2.1.229). Confirmed on official settings-reference page | COMPLETE (all 3 added to Plugin Settings table) — NEW |
| 16 | MED | Missing Info | Add `command` marketplace source type to source types list (v2.1.229). Confirmed on official settings-reference page | COMPLETE (added to marketplace source types) — NEW |
| 17 | MED | Missing Env Vars | Add 5 missing env vars: `CLAUDE_CODE_PROJECT_DIR_NAME` (v2.1.234), `CLAUDE_CODE_TOOL_MEMORY_LIMIT` (v2.1.233), `CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS` (v2.1.233), `CLAUDE_CODE_ENABLE_TODO_TOOLS` (v2.1.233), `CLAUDE_CODE_GOAL_CHECKIN_MINUTES` (v2.1.234). Confirmed on official env-vars page | COMPLETE (all 5 added to Environment Variables table) — NEW |
| 18 | LOW | Behavior Change | Update `CLAUDE_CODE_ENABLE_TASKS`: task tools no longer default on newer models (Opus 4.8, Sonnet 5, Fable 5, and newer) as of v2.1.233; added note about `CLAUDE_CODE_ENABLE_TODO_TOOLS=1` to restore. Confirmed via official docs | COMPLETE (description updated) — NEW |
