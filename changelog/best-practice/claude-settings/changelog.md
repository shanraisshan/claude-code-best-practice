...
## [2026-09-26 10:59 AM PKT] Claude Code v2.1.283

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update version badge v2.1.252 → v2.1.283; update header counts from "140+ settings / 315+ env vars" to "160+ settings / 370+ env vars". Confirmed via official settings-reference page | ✅ COMPLETE (badge and header updated) — NEW |
| 2 | HIGH | Wrong Default | Fix `skillListingMaxDescChars` default: `1536` → `300`. Fix `skillListingBudgetFraction` default: `0.01` → `0.1`. Confirmed in official settings-reference page | ✅ COMPLETE (both defaults corrected) — NEW |
| 3 | HIGH | Wrong Description | Fix `alwaysThinkingEnabled`: only setting to `false` has an effect (suppresses thinking summaries); setting to `true` is a no-op. Also has no effect on Opus 5.5 or Fable 5.1 which always show summaries. Confirmed in official settings-reference | ✅ COMPLETE (semantics corrected) — NEW |
| 4 | HIGH | Wrong Description | Fix `enableArtifact`: only `false` has effect (disables Artifacts); `true` is a no-op. Added note that `disableArtifact` (boolean) is a DEPRECATED alias. Confirmed in official settings-reference | ✅ COMPLETE (semantics corrected, deprecated alias noted) — NEW |
| 5 | HIGH | Wrong Description | Fix `advisorModel`: unset = advisor disabled; `"fable"` alias now works. Confirmed in official settings-reference | ✅ COMPLETE (description corrected) — NEW |
| 6 | HIGH | Wrong Type + Default | Fix `dialogExpiry`: type `string` (specific duration values), default `"5m"`, scope user or managed only, introduced v2.1.224. Was previously missing type/default/scope. Confirmed in official settings-reference | ✅ COMPLETE (type, default, scope, version added) — NEW |
| 7 | HIGH | Wrong Type | Fix `feedbackDrafts`: type `boolean` → `string` (`"notify"` / `"quiet"` / `"off"`), default `"notify"`. Confirmed in official settings-reference | ✅ COMPLETE (type and default corrected) — NEW |
| 8 | HIGH | Wrong Scope | Fix `autoMemoryDirectory`: accepted from any settings scope (was incorrectly documented as restricted). Confirmed in official settings-reference | ✅ COMPLETE (scope corrected) — NEW |
| 9 | HIGH | Wrong Key Path | Fix `permissions.skipDangerousModePermissionPrompt`: this is a top-level key, not nested under `permissions`. Confirmed in official settings-reference | ✅ COMPLETE (key path corrected) — NEW |
| 10 | HIGH | Wrong Scope | Fix `askUserQuestionTimeout`: scope is user or managed only (project/local ignored). Confirmed in official settings-reference | ✅ COMPLETE (scope restriction added) — NEW |
| 11 | HIGH | New Settings | Add 11 new General Settings keys: `isolatePeerMachines`, `syncClaudeAiSkills`, `syncClaudeAiPlugins`, `managedSourcesBehavior`, `maxEffortLevel`, `skipAutoPermissionPrompt`, `availableModelsMatch`, `deniedModels`, `disableDesktopLocalSessions`, `gatewayInternalNetworks`, `bashOutputMaxChars`. Confirmed in official settings-reference | ✅ COMPLETE (all 11 added to General Settings table) — NEW |
| 12 | HIGH | Missing Settings | Add `permissions.blockReadsOutsideWorkingDirectories` (boolean, managed only) to Permission Keys table. Confirmed in official settings-reference | ✅ COMPLETE (added to Permission Keys table) — NEW |
| 13 | HIGH | Missing Setting | Add `sandbox.ripgrep` (boolean) to Sandbox Settings table. Confirmed in official settings-reference | ✅ COMPLETE (added to Sandbox Settings table) — NEW |
| 14 | HIGH | Missing Settings | Add 8 new Display Settings: `maxProseWidth`, `timeFormat`, `timeZone`, `promptSuggestionEnabled`, `terminalTitleFromRename`, `subagentStatusLine`, `bashEditDiffEnabled`, `copyOnSelect`. Confirmed in official settings-reference | ✅ COMPLETE (all 8 added to Display Settings table) — NEW |
| 15 | HIGH | New Model Alias | Add `best` to Model Aliases table (maps to most capable model). Update `opus` alias to Opus 5.5 (as of v2.1.280). Confirmed in official settings-reference | ✅ COMPLETE (alias added, opus updated) — NEW |
| 16 | HIGH | Wrong Description | Fix `effortLevel`: default is medium on Opus 5.5 (not just Opus 4.7/4.8). Effort per-model saves now go to `modelSettings` since v2.1.251. Confirmed in official settings-reference | ✅ COMPLETE (description updated) — NEW |
| 17 | HIGH | Wrong Description | Fix `modelSettings`: scope user/managed/`--settings` only (not project/local). Supports `maxEffortLevel`. Confirmed in official settings-reference | ✅ COMPLETE (scope and maxEffortLevel noted) — NEW |
| 18 | HIGH | Wrong Type | Fix `modelPicker`: type is object `{options, replaceBuiltInOptions?}` not array. Confirmed in official settings-reference | ✅ COMPLETE (type corrected) — NEW |
| 19 | HIGH | Wrong Type | Fix `strictKnownMarketplaces`: type boolean → array of marketplace source objects. Confirmed in official settings-reference | ✅ COMPLETE (type corrected) — NEW |
| 20 | HIGH | Wrong Description | Fix Write/NotebookEdit path rules: never consulted in any allow rule since v2.1.210. Updated permissions table and description. Confirmed in official permissions docs | ✅ COMPLETE (description corrected) — NEW |
| 21 | HIGH | Wrong Scope | Fix `allowedMcpServers`/`deniedMcpServers`: scope Any (not Managed only). Confirmed in official settings-reference | ✅ COMPLETE (scope corrected) — NEW |
| 22 | HIGH | Missing Settings | Add `managedMcpServers` (object, managed only) and `allowClaudeInChromeWithManagedMcp` (boolean, managed only) to MCP Settings table. Confirmed in official settings-reference | ✅ COMPLETE (both added to MCP Settings table) — NEW |
| 23 | HIGH | Wrong Example | Fix MCP matching example: `serverCommand` should be exact array, not string. Confirmed in official settings-reference | ✅ COMPLETE (example corrected) — NEW |
| 24 | HIGH | Wrong TTL | Fix `promptCacheTtl`/`subagentPromptCacheTtl`: values are `"5m"` or `"1h"` strings; correct env override variable names. Confirmed in official settings-reference | ✅ COMPLETE (values and env names corrected) — NEW |
| 25 | HIGH | Deprecated Setting | Mark `keybindingFlavor` as DEPRECATED since v2.1.261. Confirmed in official settings-reference | ✅ COMPLETE (deprecated annotation added) — NEW |
| 26 | HIGH | Removed Setting | Mark `permissionExplainerEnabled` as REMOVED in v2.1.257. Confirmed in official settings-reference | ✅ COMPLETE (marked removed) — NEW |
| 27 | HIGH | Wrong Scope | Fix `axScreenReader`: accepted from any settings file (was incorrectly restricted). Confirmed in official settings-reference | ✅ COMPLETE (scope corrected) — NEW |
| 28 | HIGH | Quick Reference | Update Quick Reference: remove `alwaysThinkingEnabled: true` (no-op), fix skill listing defaults, remove top-level `mcpServers`. Confirmed against current settings schema | ✅ COMPLETE (Quick Reference updated) — NEW |
| 29 | HIGH | Removed Env Var | Mark `TASK_MAX_OUTPUT_LENGTH` as REMOVED in v2.1.277. Mark `CLAUDE_CODE_MAX_SUBAGENTS_PER_SESSION` as REMOVED in v2.1.224. Mark `CLAUDE_CODE_ENABLE_OPUS_4_7_FAST_MODE` as REMOVED in v2.1.142. All confirmed via changelog | ✅ COMPLETE (all 3 marked removed) — NEW |
| 30 | HIGH | Wrong Description | Fix `CLAUDE_CODE_RETRY_WATCHDOG`: retries 429/529 capacity errors indefinitely (not non-capacity). Confirmed in official env-vars page | ✅ COMPLETE (description corrected) — NEW |
| 31 | MED | Effort Level | Fix `effortLevel` XHigh: applicable to Opus 5.5 with default medium. Confirmed in official settings-reference | ✅ COMPLETE (Opus 5.5 added to XHigh column) — NEW |
| 32 | MED | Wrong Default | Fix `MAX_STRUCTURED_OUTPUT_RETRIES` default: 3 → 5. Confirmed in official env-vars page | ✅ COMPLETE (default corrected) — NEW |
| 33 | MED | Wrong Default | Fix `CLAUDE_CODE_WORKFLOW_PREFIX_STAGGER_MS` default: 0 → 5000. Confirmed via changelog research | ✅ COMPLETE (default corrected) — NEW |
| 34 | MED | Wrong Description | Fix `CLAUDE_CODE_FORK_SUBAGENT`: on by default in interactive sessions; `0` disables. Removed stale "not yet on official page" annotation. Confirmed in official env-vars page | ✅ COMPLETE (description corrected, annotation removed) — NEW |
| 35 | MED | Wrong Description | Fix `CLAUDE_CODE_SAFE_MODE`: "Disables all customizations; same as --safe-mode". Fix `MCP_CONNECT_TIMEOUT_MS`: "blocking-startup batch wait". Confirmed in official env-vars page | ✅ COMPLETE (both corrected) — NEW |
| 36 | MED | Stale Annotations | Remove "not yet on official env-vars page" annotations from 10 env vars now confirmed on official page: `CLAUDE_CODE_PROCESS_WRAPPER`, `CLAUDE_CODE_MAX_WEB_SEARCHES_PER_SESSION`, `MCP_CONNECTION_NONBLOCKING`, `CLAUDE_CODE_MCP_AUTO_BACKGROUND_MS`, `CLAUDE_CODE_OTEL_CONTENT_MAX_LENGTH`, `OTEL_LOG_RAW_API_BODIES`, `OTEL_LOG_USER_PROMPTS`, `OTEL_LOG_ASSISTANT_RESPONSES`, `CLAUDE_CODE_MCP_SERVER_NAME`, `CLAUDE_CODE_MCP_SERVER_URL`. Confirmed in official env-vars page | ✅ COMPLETE (all 10 annotations removed) — NEW |
| 37 | MED | Missing Env Vars | Add 29 new env vars confirmed in official env-vars page: `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS`, `CLAUDE_CODE_PROMPT_CACHE_TTL`, `CLAUDE_CODE_SUBAGENT_PROMPT_CACHE_TTL`, `CLAUDE_CODE_SUBAGENT_MODEL_FORCE`, `CLAUDE_CODE_SEND_FEEDBACK`, `CLAUDE_CODE_USER_DIALOG_TIMEOUT_MS`, `CLAUDE_STREAM_FIRST_BYTE_TIMEOUT_MS`, `CLAUDE_CODE_WEBFETCH_DEADLINE_MS`, `CLAUDE_CODE_BASH_EDIT_DIFF`, `CLAUDE_CODE_AUTO_MODE_SERVER`, `CLAUDE_CODE_GATEWAY_HINT_HEADERS`, `CLAUDE_CODE_RESTRICTED`, `CLAUDE_CODE_PLUGIN_DIRS`, `CLAUDE_CODE_MAX_MCP_DESCRIPTION_LENGTH`, `CLAUDE_CODE_MCP_STARTUP_WAIT_MS`, `MCP_SDK_GENERATION`, `MCP_PROTOCOL_NEGOTIATION`, `OTEL_LOG_MANAGED_SETTINGS`, `OTEL_METRICS_INCLUDE_REPOSITORY`, `CLAUDE_CODE_DISABLE_DANGEROUS_RM_TIMEOUT`, `CLAUDE_CODE_DISABLE_SUBSTITUTION_RM_PROMPT`, `CLAUDE_CODE_DISABLE_POWERSHELL_CMD_RM_DENY`, `VERTEX_REGION_CLAUDE_5_5_OPUS`, `VERTEX_REGION_CLAUDE_FABLE_5_1`, `ANTHROPIC_PROFILE`, `ANTHROPIC_FEDERATION_RULE_ID`, `ANTHROPIC_ORGANIZATION_ID` | ✅ COMPLETE (all 29 added to env vars table) — NEW |
| 38 | LOW | Sources | Update Sources section: `settings-reference` now has 234 rows (not ~180); `env-vars` page now has 370+ vars (not 315+). Added 3 new source links. Confirmed by audit | ✅ COMPLETE (counts and links updated) — NEW |
