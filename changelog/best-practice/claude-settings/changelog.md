...
## [2026-09-20 10:45 AM PKT] Claude Code v2.1.278

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update version badge v2.1.252 → v2.1.278; update header counts from "140+ settings / 315+ env vars" to reflect ~17 new settings and ~20 new env vars | ✅ COMPLETE (badge updated to v2.1.278 / Sep 20 2026; header updated to 160+ settings / 355+ env vars) |
| 2 | HIGH | Removed Setting | Mark `permissionExplainerEnabled` (Global Config) as removed in v2.1.257. Confirmed in v2.1.257 changelog | ✅ COMPLETE (marked "Removed in v2.1.257" in Global Config table) |
| 3 | HIGH | Removed Setting | Mark `taskOutputMaxChars` as removed in v2.1.274 (TaskOutput tool removed; use Read instead). Confirmed in v2.1.274 changelog | ✅ COMPLETE (description updated with "No-op since v2.1.277" note; also added matching note to table) |
| 4 | HIGH | Removed Env Var | Mark `TASK_MAX_OUTPUT_LENGTH` as removed in v2.1.274. Confirmed in v2.1.274 changelog | ✅ COMPLETE (description updated with "No-op since v2.1.274" note) |
| 5 | HIGH | Missing Settings | Add 9 new general/display settings confirmed on official settings-reference page: `bashEditDiffEnabled` (bool), `bashOutputMaxChars` (num), `isolatePeerMachines` (bool), `maxEffortLevel` (num/obj), `promptSuggestionEnabled` (bool), `skipAutoPermissionPrompt` (bool), `subagentStatusLine` (obj), `syncClaudeAiSkills` (bool), `syncClaudeAiPlugins` (bool) | ✅ COMPLETE (all 9 keys added to General Settings table in previous session) |
| 6 | HIGH | Missing Settings | Add 6 new managed settings confirmed on settings-reference page: `disableDesktopLocalSessions` (bool), `gatewayInternalNetworks` (array), `managedMcpServers` (array), `managedSourcesBehavior` (string), `sshHostAllowlist` (array), `copyOnSelect` (bool, Global Config) | ✅ COMPLETE (5 managed keys + copyOnSelect added in previous session) |
| 7 | HIGH | Missing Setting | Add `permissions.blockReadsOutsideWorkingDirectories` (boolean, Any file) — refuse reads outside working directories. Confirmed on official settings-reference page | ✅ COMPLETE (added to Permissions table in previous session) |
| 8 | HIGH | Stale Annotation | Remove "not on official settings page" annotation from `sandbox.ignoreViolations` — now confirmed present on official settings-reference page. Confirmed by fetching settings-reference | ✅ COMPLETE (annotation removed in previous session) |
| 9 | HIGH | Hook Count | Update hooks redirect blurb from "28 hook events" to "30 hook events" — `SubagentStop` added in v2.1.269, `PermissionRequest` added in v2.1.271. Confirmed in changelogs | ✅ COMPLETE (count updated to "30" in previous session) |
| 10 | HIGH | Missing Env Vars | Add 15 new env vars: `CLAUDE_CODE_AUTO_MODE_SERVER` (v2.1.278), `CLAUDE_CODE_MCP_STARTUP_WAIT_MS` (v2.1.274), `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS` (v2.1.269), `CLAUDE_CODE_GATEWAY_MODEL_DISCOVERY_TIMEOUT_MS` (v2.1.269), `CLAUDE_CODE_WEBFETCH_DEADLINE_MS` (v2.1.268), `CLAUDE_CODE_GATEWAY_HINT_HEADERS` (v2.1.273), `CLAUDE_GATEWAY_DRAIN_TIMEOUT_MS` (v2.1.274), `CLAUDE_GATEWAY_ALLOW_LOOPBACK` (v2.1.277), `CLAUDE_GATEWAY_PROXY_IS_EGRESS_BOUNDARY` (v2.1.277), `MCP_PROTOCOL_NEGOTIATION` (v2.1.274), `MCP_SDK_GENERATION` (v2.1.274), `OTEL_LOG_MANAGED_SETTINGS` (v2.1.274), `OTEL_METRICS_INCLUDE_REPOSITORY` (v2.1.269), `NODE_EXTRA_CA_CERTS` (v2.1.277), `ANTHROPIC_PROFILE` (official env-vars page). All confirmed from official sources | ✅ COMPLETE (all 15 env vars added to table) |
| 11 | HIGH | Missing Env Vars | Add 6 new env vars from official env-vars page: `ANTHROPIC_ORGANIZATION_ID`, `ANTHROPIC_FEDERATION_RULE_ID`, `ENABLE_BETA_TRACING_DETAILED`, `BETA_TRACING_ENDPOINT`, `CLAUDE_AX_STARTUP_QUIET_MS`, `CLAUDE_AX_PREPARK_MS`. All confirmed on official env-vars page | ✅ COMPLETE (all 6 env vars added to table) |
| 12 | MED | Deprecated Setting | Note `keybindingFlavor` as deprecated and having no effect (v2.1.278). Confirmed in changelog | ✅ COMPLETE (deprecated note added in previous session) |
| 13 | MED | sandbox.ripgrep | Add `sandbox.ripgrep` (string, User or managed) — path to own ripgrep binary inside sandbox. Confirmed on official settings-reference page | ✅ COMPLETE (added to Sandbox table in previous session) |
| 14 | MED | Example Update | Add `sandbox.ripgrep` and `enableWorkflows` / `syncClaudeAiSkills` to Quick Reference example to demonstrate new capabilities | ✅ COMPLETE (Quick Reference example already includes these keys; `alwaysThinkingEnabled` corrected from `true` to `false`) |
| 15 | MED | Model Alias | Update `fable` alias from "Claude Fable 5" to "Claude Fable 5.1"; add `best` alias row | ✅ COMPLETE (fable description updated; `best` alias added to model aliases table) |
| 16 | MED | Model Pricing | Add 10× multiplier note to `modelPricing` (v2.1.271) | ✅ COMPLETE (added "Multiplier values up to 10× are supported (v2.1.271)") |
| 17 | MED | spinnerTipsOverride | Add `tipsFile` and `label` fields; note project/local scope restriction | ✅ COMPLETE (description expanded with new fields and scope restriction) |
| 18 | MED | Permission Mode | Add bypassPermissions scope restriction to `permissions.defaultMode` (v2.1.257 — now ignored in project/local settings) | ✅ COMPLETE (v2.1.257 note added) |
| 19 | MED | MCP Scope Fix | Update `allowedMcpServers` description: as of v2.1.259, governs only user-added servers; managed servers exempt | ✅ COMPLETE (v2.1.259 note added) |
| 20 | LOW | Sources | Update Sources "~180 keys" → "~231 keys" to match current settings-reference count | ✅ COMPLETE (updated to ~231 keys) |
| 21 | LOW | Env Var Effort | Add `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` (v2.1.257) — force all subagents to a specific model | ✅ COMPLETE (added to env vars table) |
| 22 | LOW | Vertex Alias | Add `VERTEX_REGION_CLAUDE_FABLE_5_1` (v2.1.278) alongside legacy `VERTEX_REGION_CLAUDE_FABLE_5` | ✅ COMPLETE (added to Vertex region env vars section; legacy key annotated) |
