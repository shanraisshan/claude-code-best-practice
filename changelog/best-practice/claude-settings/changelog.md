...
## [2026-07-28 10:56 AM PKT] Claude Code v2.1.220

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Error Correction | Rename `CLAUDE_CODE_BASH_MAINTAIN_PROJECT_WORKING_DIR` back to `CLAUDE_BASH_MAINTAIN_PROJECT_WORKING_DIR` (no `_CODE_`) — July 27 run #10 was wrong. Official /en/env-vars page WebFetch confirmed the correct name is `CLAUDE_BASH_MAINTAIN_PROJECT_WORKING_DIR`. Also fixed description to official wording: "Return to the original working directory after each Bash or PowerShell command in the main session (1 to enable)" | ✅ COMPLETE (name and description corrected; reverts July 27 error) — RECURRING (incorrectly renamed July 27 v2.1.220) |
| 2 | HIGH | Permission Docs Fix | Update `Write`/`NotebookEdit`/`Glob` permission rules note: corrected from "Existing settings continue to work; the warning is a nudge to tighten permissions" to "**accepted but never matched**" (v2.1.210). Added: `Read` deny also blocks `Edit` on same path (v2.1.208); `dir/**` allow vs deny depth asymmetry — allow rules match only direct children at cwd, deny/ask rules match at any depth (v2.1.214) | ✅ COMPLETE (permission docs corrected and expanded in permission section) — NEW |
| 3 | HIGH | Permission Docs | Add note to `permissions.defaultMode` that `"auto"` value is ignored in project/local settings to prevent repo injection (v2.1.142). Confirmed in v2.1.142 changelog | ✅ COMPLETE (note added to permissions.defaultMode description) — NEW |
| 4 | HIGH | Bash Docs Fix | Expand Bash wrapper stripping list: add `command`, `builtin`, and zsh `noglob` as stripped prefixes. Add note that `command -v` and `nocorrect` are NOT stripped. Add env var assignment stripping note (`VAR=value cmd` → env var prefix stripped) | ✅ COMPLETE (Bash permission section updated with additional stripped wrappers) — NEW |
| 5 | MED | New Setting | Add `processWrapper` (string, unset) to General Settings table — wrap the Claude Code process at startup with a profiling/tracing tool. Env-var equivalent `CLAUDE_CODE_PROCESS_WRAPPER` already in report | ✅ COMPLETE (added to General Settings table after `askUserQuestionTimeout`) — NEW |
| 6 | MED | Display Settings | Add `theme` (string, default `"dark"`) to Display Settings table — controls the Claude Code UI color theme (dark/light/daltonism variants). Add `verbose` (boolean, default `false`) to Display Settings table — enable verbose diagnostic output | ✅ COMPLETE (both added to Display Settings table after `emojiCompletionEnabled`) — NEW |
| 7 | MED | Global Config Settings | Add `diffTool` (string, default `"auto"`) and `permissionExplainerEnabled` (boolean, default `true`) to Global Config Settings table | ✅ COMPLETE (both added to Global Config Settings table after `teammateDefaultModel`) — NEW |
| 8 | MED | Missing Env Vars | Add `CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS` (default 20, caps parallel agent fan-out, v2.1.217) and `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` (default 3, max nesting depth, v2.1.219) to env vars table | ✅ COMPLETE (both added after `CLAUDE_CODE_MAX_SUBAGENTS_PER_SESSION`) — NEW |
| 9 | MED | Missing Env Var | Add `FORCE_HYPERLINK` (set `0` to opt out of OSC 8 terminal hyperlinks, v2.1.217) to env vars table | ✅ COMPLETE (added near terminal display env vars section) — NEW |
| 10 | MED | Missing Env Vars | Add 6 Vertex AI region env vars for current models: `VERTEX_REGION_CLAUDE_4_7_OPUS`, `VERTEX_REGION_CLAUDE_4_8_OPUS`, `VERTEX_REGION_CLAUDE_5_SONNET`, `VERTEX_REGION_CLAUDE_5_OPUS`, `VERTEX_REGION_CLAUDE_FABLE_5`, `VERTEX_REGION_CLAUDE_HAIKU_4_5` | ✅ COMPLETE (added after `VERTEX_REGION_CLAUDE_4_1_OPUS`) — NEW |
| 11 | LOW | Suspect Key Recurrence | `CLAUDE_CODE_RETRY_WATCHDOG` — still NOT on official /en/env-vars page; entry annotated "*(in v2.1.199 changelog, not on official env-vars page)*" | ✋ ON HOLD (RECURRING from 2026-07-03 v2.1.199; 14 consecutive runs) |
| 12 | LOW | Suspect Key Recurrence | `OTEL_LOG_TOOL_DETAILS` — still NOT on official /en/env-vars page after 59+ consecutive runs; entry annotated "*(in v2.1.85 changelog, not yet on official env-vars page)*" | ✋ ON HOLD (RECURRING from 2026-04-14 v2.1.107; 59+ consecutive runs) |
