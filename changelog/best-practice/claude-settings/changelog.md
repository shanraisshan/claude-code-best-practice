...
## [2026-09-18 10:48 AM PKT] Claude Code v2.1.276

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update version badge v2.1.252 → v2.1.276; update header counts to "200+ settings / 335+ env vars". Confirmed via official settings-reference page (200+ keys) and changelog | ✅ COMPLETE (badge and header updated) — NEW |
| 2 | HIGH | Security Correction | Fix `Write`/`NotebookEdit`/`Glob`/`MultiEdit` rule claim: report incorrectly stated these work in `deny`/`ask` rule types. Official permissions docs confirm they are **never consulted in any rule type** — only `Edit(path)` and `Read(path)` are checked. Writing `"deny": ["Write(./secrets/**)"]` provides zero protection. Rewritten note with security warning. Confirmed by v2.1.272 changelog fix and official permissions page | ✅ COMPLETE (security warning added; table rows corrected) — NEW |
| 3 | HIGH | Changed Behavior | Fix `permissions.defaultMode: "bypassPermissions"` — v2.1.257 changed it to be ignored when set in project or local settings (like `"auto"`). Must be in user or managed settings, or passed via `--permission-mode`. Confirmed in v2.1.257 changelog | ✅ COMPLETE (v2.1.257 restriction added) — NEW |
| 4 | HIGH | Wrong Description | Fix `askUserQuestionTimeout` scope — report said "only honored from project and local settings"; official settings-reference Scope is `User or managed`. These are exact opposites. Confirmed on official settings-reference page | ✅ COMPLETE (scope corrected to "User or managed") — NEW |
| 5 | HIGH | Missing Settings | Add `permissions.blockReadsOutsideWorkingDirectories` (boolean, v2.1.257+) — makes the Read tool refuse reads outside working directories in every permission mode. Confirmed in v2.1.257 changelog and official index | ✅ COMPLETE (added to Permission Keys table) — NEW |
| 6 | HIGH | Missing Settings | Add `maxEffortLevel` (string, v2.1.267+) — caps effort level across all providers; lowest cap from any scope wins over higher managed cap. Also settable per-model via `modelSettings.<model>.maxEffortLevel`. Confirmed in v2.1.267 changelog and official index | ✅ COMPLETE (added to Model Overrides table with managed exception) — NEW |
| 7 | HIGH | Missing Settings | Add `syncClaudeAiSkills` and `syncClaudeAiPlugins` (boolean, v2.1.275+) — opt out of syncing claude.ai skills/plugins into terminal sessions. Scope exceptions: `false` from managed/`--settings`/user/local wins, but NOT from `.claude/settings.json`. Confirmed in v2.1.275 changelog | ✅ COMPLETE (both added to General Settings table) — NEW |
| 8 | HIGH | Missing Settings | Add `managedMcpServers` (array, Managed, v2.1.259+) — admin-provided HTTP/SSE MCP servers for all users. Confirmed in v2.1.259 changelog | ✅ COMPLETE (added to MCP Settings table) — NEW |
| 9 | HIGH | Wrong Scope | Fix `allowedMcpServers`/`deniedMcpServers` scope: report said "Managed only"; official settings-reference Scope is `Any file`. Additionally, v2.1.259 changed `allowedMcpServers` to govern only user-added servers — `managed-mcp.json` servers load regardless. Confirmed on official settings-reference page and v2.1.259 changelog | ✅ COMPLETE (scope corrected to "Any file"; semantic change noted) — NEW |
| 10 | HIGH | Missing Settings | Add `bashEditDiffEnabled` (boolean, User or managed, v2.1.269+) — appends a diff of files changed by a Bash command to the result. Confirmed in v2.1.269 changelog | ✅ COMPLETE (added to Display Settings table) — NEW |
| 11 | HIGH | Missing Settings | Add `gatewayInternalNetworks` (array, Managed, v2.1.268+) — CIDR ranges for the Claude apps gateway internal network routing. Confirmed in v2.1.268 changelog | ✅ COMPLETE (added to Managed-only policy keys table) — NEW |
| 12 | HIGH | Missing Settings | Add `timeFormat` and `timeZone` (v2.1.257+) for clock formatting in turn-end and transcript timestamps. Confirmed in official settings-reference index | ✅ COMPLETE (both added to Display Settings table) — NEW |
| 13 | HIGH | Hook Count | Update hooks redirect from "28 hook events" to "30 hook events". Confirmed by research agent (2 new events added after v2.1.252) | ✅ COMPLETE (count updated) — NEW |
| 14 | HIGH | Stale Model | Update `"fable"` alias from Fable 5 to **Fable 5.1** (`claude-fable-5-1`) — new default since v2.1.257, 1M context, with gateway caveat. Confirmed in v2.1.257 changelog | ✅ COMPLETE (alias description updated) — NEW |
| 15 | HIGH | Deprecated | Add `keybindingFlavor` deprecation note: no-op since v2.1.261 — word-editing keys now unconditionally match Bash behavior. Confirmed in v2.1.261 changelog | ✅ COMPLETE (deprecation noted) — NEW |
| 16 | HIGH | Removed Setting | Mark `permissionExplainerEnabled` as REMOVED in v2.1.257. The setting is no longer honored. Confirmed in v2.1.257 changelog | ✅ COMPLETE (marked REMOVED) — NEW |
| 17 | HIGH | Missing Settings | Add `bashOutputMaxChars` and `taskOutputMaxChars` (v2.1.261+, up to 128K chars). Confirmed in v2.1.261 changelog | ✅ COMPLETE (both added to General Settings) — NEW |
| 18 | HIGH | Missing Settings | Add `managedSourcesBehavior` (merge behavior for managed sources), `managedMcpServers` (HTTP/SSE MCP), `promptSuggestionEnabled` (prompt suggestion visibility). Confirmed in official index | ✅ COMPLETE (added where confirmed) — NEW |
| 19 | HIGH | Managed Exceptions | Expand "Exceptions to managed settings precedence" table from 4 inline mentions to a full 9-key table including `enableArtifact`, `isolatePeerMachines`, `syncClaudeAiSkills`, `syncClaudeAiPlugins`, `maxEffortLevel`. Confirmed in official settings docs | ✅ COMPLETE (table added) — NEW |
| 20 | HIGH | Parse Error | Document managed settings fail-closed behavior (v2.1.259): Claude Code refuses to start if any managed-settings file cannot be parsed. Confirmed in v2.1.259 changelog | ✅ COMPLETE (added to hierarchy section) — NEW |
| 21 | MED | Missing Settings | Add `copyOnSelect` to `~/.claude.json` table (copy-on-select in fullscreen rendering). Confirmed in official settings-reference index | ✅ COMPLETE (added to Global Config table) — NEW |
| 22 | MED | Missing Env Vars | Add 20 new env vars from v2.1.268–276 window: `CLAUDE_CODE_WEBFETCH_DEADLINE_MS`, `CLAUDE_CODE_GATEWAY_MODEL_DISCOVERY_TIMEOUT_MS`, `OTEL_METRICS_INCLUDE_REPOSITORY`, `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS`, `CLAUDE_CODE_BG_TASKS_REPORT_RUNNING`, `CLAUDE_CODE_GATEWAY_HINT_HEADERS`, `CLAUDE_CODE_AUTO_MODE_SERVER`, `CLAUDE_CODE_MCP_STARTUP_WAIT_MS`, `OTEL_LOG_MANAGED_SETTINGS`, `MCP_SDK_GENERATION`, `MCP_PROTOCOL_NEGOTIATION`, `CLAUDE_GATEWAY_DRAIN_TIMEOUT_MS`, `CLAUDE_CODE_SUBAGENT_MODEL_FORCE`, `ANTHROPIC_FEDERATION_RULE_ID`, `ANTHROPIC_ORGANIZATION_ID`, `ANTHROPIC_PROFILE`, `BETA_TRACING_ENDPOINT`, `CLAUDE_AX_PREPARK_MS`, `CLAUDE_AX_STARTUP_QUIET_MS`. Confirmed mix of official env-vars page and changelog | ✅ COMPLETE (all 19 added) — NEW |
| 23 | MED | modelPricing | Document markup multipliers (> 1, up to 10) in `modelPricing` for internal chargeback rates (v2.1.271). Confirmed in v2.1.271 changelog | ✅ COMPLETE (note added to modelPricing description) — NEW |
| 24 | LOW | Sources Accuracy | Update settings-reference annotation from "~180 keys" to "200+ keys". Confirmed by fetching the page | ✅ COMPLETE (annotation updated) — NEW |
