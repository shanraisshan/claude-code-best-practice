...
## [2026-09-13 10:44 AM PKT] Claude Code v2.1.270

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update version badge v2.1.252 → v2.1.270; update header counts from "140+ settings / 315+ env vars" to "160+ settings / 320+ env vars". Confirmed via official settings-reference page | ✅ COMPLETE (badge and header updated) — NEW |
| 2 | HIGH | Wrong Scope | Fix `askUserQuestionTimeout` scope — report states "Only honored from project and local settings — managed and user settings values are ignored" which is **exactly backwards**. Official settings-reference index: Scope = `User or managed`. Corrected to "User or managed settings only; project and local settings values are ignored". Confirmed on official settings-reference page (conf 0.93) | ✅ COMPLETE (scope corrected) — NEW |
| 3 | HIGH | Changed Behavior | Add `bypassPermissions` scope restriction (v2.1.257): `permissions.defaultMode: "bypassPermissions"` is now ignored when set in project (`.claude/settings.json`) or local settings — a repository cannot grant itself bypass mode. Use `~/.claude/settings.json` instead. Previously only `auto` had this restriction (v2.1.142). Updated both `defaultMode` description and `bypassPermissions` mode row. Confirmed on official settings docs (conf 0.92) | ✅ COMPLETE (both rows updated) — NEW |
| 4 | HIGH | Missing Setting | Add `maxEffortLevel` (string, Any file) to Model Overrides table — caps the effort level for every model or per model, on every provider. Also a managed-precedence exception: the lowest cap from any scope wins. Introduced v2.1.267. Confirmed on official settings-reference page (conf 0.97) | ✅ COMPLETE (added to Model Overrides table) — NEW |
| 5 | HIGH | Missing Settings | Add `bashOutputMaxChars` (number, Any file, v2.1.261) and `taskOutputMaxChars` (number, Any file, v2.1.261) to General Settings — cap Bash tool and task output to up to 128K characters. Confirmed in v2.1.261 changelog (conf 0.95) | ✅ COMPLETE (both added to General Settings) — NEW |
| 6 | HIGH | Missing Setting | Add `managedMcpServers` (object[], Managed only, v2.1.259) to MCP Settings — provide remote MCP servers to every user alongside the ones they add. Confirmed on official settings-reference page (conf 0.95) | ✅ COMPLETE (added to MCP Settings table) — NEW |
| 7 | HIGH | Missing Setting | Add `permissions.blockReadsOutsideWorkingDirectories` (boolean, Any file, v2.1.257) to Permission Keys — make file tools refuse reads outside working directories in every permission mode. Confirmed on official settings-reference page (conf 0.96) | ✅ COMPLETE (added to Permission Keys table) — NEW |
| 8 | HIGH | Missing Settings | Add `timeFormat` (string, Any file, v2.1.257) and `timeZone` (string, Any file, v2.1.257) to Display Settings — configure 12/24-hour display or strftime format and timezone. Confirmed in v2.1.257 changelog (conf 0.94) | ✅ COMPLETE (both added to Display Settings) — NEW |
| 9 | HIGH | Missing Setting | Add `managedSourcesBehavior` (string, Managed only) to Managed-only settings keys; amend L51 statement "only one managed source wins" to clarify this is the default and can be changed to `"merge"` to compose every managed source. Confirmed on official settings-reference page (conf 0.92) | ✅ COMPLETE (key added and hierarchy note updated) — NEW |
| 10 | HIGH | Missing Setting | Add `isolatePeerMachines` (boolean, Any file) to General Settings — ask before Claude messages one of your sessions on another machine. Confirmed on official settings-reference page (conf 0.93) | ✅ COMPLETE (added to General Settings) — NEW |
| 11 | HIGH | Wrong Scope | Fix `allowedMcpServers` and `deniedMcpServers` scope from "Managed only" → "Any" — official settings-reference index shows Scope = `Any file`. Confirmed on official settings-reference page (conf 0.88) | ✅ COMPLETE (scope column updated for both) — NEW |
| 12 | MED | Missing Setting | Add `bashEditDiffEnabled` (boolean, Any file, v2.1.270) to Display Settings — display file change diffs when the Bash tool handles file edits directly. Confirmed in v2.1.270 changelog (conf 0.90) | ✅ COMPLETE (added to Display Settings) — NEW |
| 13 | MED | Missing Settings | Add `syncClaudeAiSkills` (boolean) to managed-settings precedence exceptions table (false from any scope wins); add `enableWorkflows` (boolean, Any file) to General Settings. Confirmed on official settings-reference page (conf 0.90) | ✅ COMPLETE (syncClaudeAiSkills added to exceptions; enableWorkflows added to General Settings) — NEW |
| 14 | MED | Missing Settings | Add `skipAutoPermissionPrompt` (boolean, User or managed), `promptSuggestionEnabled` (boolean, Any file), `copyOnSelect` (boolean, Global config), `sandbox.ripgrep` (string path, User or managed), `sshHostAllowlist` (string[], Managed only), `disableDesktopLocalSessions` (boolean, Managed only), `gatewayInternalNetworks` (string[], Managed only, v2.1.268). Confirmed on official settings-reference page (conf 0.90) | ✅ COMPLETE (all 7 added to appropriate tables) — NEW |
| 15 | MED | Removed Setting | Mark `keybindingFlavor` as **removed in v2.1.268** — Bash-style is now the default, and the setting was removed entirely. Previously added in v2.1.236. Confirmed in v2.1.268 changelog (conf 0.92) | ✅ COMPLETE (row marked REMOVED in v2.1.268) — NEW |
| 16 | MED | Missing Env Vars | Add 5 missing env vars: `CLAUDE_CODE_GATEWAY_MODEL_DISCOVERY_TIMEOUT_MS` (v2.1.269, default 3s gateway model discovery timeout), `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS` (v2.1.269, concurrent agent limit 1–256), `CLAUDE_CODE_WEBFETCH_DEADLINE_MS` (v2.1.268, override 300s WebFetch timeout), `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` (v2.1.257, force subagent model regardless of agent definitions), `OTEL_METRICS_INCLUDE_REPOSITORY` (v2.1.269, tag metrics with VCS attributes). All confirmed in changelog (conf 0.92) | ✅ COMPLETE (all 5 added to env vars table) — NEW |
| 17 | MED | Missing Model Alias | Add Claude Fable 5.1 (`claude-fable-5-1`, v2.1.257, 1M context) to Model Aliases table. Confirmed in v2.1.257 changelog (conf 0.92) | ✅ COMPLETE (Fable 5.1 row added) — NEW |
| 18 | MED | Wrong Attribution | Fix hooks redirect version attribution: `PreModelSwitch` and `PostModelSwitch` were added in **v2.1.251**, not v2.1.252 as stated. Confirmed in v2.1.251 changelog (conf 0.88) | ✅ COMPLETE (v2.1.252 → v2.1.251 corrected) — NEW |
| 19 | MED | Broken Link | Fix broken anchor in `claude-cli-startup-flags.md` line 211: `#environment-variables` → `#environment-variables-via-env`. The anchor was correct on surrounding lines but broken on this one. Confirmed by checking the heading in claude-settings.md (conf 0.92) | ✅ COMPLETE (anchor corrected) — NEW |
