...
## [2026-08-08 10:47 AM PKT] Claude Code v2.1.226

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update version badge v2.1.224 → v2.1.226; update header from "127+ settings / 311 env vars" to "130+ settings / 335 env vars". Confirmed via changelog and agent findings | COMPLETE (badge and header updated) — NEW |
| 2 | HIGH | Inverted Scope | Fix `askUserQuestionTimeout` scope: was "project and local settings" (inverted) — should be "user settings only". `/config` writes this key to user settings; project/local values are ignored. Confirmed by Agent 1 raw-markdown fetch | COMPLETE (scope corrected) — NEW |
| 3 | HIGH | Missing Settings | Add `crossSessionInbound` (string, accept/hold/refuse), `dialogExpiry` (string, default "5m"), and `isolatePeerMachines` (boolean) — all v2.1.224. `crossSessionInbound` and `dialogExpiry` confirmed on official settings page via WebFetch; `isolatePeerMachines` confirmed by Agent 1 raw-markdown fetch (conf. 0.97). Previous items 21–22 now RESOLVED | COMPLETE (all 3 keys added to General Settings table and Quick Reference example) — RESOLVED (first seen: 2026-08-07 items 21–22) |
| 4 | HIGH | Missing Sandbox Sub-Keys | Add 10 missing `sandbox.credentials` sub-keys: `files[].decode` ("jwt" mode), `files[].maskClaims`, `envVars[].extract`, `envVars[].decode`, `envVars[].maskClaims`, `envVars[].onExtractNoMatch`, `envVars[].injectHosts` (plus `envVars[].name`, `envVars[].mode`), `sandbox.credentials.awsPairs`, `sandbox.credentials.sigv4`. All v2.1.224. Confirmed by Agent 1 raw-markdown fetch (conf. 0.95–0.96). Previous item 22 now RESOLVED | COMPLETE (all sub-keys added to Sandbox table) — RESOLVED (first seen: 2026-08-07 item 22) |
| 5 | HIGH | Missing Setting | Add `subagentStatusLine` (object) to Display Settings — custom status line for the agent panel. Confirmed by Agent 1 from official statusline docs (conf. 0.95; v2.1.205+) | COMPLETE (added to Display Settings table) — NEW |
| 6 | HIGH | Wrong/Stale Data | Fix status line `github` field: split into `workspace.repo.host`, `workspace.repo.owner`, `workspace.repo.name` and `pr.number`, `pr.url`, `pr.review_state`. Old `github` field no longer reflects actual data shape. Confirmed by Agent 1 | COMPLETE (github field replaced with split fields) — NEW |
| 7 | HIGH | Wrong Description | Fix `context_window.total_input_tokens`: was "Cumulative token counts across the session" — actually reflects tokens **currently in the context window** from the most recent API call (not cumulative). Confirmed by Agent 1 | COMPLETE (semantics corrected) — NEW |
| 8 | MED | Missing Fields | Add `fast_mode` and `prompt_id` fields to status line input table. Confirmed by Agent 1 | COMPLETE (both fields added) — NEW |
| 9 | MED | Wrong Description | Fix `session_name` status line field: falls back to AI-generated session title (not absent) when no custom name is set. Confirmed by Agent 1 | COMPLETE (fallback behavior noted) — NEW |
| 10 | MED | Wrong Description | Fix `vim.mode` status line field: missing VISUAL and VISUAL LINE modes. Confirmed by Agent 1 | COMPLETE (VISUAL and VISUAL LINE added) — NEW |
| 11 | MED | Wrong Count | Fix hooks redirect: "26 hook events" → "30 hook events" (claude-code-hooks repo README now advertises 30). Confirmed by Agent 1 fetch of hooks repo README | COMPLETE (26 → 30) — NEW |
| 12 | MED | Stale Hierarchy | Add v2.1.211 `settings.local.json` repo-root resolution note: file is now found at the git repo root, not the CWD. Pre-v2.1.211 starting-directory file still read with repo-root winning for scalar keys | COMPLETE (hierarchy entry updated) — NEW |
| 13 | MED | Missing Restriction | Add HKCU exclusion from admin-source union keys: user-level Windows registry is not part of the union (only MDM, server-managed, and file-based admin sources participate). Confirmed by Agent 1 | COMPLETE (HKCU carve-out documented) — NEW |
| 14 | MED | Missing Exceptions | Add restrictive-value exceptions for `crossSessionInbound` (stricter project/local value wins) and `isolatePeerMachines` (any `true` wins). Confirmed by Agent 1 | COMPLETE (exceptions documented in hierarchy section) — NEW |
| 15 | MED | Missing Env Vars | Add 3 missing env vars: `CLAUDE_CODE_USER_DIALOG_TIMEOUT_MS` (overrides `dialogExpiry`, v2.1.224), `CLAUDE_CODE_MESSAGING_SOCKET` (system-only, ignored in `env`, v2.1.224), `CLAUDE_SUBAGENT_BG_SHELL_MAX_MS` (subagent shell cap). Confirmed by Agent 1 | COMPLETE (all 3 added) — NEW |
| 16 | LOW | Wrong Description | Fix `sandbox.credentials.files[].mode`: add macOS fallback note — `mask` falls back to `deny` on macOS with filesystem isolation on. Confirmed by Agent 1 | COMPLETE (macOS caveat added) — NEW |
| 17 | LOW | Missing Source | Add [Claude Code Cross-Session Messaging](https://code.claude.com/docs/en/cross-session-messaging) to Sources — reference page for the 3 new v2.1.224 remote keys | COMPLETE (source added) — NEW |
| 18 | LOW | Stale Example | Update Quick Reference example with `crossSessionInbound`, `dialogExpiry`, `isolatePeerMachines` (v2.1.224 keys) | COMPLETE (3 keys added to example) — NEW |
| 19 | LOW | ON HOLD | `strictKnownMarketplaces` type: Agent 1 claims `array` but direct WebFetch of official settings page shows `boolean`. Contradiction between sources — deferring per Rule 8A until manually verified | ON HOLD (conflicting source data — confidence 0.60, needs manual verification) — NEW |
| 20 | LOW | ON HOLD | Permissions gaps: `mcp__*` deny glob, `EndConversation` exception, read-only Bash command set. Agent 1 found these via raw markdown; not captured in direct WebFetch. Pending manual verification | ON HOLD (not confirmed by direct fetch — needs permissions page read) — NEW |
| 21 | LOW | ON HOLD | `CLAUDE_CODE_RETRY_WATCHDOG` not on official env-vars page | ON HOLD (recurring — not on official page) — RECURRING (first seen: multiple prior runs) |
| 22 | LOW | ON HOLD | `OTEL_LOG_TOOL_DETAILS` not on official env-vars page | ON HOLD (recurring — not on official page) — RECURRING (first seen: multiple prior runs) |
