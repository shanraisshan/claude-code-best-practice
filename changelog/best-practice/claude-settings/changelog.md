...
## [2026-08-17 10:43 AM PKT] Claude Code v2.1.233

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update badge v2.1.224 → v2.1.233; update header counts "127+ settings / 311 env vars" → "133+ settings / 328+ env vars" | ✅ COMPLETE (badge and counts updated) — NEW |
| 2 | HIGH | Wrong Scope | Fix `askUserQuestionTimeout` scope: was listed as "Any" but official docs say "Managed only". CRITICAL — mislead users to set it in project settings where it's ignored | ✅ COMPLETE (scope corrected to "Managed only") — RECURRING (first seen: 2026-08-07) |
| 3 | HIGH | Wrong Scope | Fix `autoMemoryDirectory` scope: was listed as "Any" but official docs say "User + managed only" | ✅ COMPLETE (scope corrected) — RECURRING (first seen: 2026-08-07) |
| 4 | HIGH | Missing Settings | Add 5 new top-level settings confirmed in v2.1.225–233 changelog: `crossSessionInbound` (bool, managed only), `dialogExpiry` (number, managed only), `isolatePeerMachines` (bool, managed only), `subagentStatusLine` (bool), `promptSuggestionEnabled` (bool) | ✅ COMPLETE (all 5 added to General Settings table) — NEW |
| 5 | HIGH | Missing Settings | Add 8 sandbox credential sub-keys: `matchMode`, `extractGroup`, `extractIndex`, `injectMode`, `injectFormat`, `saltSecret`, `maskCharCount`, `passthroughHosts`. Confirmed in v2.1.221–233 changelog | ✅ COMPLETE (all 8 added to Sandbox table) — NEW |
| 6 | HIGH | Missing Settings | Add 3 missing Plugin settings: `additionalMarketplaces`, `allowedMarketplaces`, `disableCommandPluginSources`. Confirmed on official settings docs page | ✅ COMPLETE (all 3 added to Plugin Settings table) — NEW |
| 7 | HIGH | Missing Env Vars | Add 16 missing environment variables: `CLAUDE_CODE_TOOL_MEMORY_LIMIT`, `CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS`, `CLAUDE_CODE_ENABLE_TODO_TOOLS`, `CLAUDE_CODE_DISABLE_PERMISSION_PROMPT_NOTIFY_HOOKS`, `CLAUDE_CODE_WORKFLOW_PREFIX_STAGGER_MS`, `MCP_SDK_GENERATION`, `CLAUDE_CODE_MESSAGING_SOCKET`, `CLAUDE_CODE_MESSAGING_TOKEN`, `CLAUDE_CODE_USER_DIALOG_TIMEOUT_MS`, `CLAUDE_SUBAGENT_BG_SHELL_MAX_MS`, `CLAUDE_CODE_DISABLE_UNKNOWN_MODEL_WINDOW_ENFORCEMENT`, `CLAUDE_CODE_DISABLE_MTLS_RELOAD_ON_STALE_CONNECTION`, `CLAUDE_AX_STARTUP_QUIET_MS`, `ANTHROPIC_FEDERATION_RULE_ID`, `ANTHROPIC_ORGANIZATION_ID`, `ANTHROPIC_PROFILE` | ✅ COMPLETE (all 16 added to env vars table) — NEW |
| 8 | HIGH | Hook Count | Fix hook count: "26 hook events" → "31 hook events" in Hooks section intro (SubagentStart, SubagentStop, PreCompact, SessionStart, SessionEnd, Setup, PermissionRequest, TeammateIdle, TaskCompleted, ConfigChange added in recent versions) | ✅ COMPLETE (count updated) — NEW |
| 9 | HIGH | Missing Feature | Add `command` marketplace source type to Plugin Settings. Official docs now list: `github`, `gitlab`, `url`, `path`, `archive`, `command`. Confirmed on official docs | ✅ COMPLETE (source type added) — NEW |
| 10 | HIGH | Missing Status Line Fields | Add 5 missing status line input fields: `workspace.repo.host`, `pr.number`, `pr.review_state`, `fast_mode`, `prompt_id`. Annotate `github` row as deprecated in favor of split fields. Confirmed in v2.1.228–233 changelog | ✅ COMPLETE (all 5 added, github deprecated) — NEW |
| 11 | MED | Missing Env Var | Annotate 5 env vars as "(not in official docs — unverified)": `INIT_PROMPT`, `CLAUDE_CODE_USER_EMAIL`, `CLAUDE_CODE_ORGANIZATION_UUID`, `CLAUDE_CODE_PLAN_MODE_REQUIRED`, `CLAUDE_CODE_TEAM_NAME`. Per Rule 1F inverse completeness | ✅ COMPLETE (all 5 annotated) — NEW |
| 12 | MED | Incomplete Description | Update `CLAUDE_CODE_ENABLE_TASKS`: add note that Task/Todo tools removed on Opus 4.8, Sonnet 5, Fable 5, and newer — use `CLAUDE_CODE_ENABLE_TODO_TOOLS=1` to restore | ✅ COMPLETE (note added) — NEW |
| 13 | MED | Changed Description | Expand `disableBypassPermissionsMode`: add v2.1.223 note — also prevents subagents from running in bypassPermissions mode even if the parent session requested it | ✅ COMPLETE (subagent note added) — RECURRING (first seen: 2026-08-07) |
| 14 | MED | Changed Description | Fix `autoMode` description: add `--settings` flag as a way to start in auto mode alongside `--permission-mode auto`. Confirmed in official CLI reference | ✅ COMPLETE (flag added) — RECURRING (first seen: 2026-08-07) |
| 15 | MED | Missing Note | Fix `plansDirectory`: add "Relative paths are resolved from the project root" note. Confirmed on official settings docs page | ✅ COMPLETE (resolution note added) — NEW |
| 16 | MED | Missing Note | Fix `effortLevel`: add note that `max` and `xhigh` can be pinned via `CLAUDE_CODE_EFFORT_LEVEL` env var which takes precedence over this setting | ✅ COMPLETE (env var carve-out noted) — NEW |
| 17 | MED | Missing Permissions | Add `Glob` and `EndConversation` to Tool Permission Syntax table. `Glob` valid in deny/ask only (same pattern as Write/NotebookEdit). `EndConversation` for SDK session control | ✅ COMPLETE (both rows added) — NEW |
| 18 | MED | Changed Description | Fix `sandbox.enabled`: add "(macOS, Linux, and WSL2)" platform note. Confirmed on official settings docs page | ✅ COMPLETE (platform note added) — RECURRING (first seen: 2026-08-07) |
| 19 | MED | Missing Feature | Fix `sandbox.network.allowedDomains`: add IPv6 bracket notation note (`[::1]`, `[fc00::]/7`). Confirmed in v2.1.228 changelog | ✅ COMPLETE (IPv6 note added) — NEW |
| 20 | MED | Wrong Version | Fix `sandbox.credentials.files[].onExtractNoMatch`: version was v2.1.221, correct version is v2.1.224. Confirmed in v2.1.224 changelog | ✅ COMPLETE (version corrected) — NEW |
