...
## [2026-09-06 10:52 AM PKT] Claude Code v2.1.263

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update version badge v2.1.252 → v2.1.263; update header env var count from "315+" to "340+" (actual table rows: 341) | ✅ COMPLETE (badge and header updated) — NEW |
| 2 | HIGH | Missing Settings | Add `timeFormat` (timestamp display: "12h"/"24h"/"24h-UTC"/strftime) and `timeZone` (IANA identifier) to General Settings table. Confirmed on official settings-reference page v2.1.257 | ✅ COMPLETE (both added to General Settings table) — NEW |
| 3 | HIGH | Missing Settings | Add `bashOutputMaxChars` and `taskOutputMaxChars` (number, output truncation caps) to General Settings table. Confirmed in v2.1.261 changelog and settings-reference | ✅ COMPLETE (both added to General Settings table) — NEW |
| 4 | HIGH | Missing Setting | Add `permissions.blockReadsOutsideWorkingDirectories` (boolean) to Permission Keys table — blocks Read calls outside working directories in sandboxed sessions. Confirmed on official settings-reference page v2.1.257 | ✅ COMPLETE (added to Permission Keys table) — NEW |
| 5 | HIGH | Missing Setting | Add `managedMcpServers` (object, Managed only) to MCP Settings table — org-provided HTTP/SSE MCP servers via managed settings. Confirmed on official settings-reference page v2.1.259 | ✅ COMPLETE (added to MCP Settings table) — NEW |
| 6 | HIGH | Changed Behavior | Update `permissions.defaultMode` — add v2.1.257 restriction: `"bypassPermissions"` is now also ignored in project and local settings; only user and managed settings can set it | ✅ COMPLETE (bypassPermissions restriction note added to description) — NEW |
| 7 | HIGH | Deprecated Setting | Mark `permissionExplainerEnabled` as removed in v2.1.257 — "Removed the Ctrl+E command explanation on Bash and PowerShell permission prompts." Remove from Global Config Settings table | ✅ COMPLETE (row removed) — NEW |
| 8 | HIGH | Deprecated Behavior | Mark `keybindingFlavor` as deprecated/inert in v2.1.261 — "the prompt's word-editing keys now match Bash … keybindingFlavor no longer has any effect." Update description to prevent users from setting a no-op key | ✅ COMPLETE (description updated with deprecation notice) — NEW |
| 9 | HIGH | Changed Scope | Fix `allowedMcpServers` and `deniedMcpServers` scope from "Managed only" → "Any" per official settings-reference. Add v2.1.259 upgrade-behavior warning: `allowedMcpServers` no longer filters `managed-mcp.json` servers | ✅ COMPLETE (scope corrected, upgrade warning added) — NEW |
| 10 | HIGH | Model Alias | Update `fable` alias: v2.1.257 made Claude Fable 5.1 (`claude-fable-5-1`) the default Fable model. Update from "Claude Fable 5" to "Claude Fable 5.1 (`claude-fable-5-1`)" | ✅ COMPLETE (alias updated) — NEW |
| 11 | HIGH | Missing Env Var | Add `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` (v2.1.257) — force-override model for all subagents, unlike `CLAUDE_CODE_SUBAGENT_MODEL` which is now a default. Confirmed on official env-vars page | ✅ COMPLETE (added to env vars table after CLAUDE_CODE_SUBAGENT_MODEL) — NEW |
| 12 | MED | Missing Env Var | Add `CLAUDE_CODE_RESTRICTED` (v2.1.248) — removes Bash/code tools from session. Confirmed on official env-vars page. Note: may belong in startup-flags doc (confidence 0.88 per agent); added here pending ownership review | ✅ COMPLETE (added to env vars table with note) — NEW |
| 13 | MED | Missing Env Var | Add `SELF_HOSTED_RUNNER_CLIENT_LABEL` (v2.1.248) — override runner label. Confirmed on official env-vars page. Note: possibly startup-flags only (confidence 0.85); added pending ownership review | ✅ COMPLETE (added to env vars table with note) — NEW |
| 14 | MED | Attribution Fix | Fix hook-event attribution at hooks redirect: `PreModelSwitch`/`PostModelSwitch` were added in **v2.1.251**, not v2.1.252. Confirmed in v2.1.251 changelog | ✅ COMPLETE (version corrected from v2.1.252 to v2.1.251) — NEW |
