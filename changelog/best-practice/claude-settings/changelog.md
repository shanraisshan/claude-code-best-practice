...
## [2026-09-11 10:43 AM PKT] Claude Code v2.1.268

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update version badge v2.1.252 → v2.1.268; update header counts from "140+ settings" to "220+ settings" and "315+ env vars" to "350+ env vars" | ✅ COMPLETE (badge and header updated) — NEW |
| 2 | HIGH | Missing Settings | Add `bashOutputMaxChars` (number, default 30000, clamped 4000–128000, v2.1.261) and `taskOutputMaxChars` (number, default 32000, clamped 4000–128000, v2.1.261) to General Settings — raise inline output limits for bash commands and background tasks respectively. Confirmed in v2.1.261 changelog | ✅ COMPLETE (both added to General Settings table; defaults and versions corrected) — NEW |
| 3 | HIGH | Missing Settings | Add `maxEffortLevel` (string/object, v2.1.267) to Model Overrides — caps effort level across all providers (Bedrock, Vertex, Foundry, Anthropic). Accepts same values as `effortLevel` or per-model object. Confirmed in v2.1.267 changelog | ✅ COMPLETE (added to Model Overrides table) — NEW |
| 4 | HIGH | Missing Settings | Add `timeFormat` (string, default "12h", v2.1.257) and `timeZone` (string, v2.1.257) to Display Settings — configure timestamp display format and timezone. Confirmed in v2.1.257 changelog | ✅ COMPLETE (both added to Display Settings table) — NEW |
| 5 | HIGH | Missing Settings | Add `managedMcpServers` (object keyed by server name, Managed only, v2.1.259) to MCP Settings — organization-deployed HTTP/SSE MCP servers pushed to all users; load regardless of `allowedMcpServers` allowlist. Fix `allowedMcpServers`/`deniedMcpServers` scope from "Managed only" to "Any file". Confirmed in v2.1.259 changelog | ✅ COMPLETE (added to MCP Settings table; scopes corrected) — NEW |
| 6 | MED | Missing Env Var | Add `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` (v2.1.267) — force-override model for every subagent, ignoring per-spawn agent definition overrides. Companion to `CLAUDE_CODE_SUBAGENT_MODEL`. Confirmed in v2.1.267 changelog | ✅ COMPLETE (added to Model Environment Variables) — NEW |
| 7 | MED | Missing Status Line Field | Add `prompt_cache.miss_reason` to Status Line Input Fields — reports cache miss reason (model_changed, effort_changed, context_cleared, tools_changed, unknown) on the last turn (v2.1.268). Confirmed in v2.1.268 changelog | ✅ COMPLETE (added to Status Line Input Fields table) — NEW |
