...
## [2026-09-02 10:45 AM PKT] Claude Code v2.1.258

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update version badge v2.1.224 → v2.1.258; update header counts from "127+ settings / 311 environment variables" to reflect newly confirmed keys | COMPLETE (badge and header updated) — NEW |
| 2 | HIGH | Source Accuracy | Add `/docs/en/settings-reference` as the primary source for all settings keys — official docs restructured: all per-key tables moved from `/docs/en/settings` (now narrative-only) to the new reference page. Add to Sources section | COMPLETE (settings-reference added to Sources) — NEW |
| 3 | HIGH | Wrong Behavior | Fix `permissions.defaultMode: "bypassPermissions"` — since v2.1.257 this value is ignored when set in project `.claude/settings.json` or `.claude/settings.local.json`; must be set in `~/.claude/settings.json` (user) or managed settings, or passed via `--permission-mode` flag. Confirmed on official settings-reference page | COMPLETE (scope restriction documented) — NEW |
| 4 | HIGH | Wrong Description | Fix `CLAUDE_CODE_SUBAGENT_MODEL` — v2.1.251 changed from "override" to "default-only": agent definition `model:` and per-spawn model now take precedence. Add `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` (v2.1.257) as the new true-override variable. Confirmed on official env-vars page | COMPLETE (semantics corrected, FORCE var added) — NEW |
| 5 | HIGH | Removed Setting | Remove `permissionExplainerEnabled` from report — settings-reference annotates it "Removed in v2.1.257". Confidence 0.62 on removal date; confirmed absent from current settings-reference. Removed with note | COMPLETE (marked removed, v2.1.257) — NEW |
| 6 | HIGH | Wrong Scope | Add scope caveat to `CLAUDE_CONFIG_DIR` and `CLAUDE_CODE_TMPDIR` env vars — since v2.1.251 project `.claude/settings.json` `env` can no longer set these (or `TMPDIR`/`TMP`/`TEMP`); shell, user, or managed settings only. Confirmed on official settings page | COMPLETE (scope restriction added to both vars) — NEW |
| 7 | HIGH | Anti-Pattern Example | Fix `Bash(git * main)` allow-rule example at permission syntax table — since v2.1.246 wildcard-before-subcommand patterns trigger a startup warning; recommend `Bash(git push origin main)` or positional-specific form. Confirmed in changelog v2.1.246 | COMPLETE (example and warning note updated) — NEW |
| 8 | HIGH | Missing Hierarchy | Add "Managed settings precedence exceptions" table to Settings Hierarchy — 7 keys use stricter-value-wins rather than priority-order: `disableClaudeAiConnectors`, `enableArtifact`, `isolatePeerMachines`, `remoteControlAtStartup`, `crossSessionInbound`, `useAutoModeDuringPlan`, `syncClaudeAiSkills`. Confirmed on official settings-reference page | COMPLETE (exceptions table added) — NEW |
| 9 | HIGH | Resolved ON HOLD | Resolve two previous ON HOLD items: (a) `crossSessionInbound`/`dialogExpiry` — now on official settings-reference (confidence 0.95/0.95), add to Remote/Desktop settings table; (b) JWT masking options (`sandbox.credentials.awsPairs`, `sandbox.credentials.sigv4`, `sandbox.credentials.files[].decode`, `files[].maskClaims`) — now confirmed on official settings-reference, add to Sandbox Credentials table | COMPLETE (all 4 keys added, ON HOLD items resolved) — NEW |
| 10 | MED | Missing Settings | Add 16 missing settings keys from official settings-reference (all confirmed, confidence ≥0.88): `timeFormat` (v2.1.257), `timeZone` (v2.1.257), `permissions.blockReadsOutsideWorkingDirectories` (v2.1.257), `spellcheck` (v2.1.235), `keybindingFlavor` (v2.1.238), `modelPicker` (v2.1.243), `promptCacheTtl` (v2.1.243), `subagentPromptCacheTtl` (v2.1.243), `modelPricing` (v2.1.243), `feedbackDrafts` (v2.1.247), `desktopSessionCleanupPeriodDays` (v2.1.248), `modelSettings` (v2.1.251), `sandbox.ripgrep` (v2.1.232), `autoContinueAtUsageLimit` (v2.1.234), `subagentStatusLine` (v2.1.234), `enableWorkflows`/`disableWorkflows` | COMPLETE (all added to appropriate sections) — NEW |
| 11 | MED | Missing Settings | Add 9 additional settings confirmed on official settings-reference (confidence 0.70–0.90): `skipAutoPermissionPrompt`, `syncClaudeAiSkills`, `disableCommandPluginSources`, `disableDesktopLocalSessions`, `sshHostAllowlist`, `isolatePeerMachines`, `managedSourcesBehavior`, `promptSuggestionEnabled`, `allowedChannelPlugins` | COMPLETE (added with confidence notes where applicable) — NEW |
| 12 | MED | Model Alias | Add Claude Fable 5.1 (`claude-fable-5-1`) as the new default for the `fable` and `best` aliases since v2.1.257. Confirmed in settings-reference model aliases table | COMPLETE (alias table updated) — NEW |
| 13 | MED | Wrong UX | Fix `/model` picker keystroke: report says "press `d` to set as default" — was replaced by `s` (set as session default); `d` no longer exists in the picker. Confirmed in changelog (v2.1.153 change carried forward) | COMPLETE (d → s corrected) — NEW |
| 14 | MED | Wrong Count | Fix hook events count in hooks redirect section: "26" → "28" — `PreModelSwitch` and `PostModelSwitch` added in v2.1.251. Confirmed in official hooks-guide page | COMPLETE (count updated) — NEW |
| 15 | MED | Missing Warning | Add approval requirement note to `ANTHROPIC_CUSTOM_HEADERS` — since v2.1.251 requires user approval when set from managed/project settings and carries credential, org/tenant, routing, or API-behavior header. Confirmed on official settings page | COMPLETE (approval note added) — NEW |
| 16 | MED | Wrong Behavior | Fix `policyHelper.timeoutMs`/`refreshIntervalMs` — since v2.1.257 both are clamped to the timer maximum (2147483647 ms); values above previously caused failures. Confirmed in v2.1.257 changelog | COMPLETE (clamping noted) — NEW |
| 17 | MED | Wrong Behavior | Fix `permissions.additionalDirectories` — since v2.1.257, network paths (UNC shares, `/net/<host>` automounts) are refused with a message; Windows requires a mapped drive letter. Confirmed in v2.1.257 changelog | COMPLETE (network path refusal documented) — NEW |
| 18 | MED | Missing Section | Add "Settings in cloud sessions" scope summary to Settings Hierarchy — shared project settings read; user + project-local NOT read in cloud; `/config` redirects to claude.ai settings. Confirmed on official settings page | COMPLETE (cloud sessions scope added) — NEW |
| 19 | MED | Incomplete Description | Update `spinnerTipsOverride` — now accepts rich object entries `{id, text, cooldownSessions, priority}` plus `tipsFile` and `label` fields alongside the original array form. Confirmed on official settings-reference page | COMPLETE (structure documented) — NEW |
| 20 | MED | Missing Source Type | Add `command` marketplace source type (v2.1.229) — a local command prints the plugin directory, re-resolved each session, applied without restart, with `mode: "link"` option. Confirmed in changelog v2.1.229 | COMPLETE (command source type added) — NEW |
| 21 | MED | Missing Syntax | Add `Monitor` row and `Glob(path)` row to Tool Permission Syntax table — `Monitor` allow rules exist and are set aside during active auto mode (v2.1.236); `Glob` is deny/ask-only. Confirmed on official permissions page | COMPLETE (both rows added) — NEW |
| 22 | LOW | Missing Env Vars | Add 10 missing env vars from official env-vars page (all confirmed ≥0.90): `ANTHROPIC_DEFAULT_MODEL` (v2.1.236), `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` (v2.1.257), `CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS` (v2.1.233), `CLAUDE_CODE_TOOL_MEMORY_LIMIT` (v2.1.233), `CLAUDE_CODE_ENABLE_TODO_TOOLS` (v2.1.233), `CLAUDE_CODE_PROJECT_DIR_NAME` (v2.1.234), `CLAUDE_CODE_GOAL_CHECKIN_MINUTES` (v2.1.234), `CLAUDE_CODE_RESTRICTED` (v2.1.248), `SELF_HOSTED_RUNNER_CLIENT_LABEL` (v2.1.248), `CLAUDE_CODE_WORKFLOW_PREFIX_STAGGER_MS` (v2.1.229) | COMPLETE (all 10 added to env vars table) — NEW |
| 23 | LOW | Removed Setting | Mark `teammateDefaultModel` as removed in v2.1.234 — "Default teammate model" removed from `/config`; teammates now use the leader's model unless the spawn names one. Absent from current settings-reference. Confirmed in v2.1.234 changelog | COMPLETE (marked removed) — NEW |
| 24 | LOW | Missing Behavior | Add Bash deny rule extension note — since v2.1.258, Bash `Read()`/`Edit()` deny rules apply to `< file` redirects and reader commands (`tac`, `egrep`). Also Grep/Glob now apply `Read(...)` deny rules through symlinked paths (v2.1.251). Confirmed in changelog | COMPLETE (notes added to permission behavior section) — NEW |
| 25 | LOW | Stale Example | Update Quick Reference example — replace stale `claude-opus-4-6` model reference; add examples for `timeFormat`, `permissions.blockReadsOutsideWorkingDirectories`, `crossSessionInbound`. Confirmed vs. settings-reference | COMPLETE (example updated) — NEW |
