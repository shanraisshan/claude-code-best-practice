...
## [2026-09-23 10:46 AM PKT] Claude Code v2.1.280

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update version badge v2.1.252 → v2.1.280; update header counts to "180+ settings / 360+ env vars". Confirmed via official settings-reference page | ✅ COMPLETE — NEW |
| 2 | HIGH | Missing Settings | Add 20+ new settings keys: `timeFormat`, `timeZone` (v2.1.257), `bashOutputMaxChars` (v2.1.261), `maxEffortLevel` (v2.1.267), `gatewayInternalNetworks` (v2.1.268), `bashEditDiffEnabled` (v2.1.269), `syncClaudeAiSkills`, `syncClaudeAiPlugins` (v2.1.275), `skipDangerousModePermissionPrompt` (top-level, moved from permissions), `skipAutoPermissionPrompt`, `enableWorkflows`, `isolatePeerMachines`, `promptSuggestionEnabled`, `subagentStatusLine`, `terminalTitleFromRename`, `disableAllHooks`, `disableDesktopLocalSessions`, `sshHostAllowlist`, `managedSourcesBehavior`, `autoMode.classifyAllShell`, `env`. All confirmed in official settings-reference | ✅ COMPLETE — NEW |
| 3 | HIGH | Wrong Type/Description | Fix `feedbackDrafts` type: `boolean` → `string` (`"notify"`/`"quiet"`/`"off"`, default `"notify"`). Fix `askUserQuestionTimeout` scope: project/local → User or managed. Fix `dialogExpiry` values: arbitrary durations → `"60s"`/`"5m"`/`"10m"`/`"never"` (default `"5m"`). Fix `autoContinueAtUsageLimit` default: blank → `true`. All confirmed in official settings-reference | ✅ COMPLETE — NEW |
| 4 | HIGH | Wrong Description | Update model aliases: `opus` → Opus 5.5 (`claude-opus-5-5`, v2.1.280); `sonnet` Bedrock/Vertex/Foundry → Sonnet 4.5; `fable` → Fable 5.1 (v2.1.257); add `best` alias (v2.1.257). Confirmed in v2.1.257 and v2.1.280 changelog | ✅ COMPLETE — NEW |
| 5 | HIGH | Wrong Description | Fix `modelPicker` shape: `[{id, label}]` array → `{options: [{model, label?, description?}], replaceBuiltInOptions?: bool}` object. Fix `effortLevel` per-model date: v2.1.243 → v2.1.251. Update Effort Level table `max`/`xhigh` notes for Opus 5.5 and Fable 5.1. Add `maxEffortLevel` to Model Overrides table. Confirmed in official settings-reference | ✅ COMPLETE — NEW |
| 6 | HIGH | Missing Settings (Permissions) | Move `skipDangerousModePermissionPrompt` from permissions sub-key to top-level (was a bug). Add `allowedHttpHookUrls`, `httpHookAllowedEnvVars`, `allowManagedHooksOnly` to Permission Keys. Add `permissions.blockReadsOutsideWorkingDirectories` (v2.1.257). Fix `permissions.defaultMode` description: `bypassPermissions` also ignored in project/local settings since v2.1.257. Confirmed in official settings-reference | ✅ COMPLETE — NEW |
| 7 | HIGH | Wrong Scope | Fix `allowedMcpServers`/`deniedMcpServers` scope: `"Managed only"` → `"Any file (entries merge)"`. Add `managedMcpServers` (v2.1.259, Managed only). Confirmed in official settings-reference | ✅ COMPLETE — NEW |
| 8 | HIGH | Wrong Type/Scope | Fix `strictKnownMarketplaces` type: `boolean` → array of source objects. Fix `extraKnownMarketplaces` scope: `"Project"` → `"Any file (gated by workspace trust)"`. Confirmed in official settings-reference | ✅ COMPLETE — NEW |
| 9 | HIGH | Wrong Description | Fix `diffTool` (Display/Global Config): external command → enum `"auto"`/`"terminal"`. Add `copyOnSelect` to Global Config Settings. Mark `permissionExplainerEnabled` as removed v2.1.257. Fix `spellcheck` shape: removed incorrect `binary` field; correct shape is `{enabled, checker, language, color}`. Mark `keybindingFlavor` as deprecated v2.1.261. Confirmed in official settings-reference and v2.1.257/261 changelog | ✅ COMPLETE — NEW |
| 10 | HIGH | Missing Key | Add `sandbox.ripgrep` (object with `command` and optional `args`) to Sandbox Settings. Confirmed in official settings-reference | ✅ COMPLETE — NEW |
| 11 | HIGH | Missing Env Vars | Add 21 new env vars since v2.1.252: `MCP_SDK_GENERATION` (v2.1.218), `MCP_PROTOCOL_NEGOTIATION` (v2.1.221), `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` (v2.1.257), `CLAUDE_CODE_DISABLE_CFC_PROMPT` (v2.1.257), `VERTEX_REGION_CLAUDE_FABLE_5_1` (v2.1.257), `CLAUDE_CODE_NONBLOCKING_STDOUT` (v2.1.261), `CLAUDE_CODE_WEBFETCH_DEADLINE_MS` (v2.1.268), `CLAUDE_CODE_BASH_EDIT_DIFF` (v2.1.269), `CLAUDE_CODE_BG_TASKS_REPORT_RUNNING` (v2.1.269), `CLAUDE_CODE_DISABLE_WINDOWS_SHELL_LAUNCHER` (v2.1.269), `CLAUDE_CODE_GATEWAY_MODEL_DISCOVERY_TIMEOUT_MS` (v2.1.269), `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS` (v2.1.269), `OTEL_METRICS_INCLUDE_REPOSITORY` (v2.1.269), `CLAUDE_CODE_AUTO_MODE_SERVER` (v2.1.271), `CLAUDE_CODE_GATEWAY_HINT_HEADERS` (v2.1.273), `CLAUDE_CODE_MCP_STARTUP_WAIT_MS` (v2.1.274), `CLAUDE_CODE_STARTUP_FAILURE_RESULTS` (v2.1.274), `OTEL_LOG_MANAGED_SETTINGS` (v2.1.274), `CLAUDE_CODE_MAX_MCP_DESCRIPTION_LENGTH` (v2.1.280), `CLAUDE_CODE_PLUGIN_DIRS` (v2.1.280), `VERTEX_REGION_CLAUDE_5_5_OPUS` (v2.1.280). Confirmed in official env-vars page and changelog | ✅ COMPLETE — NEW |
| 12 | HIGH | Removed Env Vars | Mark as removed: `TASK_MAX_OUTPUT_LENGTH` (v2.1.277), `CLAUDE_CODE_MAX_SUBAGENTS_PER_SESSION` (v2.1.224), `CLAUDE_CODE_ENABLE_OPUS_4_7_FAST_MODE` (v2.1.142). Confirmed in changelog | ✅ COMPLETE — NEW |
| 13 | MED | Wrong Description | Fix `desktopSessionCleanupPeriodDays` default (blank → `0` / no limit) and scope (→ User or managed). Fix `enableWorkflows` scope note. Confirmed in official settings-reference | ✅ COMPLETE — NEW |
| 14 | MED | Useful Commands | Update `/model` picker models (→ Fable 5.1, Opus 5.5, Sonnet 5, Haiku 4.5). Fix `--doctor` → `claude doctor`. Confirmed in v2.1.257+ changelog | ✅ COMPLETE — NEW |
| 15 | LOW | Stale Count | Fix Sources section key count: `~180 keys` → `~230 keys`. Consistent with official settings-reference row count | ✅ COMPLETE — NEW |
