...
## [2026-09-24 10:46 AM PKT] Claude Code v2.1.281

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update version badge v2.1.252 → v2.1.281; update header from "140+ settings / 315+ env vars" to "175+ settings / 365+ env vars". Confirmed on official settings-reference page (231 index rows) and env-vars page (368 rows) | COMPLETE (badge and header updated) — NEW |
| 2 | HIGH | New Settings | Add `syncClaudeAiSkills` (boolean, default true, Any file, v2.1.275) and `syncClaudeAiPlugins` (boolean, default true, Any file, v2.1.275) to Plugin Settings. Confirmed in v2.1.275 changelog and settings-reference | COMPLETE (both added to Plugin Settings table) — NEW |
| 3 | HIGH | New Settings | Add `permissions.blockReadsOutsideWorkingDirectories` (boolean, Any; `true` from any source wins, v2.1.257) to Permissions settings. Memory dirs auto-excluded. Confirmed on settings-reference | COMPLETE (added to Permissions table) — NEW |
| 4 | HIGH | New Settings | Add `maxEffortLevel` (number, Any; lowest cap wins, v2.1.267) to Model Configuration; add `bashOutputMaxChars` (number, range 4000–128000, default 30000, Any, v2.1.261) to General Settings. Confirmed on settings-reference | COMPLETE (both added) — NEW |
| 5 | HIGH | New Settings | Add `managedMcpServers` (array, Managed-only, v2.1.259) to MCP Settings; add `managedSourcesBehavior` (string, `"first-wins"` or `"merge"`, Managed-only, v2.1.257) to Managed Settings section. Confirmed on settings-reference | COMPLETE (both added) — NEW |
| 6 | HIGH | New Settings | Add `isolatePeerMachines` (boolean, Any; true wins, v2.1.224), `bashEditDiffEnabled` (boolean, User or managed, v2.1.269; paired with `CLAUDE_CODE_BASH_EDIT_DIFF`), `timeFormat` and `timeZone` (strings, Any, v2.1.257) to General Settings. Confirmed on settings-reference | COMPLETE (all 4 added) — NEW |
| 7 | HIGH | New Settings | Add `requiredMinimumVersion` and `requiredMaximumVersion` (strings, Managed-only, v2.1.280; refuse to start outside range) and `enforceAvailableModels` (boolean, Any, v2.1.280; enforce `availableModels` list when true) to General/Managed sections. Confirmed in v2.1.280 changelog and settings-reference | COMPLETE (all 3 added) — NEW |
| 8 | HIGH | Wrong Shape | Fix `modelPicker` — official shape is `{options:[{model,label?,description?}], replaceBuiltInOptions?}`; report's example `[{id,label}]` is invalid and would break configs. Confirmed on settings-reference | COMPLETE (shape and example corrected) — NEW |
| 9 | HIGH | Wrong Type | Fix `strictKnownMarketplaces` — type is **array of marketplace source objects**, not boolean. An empty array blocks everything including the official marketplace. Report description is wrong | COMPLETE (type and description fixed) — NEW |
| 10 | HIGH | Wrong Example | Fix `serverCommand` in `allowedMcpServers`/`deniedMcpServers` — must be an **array** (exact match), not a string glob. Also fix scope: these are Any file, not Managed-only. Confirmed on settings-reference and permissions page | COMPLETE (example and scope corrected) — NEW |
| 11 | HIGH | Wrong Type | Fix `dialogExpiry` — accepts duration strings (`60s`, `5m`, `10m`, `never`), default `5m`. Not a number as currently documented. Paired env var is `CLAUDE_CODE_USER_DIALOG_TIMEOUT_MS` | COMPLETE (type corrected; paired env var noted) — NEW |
| 12 | HIGH | Wrong Type | Fix `feedbackDrafts` — type is string (`notify`, `quiet`, `off`), default `notify`. Not boolean as documented. Paired env var is `CLAUDE_CODE_SEND_FEEDBACK` | COMPLETE (type corrected) — NEW |
| 13 | HIGH | Model Alias | Update `opus` alias: now resolves to `claude-opus-5-5` since v2.1.280 (report says Opus 5). Opus 5.5 defaults to `medium` effort (not `high`). Also add Foundry exception: `opus` is Opus 4.6 on Foundry | COMPLETE (alias and effort default updated) — NEW |
| 14 | HIGH | New Env Vars | Add `CLAUDE_CODE_AUTO_MODE_SERVER` (v2.1.278/281 — set to `0` to opt out of server-side auto mode classifier), `CLAUDE_CODE_DISABLE_DANGEROUS_RM_TIMEOUT` (v2.1.281), `CLAUDE_CODE_DISABLE_SUBSTITUTION_RM_PROMPT` (v2.1.281). Confirmed in changelog and official env-vars page | COMPLETE (all 3 added) — NEW |
| 15 | HIGH | New Env Vars | Add `CLAUDE_CODE_MAX_MCP_DESCRIPTION_LENGTH` (v2.1.280, default 2048), `CLAUDE_CODE_MCP_STARTUP_WAIT_MS` (v2.1.274), `CLAUDE_CODE_GATEWAY_HINT_HEADERS` (v2.1.273), `OTEL_LOG_MANAGED_SETTINGS` (v2.1.274), `OTEL_METRICS_INCLUDE_REPOSITORY` (v2.1.269). Confirmed in changelog | COMPLETE (all 5 added) — NEW |
| 16 | HIGH | New Env Vars | Add auth vars: `ANTHROPIC_PROFILE`, `ANTHROPIC_ORGANIZATION_ID`, `ANTHROPIC_FEDERATION_RULE_ID`; Vertex: `VERTEX_REGION_CLAUDE_5_5_OPUS` (v2.1.280), `VERTEX_REGION_CLAUDE_FABLE_5_1` (v2.1.257); Agent: `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` (v2.1.257); Gateway: `CLAUDE_GATEWAY_PROXY_IS_EGRESS_BOUNDARY` (v2.1.277). Confirmed on settings-reference/env-vars pages | COMPLETE (all 7 added) — NEW |
| 17 | HIGH | New Env Vars | Add accessibility: `CLAUDE_AX_PREPARK_MS`, `CLAUDE_AX_STARTUP_QUIET_MS`; tracing: `ENABLE_BETA_TRACING_DETAILED`, `BETA_TRACING_ENDPOINT`; MCP: `MCP_SDK_GENERATION`, `MCP_PROTOCOL_NEGOTIATION` (v2.1.274); display: `CLAUDE_CODE_TMUX_TRUECOLOR` (already present — SKIP) | COMPLETE (accessibility/tracing/MCP vars added) — NEW |
| 18 | MED | Deprecated/Removed | Mark `taskOutputMaxChars` removed v2.1.277 (also remove active description for `TASK_MAX_OUTPUT_LENGTH`); mark `keybindingFlavor` as no-op since v2.1.261; note `disableArtifact` deprecated (use `enableArtifact: false` instead). Confirmed in changelog | COMPLETE (removals and deprecations marked) — NEW |
| 19 | MED | Stale Annotations | Remove/update "not in official docs" annotations from: `CLAUDE_CODE_SESSION_ID`, `DISABLE_UPDATES`, `CLAUDE_CODE_DISABLE_BG_SHELL_PRESSURE_REAP`, `CLAUDE_CODE_DISABLE_AGENT_VIEW`, `ENABLE_PROMPT_CACHING_1H`, `FORCE_PROMPT_CACHING_5M`, `ENABLE_PROMPT_CACHING_1H_BEDROCK` (deprecated), `CLAUDE_EFFORT`, `CLAUDE_CODE_MAX_TURNS`, `CLAUDE_CODE_PROCESS_WRAPPER`, `CLAUDE_CODE_MAX_WEB_SEARCHES_PER_SESSION`, `CLAUDE_CODE_MAX_SUBAGENTS_PER_SESSION`, `MCP_CONNECTION_NONBLOCKING`, `DISABLE_AUTO_COMPACT`, and 8 others now on official env-vars page | COMPLETE (annotations updated) — NEW |
| 20 | MED | Wrong Description | Fix `USE_BUILTIN_RIPGREP` description — set to `0` to use system rg (currently says "Set to 1 to use bundled"). Fix `CLAUDE_CODE_ENABLE_AUTO_MODE` — now a no-op on every provider. Fix `sandbox.ignoreViolations` — stale "not on official settings page" note; key is now official, type is command-substring-to-violation-array map. Confirmed on official pages | COMPLETE (descriptions corrected) — NEW |
| 21 | MED | Settings Hierarchy | Add "Exceptions to managed settings precedence" subsection: 9 keys where lower-level settings can override managed (`disableClaudeAiConnectors`, `enableArtifact`, `isolatePeerMachines`, `remoteControlAtStartup`, `crossSessionInbound`, `useAutoModeDuringPlan`, `syncClaudeAiSkills`, `syncClaudeAiPlugins`, `maxEffortLevel`). Confirmed on official settings page | COMPLETE (exceptions subsection added) — NEW |
| 22 | MED | Sources | Update settings-reference description from "~180 keys" to "231+ keys with Scope/Type/Default columns". Add [Managed Settings](https://code.claude.com/docs/en/managed-settings) as a source | COMPLETE (updated) — NEW |
| 23 | LOW | New Env Vars | Add `CLAUDE_CODE_BASH_EDIT_DIFF`, `CLAUDE_CODE_USER_DIALOG_TIMEOUT_MS`, `CLAUDE_CODE_SEND_FEEDBACK`, `CLAUDE_CODE_PROMPT_CACHE_TTL`, `CLAUDE_CODE_SUBAGENT_PROMPT_CACHE_TTL`, `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS`. Confirmed on official env-vars page | COMPLETE (all 6 added) — NEW |
| 24 | LOW | Stale Annotation | Remove stale "in JSON schema, not on official settings page" annotation from `sandbox.ignoreViolations` (now official, see item 20) | COMPLETE (annotation removed) — NEW |
