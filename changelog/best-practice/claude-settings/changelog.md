...
## [2026-09-10 10:42 AM PKT] Claude Code v2.1.267

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Missing Setting | Add `permissions.blockReadsOutsideWorkingDirectories` (boolean) to Permission Keys — one-time prompt before first Read outside working directory (v2.1.257). Confirmed in official CHANGELOG.md | ✅ COMPLETE (added to Permission Keys table) — NEW |
| 2 | HIGH | Missing Setting | Add `managedMcpServers` (array, managed-only) to MCP Settings table — org-provided HTTP/SSE servers auto-loaded for all users (v2.1.259). Confirmed in official CHANGELOG.md | ✅ COMPLETE (added to MCP Settings table) — NEW |
| 3 | HIGH | Missing Setting | Add `maxEffortLevel` (string or object) to Model Configuration section — caps effort level across all providers with optional per-model granularity (v2.1.267). Confirmed in official CHANGELOG.md | ✅ COMPLETE (added after `modelSettings` row) — NEW |
| 4 | HIGH | Missing Setting | Add `bashOutputMaxChars` (number, default 8000) to General Settings — raise inline command output limit up to 128K chars before saving to file (v2.1.261). Confirmed in official CHANGELOG.md | ✅ COMPLETE (added near `autoCompactWindow`) — NEW |
| 5 | HIGH | Missing Setting | Add `taskOutputMaxChars` (number, default 8000) to General Settings — raise inline background-task output limit up to 128K chars (v2.1.261). Confirmed in official CHANGELOG.md | ✅ COMPLETE (added after `bashOutputMaxChars`) — NEW |
| 6 | HIGH | Changed Description | Update `fable` model alias — as of v2.1.257, default Fable model is Fable 5.1 (`claude-fable-5-1`): 1M context, $10/$50 per Mtok, $0.25 cache reads. Confirmed in official CHANGELOG.md | ✅ COMPLETE (alias description updated) — NEW |
| 7 | MED | Missing Setting | Add `timeFormat` (string) to Display Settings — `"12h"`, `"24h"`, `"24h-utc"`, or strftime pattern (v2.1.257). Confirmed in official CHANGELOG.md | ✅ COMPLETE (added to Display Settings table) — NEW |
| 8 | MED | Missing Setting | Add `timeZone` (string) to Display Settings — IANA timezone for timestamp display (v2.1.257). Confirmed in official CHANGELOG.md | ✅ COMPLETE (added to Display Settings table) — NEW |
| 9 | MED | Changed Behavior | Update `keybindingFlavor` description — has no effect as of v2.1.261; Bash readline-style word editing is now default. Confirmed in official CHANGELOG.md | ✅ COMPLETE (description updated to note no effect) — NEW |
| 10 | MED | Missing Env Var | Add `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` — forces model on all subagent spawns regardless of agent-definition overrides (v2.1.257). Confirmed in official CHANGELOG.md | ✅ COMPLETE (added after `CLAUDE_CODE_SUBAGENT_MODEL`) — NEW |
| 11 | MED | Missing Env Var | Add `SELF_HOSTED_RUNNER_CLIENT_LABEL` — override runner registration label; alternative to `--client-label` flag (v2.1.248). Confirmed in official CHANGELOG.md | ✅ COMPLETE (added after `CCR_FORCE_BUNDLE`) — NEW |
| 12 | HIGH | Scope Inversion | Fix `askUserQuestionTimeout` scope: was "Only honored from project and local settings — managed and user settings values are ignored", corrected to "Only honored from user and managed settings — project and local settings values are ignored" (verified from official settings page) | ✅ COMPLETE (scope description corrected) |
| 13 | HIGH | Removed Key | Mark `permissionExplainerEnabled` as "Removed in v2.1.257" — key no longer read or honored; default was `true`, showed AI-generated permission explanation (verified from official changelog) | ✅ COMPLETE (marked as removed in Global Config Settings table) |
| 14 | HIGH | Changed Behavior | Add v2.1.257 note to `permissions.defaultMode`: `"bypassPermissions"` value is now ignored when set in project or local settings (same restriction as `"auto"`) | ✅ COMPLETE (note added to defaultMode description) |
| 15 | MED | Scope Fix | Remove "(Managed only)" from `enforceAvailableModels` — official settings page shows this key is available at any scope (verified from official settings page) | ✅ COMPLETE (scope qualifier removed) |
| 16 | MED | Scope Fix | Remove "(Managed only)" from `forceLoginMethod` — official settings page shows this key is available at any scope (verified from official settings page) | ✅ COMPLETE (scope qualifier removed) |
| 17 | MED | Scope Fix | Fix `allowedMcpServers` and `deniedMcpServers` Scope column from "Managed only" to "Any" — official settings page confirms these keys work at any scope, not only in managed settings (verified from official settings page) | ✅ COMPLETE (Scope column corrected for both rows) |
| 18 | LOW | Stale Annotation | Remove "*(in JSON schema, not on official settings page)*" annotation from `sandbox.ignoreViolations` — key now appears in official settings documentation index (verified from official settings page) | ✅ COMPLETE (annotation removed) |
