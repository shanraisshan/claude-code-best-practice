...
## [2026-09-09 10:45 AM PKT] Claude Code v2.1.266

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update version badge v2.1.252 → v2.1.266; update header version references in body text | ✅ COMPLETE (badge and header updated) — NEW |
| 2 | HIGH | New Setting | Add `timeFormat` (string, Display Settings, v2.1.257) — time display format: 12-hour, 24-hour, 24-hour UTC, or strftime pattern. Confirmed by both research agents via changelog | ✅ COMPLETE (added to Display Settings table) — NEW |
| 3 | HIGH | New Setting | Add `timeZone` (string, Display Settings, v2.1.257) — IANA timezone string for timestamps. Confirmed by both research agents | ✅ COMPLETE (added to Display Settings table) — NEW |
| 4 | HIGH | New Setting | Add `permissions.blockReadsOutsideWorkingDirectories` (boolean, Permission Keys, v2.1.257) — one-time prompt before first file read outside working dir. Confirmed by both research agents via changelog | ✅ COMPLETE (added to Permission Keys table) — NEW |
| 5 | HIGH | New Setting | Add `managedMcpServers` (array, MCP Settings, Managed only, v2.1.259) — org-provided HTTP/SSE MCP servers; command-typed entries skipped. Confirmed by both research agents via changelog | ✅ COMPLETE (added to MCP Settings table) — NEW |
| 6 | HIGH | New Setting | Add `bashOutputMaxChars` (number, General Settings, v2.1.261) — raise inline bash output limit up to 128K chars. Confirmed by both research agents via changelog | ✅ COMPLETE (added to General Settings table) — NEW |
| 7 | HIGH | New Setting | Add `taskOutputMaxChars` (number, General Settings, v2.1.261) — raise background-task output limit up to 128K chars. Confirmed by both research agents via changelog | ✅ COMPLETE (added to General Settings table) — NEW |
| 8 | HIGH | Deprecated Setting | Annotate `keybindingFlavor` (Display Settings) as "Deprecated and has no effect (v2.1.261)" — word-editing keys now always match Bash/GNU readline unconditionally. Confirmed by Agent 1 and changelog | ✅ COMPLETE (description updated with deprecation notice) — NEW |
| 9 | HIGH | Wrong Description | Update `permissions.defaultMode` — v2.1.257 added: `"bypassPermissions"` now also ignored in project/local settings (previously only `"auto"` was restricted). Confirmed by Agent 1 and v2.1.257 changelog | ✅ COMPLETE (description updated) — NEW |
| 10 | HIGH | Model Alias | Update `"fable"` alias row — Fable 5.1 (`claude-fable-5-1`) is now the default Fable model since v2.1.257; note gateway sessions may still resolve to Fable 5. Confirmed by Agent 1 and v2.1.257 changelog | ✅ COMPLETE (alias row updated) — NEW |
| 11 | HIGH | New Env Var | Add `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` (v2.1.257) — forces specific model on every subagent, overriding agent definitions and per-spawn flags. Confirmed by Agent 1 and v2.1.257 changelog | ✅ COMPLETE (added to env vars table) — NEW |
| 12 | MED | Missing Settings | Add 8+ older keys confirmed by Agent 1 on official settings-reference page but absent from report: `isolatePeerMachines`, `syncClaudeAiSkills`, `skipAutoPermissionPrompt`, `sandbox.ripgrep`, `sshHostAllowlist`, `enableWorkflows`, `promptSuggestionEnabled`, `managedSourcesBehavior` | ✋ ON HOLD (need official settings-reference page confirmation for each key on next run) — NEW |
| 13 | MED | Missing Sub-Fields | Add `spinnerTipsOverride.tipsFile` and `spinnerTipsOverride.label` sub-fields (v2.1.247). Add `prompt_cache` status line field (v2.1.260) and `rate_limits.spend_limit` (v2.1.251). Confirmed by Agent 1 | ✋ ON HOLD (pending official source confirmation) — NEW |
| 14 | LOW | Missing Env Vars | Add 6 missing env vars: `ANTHROPIC_FEDERATION_RULE_ID`, `ANTHROPIC_ORGANIZATION_ID`, `ANTHROPIC_PROFILE`, `BETA_TRACING_ENDPOINT`, `CLAUDE_AX_PREPARK_MS`, `CLAUDE_AX_STARTUP_QUIET_MS`. Confirmed by Agent 1 via env-vars page | ✋ ON HOLD (need env-vars page confirmation for each) — NEW |
| 15 | LOW | Hierarchy Additions | Add "Exceptions to managed settings precedence" table (7 keys); document v2.1.211 repository-root rule for `.claude/settings.local.json`; note `managedSourcesBehavior` qualifies the "sources do not merge" claim at line 51. Confirmed by Agent 1 | ✋ ON HOLD (scope too broad; defer to dedicated run) — NEW |
| 16 | LOW | RECURRING | `OTEL_LOG_TOOL_DETAILS` — env var present in changelog (v2.1.85) but not on official env-vars page. Annotation already in report | ✋ ON HOLD (in v2.1.85 changelog, not yet on official env-vars page) — RECURRING (first seen: 2026-04-14) |
