...
## [2026-08-12 10:45 AM PKT] Claude Code v2.1.228

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Wrong Description | Rewrote `Write`/`NotebookEdit`/`Glob` permission rows (lines 315–316, 326): the report claimed these path forms were "valid in deny and ask rules only" — official docs confirm path forms are **never consulted in any list** (allow, deny, or ask). Added `MultiEdit` to the never-consulted list, the `--allowedTools` Glob exception, and the bare-tool-name-still-works clarification. Confirmed on official permissions docs page | COMPLETE (permission rows rewritten, v2.1.210 note corrected) — RECURRING (first seen: 2026-08-12) |
| 2 | HIGH | New Rule | Added v2.1.228 `Read`-deny behavior: a `Read` deny rule now also blocks `Edit` and `Write` on the same path including creating a new file there; `NotebookEdit` not covered; add `Edit` deny for paths no tool may change. Confirmed on official permissions docs page | COMPLETE (v2.1.228 note added to permission syntax section) — NEW |
| 3 | HIGH | New Settings | Added `crossSessionInbound` (string, v2.1.224) and `dialogExpiry` (string, default `"5m"`, v2.1.224) to General Settings table. Both confirmed on official settings docs page. Resolved: were ON HOLD since v2.1.224 run (item 21) | COMPLETE (both keys added with scope rules and accepted values) — RESOLVED (was ON HOLD since 2026-08-07) |
| 4 | HIGH | New Settings | Added missing `sandbox.credentials` v2.1.224 keys: `awsPairs` (SigV4 credential grouping), `files[].decode`/`maskClaims` (JWT-aware masking), and full `envVars[]` field set (`extract`, `decode`, `maskClaims`, `onExtractNoMatch`). Also added mask→deny silent fallback conditions (directory path, glob, >8 MiB, non-UTF-8). All confirmed on official sandboxing docs page. Resolved: JWT masking keys were ON HOLD since v2.1.224 run (item 22) | COMPLETE (new keys added to Sandbox table) — RESOLVED (was ON HOLD since 2026-08-07) |
| 5 | MED | Wrong Version | Fixed `sandbox.credentials.files[]` extract-family version: `extract`, `onExtractNoMatch`, `maskDuplicates` were dated v2.1.221 — official docs confirm v2.1.224. Confirmed on official sandboxing docs page | COMPLETE (version corrected on 3 rows) — NEW |
| 6 | MED | Wrong Description | Corrected `maskDuplicates` scope: report said "other files and env vars" — actual behavior is "outside matched spans within the same file or value". Added warning to reserve for long high-entropy secrets. Confirmed on official sandboxing docs page | COMPLETE (description corrected) — NEW |
| 7 | MED | Incomplete Description | Added `onExtractNoMatch` deny→error escalation: on macOS with filesystem isolation active, or when `filesystem.allowRead` re-opens the file path, `deny` is treated as `error`. Confirmed on official sandboxing docs page | COMPLETE (escalation rule added to onExtractNoMatch row) — NEW |
| 8 | MED | Incomplete Description | Added `filesystem.disabled` managed-pinning rules: when managed settings configure `sandbox.filesystem` or list any files entry with `mode: "deny"`, only managed settings can modify those keys; `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB` overrides all sources including managed. Confirmed on official sandboxing docs page | COMPLETE (pinning rules added to sandbox section) — NEW |
| 9 | LOW | New Env Var | Added `CLAUDE_CODE_USER_DIALOG_TIMEOUT_MS` (runtime override for `dialogExpiry`, v2.1.224) to environment variables table near `CLAUDE_AFK_TIMEOUT_MS`. Confirmed on official settings docs page | COMPLETE (added to env table) — NEW |
| 10 | LOW | Version Metadata | Updated version badge v2.1.224 → v2.1.228 and env var count 311 → 335 (actual row count). Confirmed via badge and table audit | COMPLETE (badge and header count updated) — NEW |
