...
## [2026-08-19 10:44 AM PKT] Claude Code v2.1.235

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Missing Setting | Add `crossSessionInbound` (string, "accept"/"hold"/"refuse", default "refuse") to General Settings — controls how inbound cross-session messages are handled; scope: managed → --settings → user, project/local apply only if stricter. Confirmed on official settings docs page (v2.1.224; /config label added v2.1.232). Was ON HOLD since v2.1.224 run | COMPLETE (added to General Settings table; previous ON HOLD resolved) |
| 2 | HIGH | Missing Setting | Add `dialogExpiry` (string, "60s"/"5m"/"10m"/"never", default "5m") to General Settings — deadline for dialogs forwarded to Remote Control/SDK host and held cross-session message approvals; user/managed/--settings only. Confirmed on official settings docs page (v2.1.224). Was ON HOLD since v2.1.224 run (distinct key from `askUserQuestionTimeout`) | COMPLETE (added to General Settings table; previous ON HOLD resolved) |
| 3 | HIGH | Wrong Description | Fix `advisorModel`: report said Fable was unavailable (v2.1.210+ caveat). Reverted in v2.1.232 — Fable 5 is available again for organizations with access. Updated description to reflect current state | COMPLETE (advisorModel description updated) |
| 4 | HIGH | Missing Setting | Add `spellcheck` (boolean, default false) to Display & UX — show spell-check underlines for misspelled words via aspell/hunspell/ispell. Confirmed in v2.1.235 changelog | COMPLETE (added to Display & UX table) |
| 5 | HIGH | Missing Setting | Add `disableCommandPluginSources` (boolean, managed only, v2.1.229) to Managed-only keys table — blocks `command`-type plugin sources. Confirmed on official settings docs page | COMPLETE (added to Managed-only table) |
| 6 | HIGH | Deprecated Setting | Mark `teammateDefaultModel` as REMOVED in v2.1.234 — teammates now inherit lead's model. Confirmed in v2.1.234 changelog | COMPLETE (row updated with REMOVED notice) |
| 7 | HIGH | Missing Feature | Add `command` marketplace source type (v2.1.229) to marketplace source list. Confirmed in v2.1.229 changelog | COMPLETE (command source type added to marketplace list) |
| 8 | HIGH | Wrong Description | Fix `autoMemoryDirectory`: report said "not accepted in project settings". Official docs: honored from project/local settings under workspace trust rule (same as hooks). Confirmed on official settings docs page | COMPLETE (description corrected) |
| 9 | HIGH | Wrong Count | Correct env var count: report header claimed "311" but env var table had 335 rows. Updated header to "343" after adding 8 new vars | COMPLETE (header updated) |
| 10 | HIGH | Missing Env Vars | Add 8 missing env vars: `CLAUDE_CODE_PROJECT_DIR_NAME` (v2.1.234), `CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS` (v2.1.233), `CLAUDE_CODE_USER_DIALOG_TIMEOUT_MS` (v2.1.224), `ANTHROPIC_FEDERATION_RULE_ID`, `ANTHROPIC_ORGANIZATION_ID`, `ANTHROPIC_PROFILE` (v2.1.234), `CLAUDE_AX_PREPARK_MS`, `CLAUDE_AX_STARTUP_QUIET_MS`. Confirmed on official env-vars page | COMPLETE (all 8 added) |
| 11 | HIGH | Missing Env Var | Add `CLAUDE_CODE_GOAL_CHECKIN_MINUTES` (v2.1.232) — interval in minutes for goal check-ins during long sessions; 0 disables. Confirmed in v2.1.232 changelog | COMPLETE (added to env vars) |
| 12 | HIGH | Missing Env Var | Add `CLAUDE_CODE_TOOL_MEMORY_LIMIT` (v2.1.233) — memory cgroup limit for Bash tool subprocess invocations. Confirmed in v2.1.233 changelog | COMPLETE (added to env vars) |
| 13 | HIGH | Missing Env Var | Add `CLAUDE_CODE_ENABLE_TODO_TOOLS` (v2.1.233) — re-enable TodoWrite/TodoRead on newer models that default to Task tools. Confirmed in v2.1.233 changelog | COMPLETE (added to env vars) |
| 14 | MED | Incomplete Description | Fix `disableAllHooks`: report said "(also disables any custom status line)" — official docs add "and custom file suggestion command". Confirmed on official settings docs page | COMPLETE (third effect added) |
| 15 | MED | Wrong Description | Fix `CCR_FORCE_BUNDLE`: report referenced `claude --remote`; `--remote` is deprecated alias for `--cloud`. Confirmed in CLI reference | COMPLETE (updated to --cloud with deprecation note) |
| 16 | MED | Stale Annotation | Remove "not yet on official env-vars page" annotation from `ANTHROPIC_BEDROCK_REGION_PREFIX` — now listed on official env-vars page. Also removed same annotation from `CLAUDE_CODE_MAX_WEB_SEARCHES_PER_SESSION`, `CLAUDE_CODE_MAX_SUBAGENTS_PER_SESSION`, and `CLAUDE_CODE_MCP_AUTO_BACKGROUND_MS` | COMPLETE (annotations removed) |
| 17 | MED | Missing Documentation | Add IPv6 address literal note to `sandbox.network.allowedDomains` and `deniedDomains`: IPv6 must use bracket notation `"[::1]"` (all ports) or `"[::1]:443"` (specific port). Requires v2.1.229+. Confirmed in v2.1.229 changelog | COMPLETE (IPv6 note added to both domain list rows) |
| 18 | LOW | Unverified Keys | v2.1.224 JWT masking options (`decode`, `maskClaims`, `awsPairs`/`sigv4`) for `sandbox.credentials.files[]` — not yet on official settings page, changelog-only, confidence 0.70 | ON HOLD (RECURRING since v2.1.224 run — awaiting official docs page update) |
| 19 | LOW | Missing Setting | `sandbox.ripgrep` — referenced in v2.1.232 changelog ("honored only from user/managed/--settings; project can't override") but exact key shape unverified (sandboxing docs treat ripgrep only as a dependency). Confidence 0.80 | ON HOLD (exact shape unconfirmed — awaiting official sandboxing docs update) |
