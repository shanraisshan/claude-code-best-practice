...
## [2026-08-29 10:48 AM PKT] Claude Code v2.1.251

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update badge v2.1.224 → v2.1.251; update header counts to "130+ settings / 340+ env vars" (actual env-var table holds 336 rows vs reported 311) | ✅ COMPLETE (badge and header updated) — NEW |
| 2 | HIGH | Missing Setting | Add `crossSessionInbound` (string, any scope) and `dialogExpiry` (string, user/managed) to General Settings — were ON HOLD in v2.1.224 run, now confirmed on official settings-reference page | ✅ COMPLETE (both added to General Settings table) — RESOLVED (ON HOLD since 2026-08-07) |
| 3 | HIGH | Missing Settings | Add sandbox AWS re-signing keys: `sandbox.credentials.awsPairs` (object, user/managed — link custom AWS key variables for re-signing) and `sandbox.credentials.sigv4` (string, user/managed — streaming/presigned/SigV4A behavior); were ON HOLD in v2.1.224 run as "JWT masking options", now confirmed on official settings-reference page | ✅ COMPLETE (both added to Sandbox table) — RESOLVED (ON HOLD since 2026-08-07) |
| 4 | HIGH | Missing Setting | Add `modelPicker` (any, user or managed only — curated `/model` picker with ordered model list; never merges — whole value from highest of managed/`--settings`/user). Also add `modelPicker` to the non-merge list alongside `fallbackModel` and `availableModels` | ✅ COMPLETE (added to General Settings table and non-merge list) — NEW |
| 5 | HIGH | Missing Settings | Add `keybindingFlavor` (string, any — `"readline"` or `"classic"`) and `spellcheck` (boolean, user/managed — real-time spell-checking) | ✅ COMPLETE (added to General Settings table) — NEW |
| 6 | HIGH | Missing Settings | Add `promptCacheTtl` (string, any — prompt cache lifetime e.g. `"5m"`/`"1h"`) and `subagentPromptCacheTtl` (string, any — cache lifetime for subagent requests) | ✅ COMPLETE (added to General Settings table) — NEW |
| 7 | HIGH | Missing Hierarchy | Add "Exceptions to managed settings precedence" table (7 keys) under Settings Hierarchy: `disableClaudeAiConnectors`, `enableArtifact`/`disableArtifact`, `isolatePeerMachines`, `remoteControlAtStartup`, `crossSessionInbound`, `useAutoModeDuringPlan`, `syncClaudeAiSkills` | ✅ COMPLETE (table added to Settings Hierarchy section) — NEW |
| 8 | HIGH | Wrong Scope | Fix `allowedMcpServers` and `deniedMcpServers` scope from "Managed only" → "Any file" per official settings-reference page | ✅ COMPLETE (scope corrected) — NEW |
| 9 | HIGH | Deprecated Setting | Mark `teammateDefaultModel` as REMOVED in v2.1.234 (teammates now follow the lead's model) | ✅ COMPLETE (removal note added) — NEW |
| 10 | HIGH | Changed Behavior | Fix `CLAUDE_CODE_SUBAGENT_MODEL`: changed in v2.1.251 from "override everything" to "set DEFAULT subagent model" — agent definition `model:` and explicit per-spawn model take precedence over this env var | ✅ COMPLETE (behavior clarified in description) — NEW |
| 11 | HIGH | Missing Env Vars | Add 6 missing env vars confirmed on official env-vars page: `ANTHROPIC_DEFAULT_MODEL` (v2.1.237 — model new sessions start with, applies only when no file sets `model`), `CLAUDE_CODE_PROJECT_DIR_NAME` (override project directory name), `CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS` (WebFetch cache TTL), `CLAUDE_CODE_ENABLE_TODO_TOOLS` (re-enable todo tools, set to `1`), `CLAUDE_CODE_RESTRICTED` (v2.1.248 — removes execution tools, refuses bypassPermissions, ignores user/project/local settings), `SELF_HOSTED_RUNNER_CLIENT_LABEL` (override self-hosted runner label) | ✅ COMPLETE (all 6 added to env vars table) — NEW |
| 12 | HIGH | Missing Source | Add `https://code.claude.com/docs/en/settings-reference` as source #1 — the canonical "all settings with scope/type/default" index; re-label existing `/docs/en/settings` source as "Settings scopes & precedence" | ✅ COMPLETE (source added, existing source re-labeled) — NEW |
| 13 | HIGH | Permission Syntax | Add startup warning annotation to `Bash(git * main)` examples at lines 311 and 346 — v2.1.246 added a startup warning for this pattern since a wildcard before the subcommand also matches options inserted before it | ✅ COMPLETE (warning annotation added) — NEW |
| 14 | MED | Missing Settings | Add `autoContinueAtUsageLimit` (boolean, user/managed — auto-continue after usage limit resets; repository file can switch off), `syncClaudeAiSkills` (boolean, any — stop downloading claude.ai-synced skills), `enableWorkflows` (boolean, any — enable/disable workflows against plan default, distinct from `disableWorkflows`) | ✅ COMPLETE (all 3 added to General Settings table) — NEW |
| 15 | MED | Missing Setting | Add `feedbackDrafts` (boolean, user/managed — whether Claude queues feedback drafts for review via `SendFeedback` tool, v2.1.247) | ✅ COMPLETE (added to General Settings table) — NEW |
| 16 | MED | Missing Setting | Add `disableCommandPluginSources` (boolean, Managed only — blocks plugins that install by running a marketplace-declared command) | ✅ COMPLETE (added to Plugin Settings table) — NEW |
| 17 | MED | Wrong Type/Scope | Fix `strictPluginOnlyCustomization`: official index documents it as an object with 4 boolean sub-keys (`.agents`, `.hooks`, `.mcp`, `.skills`) in addition to accepting `true`/array form; update description | ✅ COMPLETE (sub-keys documented) — NEW |
| 18 | MED | Changed Behavior | Update `spinnerTipsOverride` description: v2.1.247 added entry-object form (`{id, text, cooldownSessions, priority}`), `tipsFile`, and `label` sub-fields for org-rotated tips. Update report's example at Display Settings section | ✅ COMPLETE (description and example updated) — NEW |
| 19 | MED | Missing Fields | Add two status line input fields: `rate_limits.spend_limit` (spend limit bar for Claude apps gateway developers) and `prompt_cache` (per-session cache hit ratio, misses, tokens re-cached). Both added in v2.1.251 | ✅ COMPLETE (both fields added to Status Line Input Fields table) — NEW |
| 20 | MED | Stale Count | Fix hook event count in hooks redirect blurb: "all 26 hook events" → "all 28 hook events" (v2.1.251 added `PreModelSwitch` and `PostModelSwitch`) | ✅ COMPLETE (count updated) — NEW |
| 21 | MED | Changed Behavior | Add note to `CLAUDE_CODE_TMPDIR` and `CLAUDE_CONFIG_DIR` env vars: as of v2.1.251, project-level `.claude/settings.json` `env` can no longer set these variables (or `TMPDIR`/`TMP`/`TEMP`) | ✅ COMPLETE (scope caveat added) — NEW |
| 22 | LOW | Type Mismatches | 17 type mismatches found between report and official settings-reference (e.g., `effortLevel`: string vs number; `statusLine`: object vs string; `disableAutoMode`: string vs boolean; etc.). Confidence 0.75 — official table appears to simplify complex types. Needs manual reconciliation against live behavior | ✋ ON HOLD (confidence 0.75; official docs may be simplified; needs manual verification) — NEW |
| 23 | LOW | Scope Discrepancy | `askUserQuestionTimeout` scope is disputed: report says "only honored from project and local settings" (added in v2.1.220 run as confirmed); drift agent says official scope is "User or managed". Conflicting sources — requires manual verification of current official settings-reference | ✋ ON HOLD (conflicting sources; previous run confirmed "project/local only" from official docs; needs manual recheck) — RECURRING (first seen: 2026-08-02) |
| 24 | LOW | Missing Settings | 13 lower-priority missing settings not applied this run: `modelPricing` (Managed), `isolatePeerMachines`, `subagentStatusLine`, `promptSuggestionEnabled`, `skipAutoPermissionPrompt`, `sandbox.ripgrep`, `managedSourcesBehavior` (Managed), `desktopSessionCleanupPeriodDays`, `disableDesktopLocalSessions` (Managed), `sshHostAllowlist` (Managed), local-settings repo-root relocation (v2.1.211), cloud-sessions settings section, effort level per-model behavior (v2.1.251) | ✋ ON HOLD (lower priority or complex description changes; schedule for next run) — NEW |
