...
## [2026-09-25 10:50 AM PKT] Claude Code v2.1.282

| # | Priority | Type | Action | Status |
|---|----------|------|--------|--------|
| 1 | HIGH | Version Metadata | Update version badge v2.1.252 → v2.1.282; update header counts from "140+ settings / 315+ env vars" to "230+ settings / 370+ env vars". Confirmed via official settings-reference page (231 keys) | ✅ COMPLETE (badge and header updated) — NEW |
| 2 | HIGH | Critical Bug Fix | Fix `askUserQuestionTimeout` scope inversion (line 134) — report says "project/local only"; official settings-reference says "User or managed". Completely inverted. Confirmed in settings-reference meta | ✅ COMPLETE (scope corrected to "User or managed") — NEW |
| 3 | HIGH | Critical Bug Fix | Fix `feedbackDrafts` type/default/scope — report says boolean/true; official says string "notify"/"quiet"/"off", default "notify", User or managed only. Type is wrong, default is wrong, scope is wrong | ✅ COMPLETE (type, default, and scope all corrected) — NEW |
| 4 | HIGH | Wrong Description | Fix `dialogExpiry` — report says free duration format; official says enum "60s"/"5m"/"10m"/"never", default "5m", User or managed only | ✅ COMPLETE (type corrected to enum, default "5m" added, scope documented) — NEW |
| 5 | HIGH | Wrong Default | Fix `autoContinueAtUsageLimit` default — report shows `false`; official shows `true`. Also add scope restriction (user/managed/--settings) | ✅ COMPLETE (default corrected to true, scope documented) — NEW |
| 6 | HIGH | Missing Settings | Add `attribution: false` support to Attribution section — v2.1.281 added boolean `false` to disable all attribution. Confirmed in settings-reference | ✅ COMPLETE (`attribution` object|boolean row added with v2.1.281 note) — NEW |
| 7 | HIGH | Wrong Description | Fix `permissions.defaultMode` — `bypassPermissions` now also ignored in project/local settings (v2.1.257); report only mentions `auto` restriction | ✅ COMPLETE (bypassPermissions restriction added for v2.1.257) — NEW |
| 8 | HIGH | Missing Settings | Add `permissions.blockReadsOutsideWorkingDirectories` (boolean, v2.1.257, restricts reads to workspace dirs). Confirmed in official settings-reference | ✅ COMPLETE (key added to Permissions table) — NEW |
| 9 | HIGH | Wrong Type | Fix `strictKnownMarketplaces` — report says boolean; official says array of marketplace source objects. Confirmed in settings-reference | ✅ COMPLETE (type corrected to array of objects) — NEW |
| 10 | HIGH | Missing Settings | Add `syncClaudeAiSkills` and `syncClaudeAiPlugins` to Plugin Settings (v2.1.275). Confirmed in settings-reference | ✅ COMPLETE (both keys added to Plugin Settings table) — NEW |
| 11 | HIGH | Wrong Description | Fix `opus` model alias — says "Claude Opus 5"; should be "Claude Opus 5.5" (v2.1.280). Confirmed in settings-reference | ✅ COMPLETE (description updated to Opus 5.5) — NEW |
| 12 | HIGH | Wrong Description | Fix `fable` model alias — says "Fable 5"; should be "Fable 5.1" (v2.1.257). Confirmed in settings-reference | ✅ COMPLETE (description updated to Fable 5.1) — NEW |
| 13 | HIGH | Missing Settings | Add `best` model alias (maps to best available model). Confirmed in settings-reference | ✅ COMPLETE (`best` alias added to Model Aliases table) — NEW |
| 14 | HIGH | Wrong Description | Fix effort level defaults — Opus 5.5 defaults to medium, not high. Confirmed in settings-reference | ✅ COMPLETE (effort level note updated for Opus 5.5 → medium default) — NEW |
| 15 | HIGH | Missing Settings | Add `maxEffortLevel` to Model Overrides table (v2.1.267, caps session effort level). Confirmed in settings-reference | ✅ COMPLETE (key added to Model Overrides table) — NEW |
| 16 | HIGH | Hook Count | Update hooks redirect count from "28 hook events" to "33 hook events" — 5 new hook events added between v2.1.252 and v2.1.282. Confirmed in changelog | ✅ COMPLETE (count updated to 33) — NEW |
| 17 | HIGH | Removed Key | Mark `permissionExplainerEnabled` as removed in v2.1.257 — report still shows it as active. Confirmed via official docs | ✅ COMPLETE (marked as Removed in v2.1.257) — NEW |
| 18 | HIGH | Wrong Type | Fix `diffTool` — report says "external diff tool command"; official says enum "auto"/"terminal". Confirmed in settings-reference | ✅ COMPLETE (corrected to enum "auto"/"terminal") — NEW |
| 19 | HIGH | Missing Settings | Add `timeFormat` and `timeZone` to Display Settings (v2.1.257). Confirmed in settings-reference | ✅ COMPLETE (both keys added to Display Settings table) — NEW |
| 20 | HIGH | Missing Settings | Add `terminalTitleFromRename`, `promptSuggestionEnabled`, `subagentStatusLine` to Display Settings. Confirmed in settings-reference | ✅ COMPLETE (all three keys added to Display Settings table) — NEW |
| 21 | HIGH | Missing Env Vars | Add 15 missing env vars confirmed in official env-vars page: `CLAUDE_CODE_AUTO_MODE_SERVER`, `CLAUDE_CODE_BASH_EDIT_DIFF`, `CLAUDE_CODE_PROMPT_CACHE_TTL`, `CLAUDE_CODE_SUBAGENT_PROMPT_CACHE_TTL`, `CLAUDE_CODE_SUBAGENT_MODEL_FORCE`, `CLAUDE_CODE_MAX_MCP_DESCRIPTION_LENGTH`, `CLAUDE_CODE_MCP_STARTUP_WAIT_MS`, `CLAUDE_CODE_USER_DIALOG_TIMEOUT_MS`, `CLAUDE_CODE_SEND_FEEDBACK`, `CLAUDE_CODE_GATEWAY_HINT_HEADERS`, `ANTHROPIC_FEDERATION_RULE_ID`, `ANTHROPIC_ORGANIZATION_ID`, `ANTHROPIC_PROFILE`, `VERTEX_REGION_CLAUDE_5_5_OPUS`, `VERTEX_REGION_CLAUDE_FABLE_5_1` | ✅ COMPLETE (all 15 env vars added) — NEW |
| 22 | HIGH | Stale Annotations | Remove "not on official page" annotations from env vars confirmed on official settings-reference env-vars page | ✅ COMPLETE (5 annotations removed per Rule 8A: CLAUDE_CODE_MAX_TURNS, DISABLE_AUTO_COMPACT, CLAUDE_EFFORT, ENABLE_PROMPT_CACHING_1H_BEDROCK, sandbox.ignoreViolations) — NEW |
| 23 | MED | Wrong Description | Fix `advisorModel` stale note — "fable no longer attaches an advisor" is outdated; fable now works as advisor. Confirmed in settings-reference | ✅ COMPLETE (stale note removed; fable restored as valid advisor model) — NEW |
| 24 | MED | Wrong Scope | Fix `enforceAvailableModels` scope — marked "(Managed only)" but official says Any file. Confirmed in settings-reference | ✅ COMPLETE (Managed only label removed) — NEW |
| 25 | MED | Wrong Scope | Fix `allowedMcpServers`/`deniedMcpServers` scope — marked "(Managed only)" but official says Any file. Confirmed in settings-reference | ✅ COMPLETE (scope corrected to Any) — NEW |
| 26 | MED | Missing Settings | Add `managedMcpServers` to MCP Settings (v2.1.259, object, managed-only array of server configs). Confirmed in settings-reference | ✅ COMPLETE (key added to MCP Settings table) — NEW |
| 27 | MED | Wrong Default | Fix `model`, `language`, `alwaysThinkingEnabled`, `tui` defaults — all shown with explicit values but official says these are unset by default | ✅ COMPLETE (all four defaults corrected to unset "-") — NEW |
| 28 | MED | Deprecated Key | Mark `keybindingFlavor` as deprecated (v2.1.261, no longer has effect). Confirmed in settings-reference | ✅ COMPLETE (deprecation notice added with v2.1.261) — NEW |
| 29 | MED | Wrong Description | Fix `spellcheck` field name — report says `binary`; official says `checker`. Confirmed in settings-reference | ✅ COMPLETE (field name corrected to checker) — NEW |
| 30 | MED | Missing Settings | Add `copyOnSelect` to Global Config Settings. Confirmed in settings-reference | ✅ COMPLETE (key added to Global Config table) — NEW |
| 31 | MED | Wrong Description | Fix `TASK_MAX_OUTPUT_LENGTH` — no-op since v2.1.277; use `taskOutputMaxChars` setting instead | ✅ COMPLETE (marked no-op with migration note) — NEW |
| 32 | MED | Wrong Description | Fix `USE_BUILTIN_RIPGREP` description — inverted; bundled ripgrep is the DEFAULT; set to `0` to use system rg | ✅ COMPLETE (description corrected; bundled is default) — NEW |
| 33 | MED | Source Count | Update settings-reference source annotation from "~180 keys" → "231 keys" | ✅ COMPLETE (source count updated) — NEW |
