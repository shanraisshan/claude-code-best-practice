...
| 3E | Allow vs Deny Depth Asymmetry | Verify that the report documents the `dir/**` depth asymmetry for allow vs deny rules: `Edit`/`Read` **allow** rules using `dir/**` match only direct children of `dir` at cwd; **deny and ask** rules using `dir/**` match at any depth in the file tree. Use `**/dir/**` in allow rules to match at any depth | content-match | settings + permissions documentation pages | 2026-07-28 | v2.1.214 introduced this asymmetry; v2.1.220 run caught it missing from the report. No existing rule covered the behavioral difference between allow and deny path glob semantics |
