...
| 1J | Scope-in-Description | For settings keys where official docs describe scope restrictions that are NOT captured in a dedicated Scope column (particularly Display Settings, General Settings, and Sandbox tables), verify the scope restriction appears in the Description column of the report. Pay special attention to security-sensitive restrictions (e.g., "NOT honored from project/local settings" for `sandbox.filesystem.disabled`) and usability scope limits (e.g., "read from user settings and managed settings only" for `footerLinksRegexes`). Rule 1E only checks explicit Scope column cells in MCP/Plugin/Permission tables — it does not cover scope restrictions embedded in Description cells for other tables | content-match | settings documentation page | 2026-07-23 | v2.1.218 run caught `footerLinksRegexes` (v2.1.176) and `sandbox.filesystem.disabled` (v2.1.216) both having scope restrictions in official docs that were absent from the report's Description column. Both are in tables without a Scope column, so Rule 1E did not trigger. The `disabled` key's project-scope exclusion is a security-critical restriction that prevents projects from disabling filesystem isolation |
