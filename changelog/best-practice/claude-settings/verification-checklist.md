...
| 1J | Primary Settings Reference Page | Verify that the agent fetches `https://code.claude.com/docs/en/settings-reference` (the authoritative key index) and NOT only `/docs/en/settings` (now a how-to page with no key list). If `settings-reference` returns no keys but `settings` does, flag as a page-structure change and verify which is the current key index. Both must be in the Sources section of the report | content-match | settings-reference documentation page | 2026-08-26 | v2.1.246 run found that `/docs/en/settings` no longer contains the key reference — the authoritative list moved to `/docs/en/settings-reference`. Agents fetching only the old URL would see zero keys and report zero drift, producing false confidence. Rule 8A requires official sources, so fetching the wrong official URL passes 8A while silently missing all drift |
