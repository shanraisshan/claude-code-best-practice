...
| 11 | `design` | Draft UI mockups, screen flows, landing pages, or posters as artboards on one canvas, published as an artifact running a research preview of Claude Design's editor. Where saving is enabled artboards are edited on the canvas; otherwise view-and-export as PNG or PDF. Requires a session where artifacts are available. Available on the Anthropic API only (unavailable on Bedrock, Google Cloud Agent Platform, and Microsoft Foundry). Introduced v2.1.234 |
