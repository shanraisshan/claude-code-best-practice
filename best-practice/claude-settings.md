...
![Last Updated](https://img.shields.io/badge/Last_Updated-Sep%2010%2C%202026%2010%3A42%20AM%20PKT-white?style=flat&labelColor=555) ![Version](https://img.shields.io/badge/Claude_Code-v2.1.267-blue?style=flat&labelColor=555)<br>
[![Implemented](https://img.shields.io/badge/Implemented-2ea44f?style=flat)](../.claude/settings.json)

A comprehensive guide to all available configuration options in Claude Code's `settings.json` files. As of v2.1.267, Claude Code exposes **140+ settings** and **315+ environment variables** (use the `"env"` field in `settings.json` to avoid wrapper scripts).
...
| `bashOutputMaxChars` | number | `8000` | Maximum number of characters of Bash command output that Claude sees inline in its context before the rest is saved to a temporary file and the file path is provided instead. Raise this when tools produce large outputs you want Claude to process in full (maximum `131072` / 128K). Setting this higher increases context usage per command (v2.1.261) |
| `taskOutputMaxChars` | number | `8000` | Maximum number of characters of background-task (subagent) output shown inline before the remainder is saved to a file. Mirrors `bashOutputMaxChars` but applies to task tool output specifically. Maximum `131072` / 128K (v2.1.261) |
...
| `forceLoginMethod` | string | Restrict login to `"claudeai"`, `"console"`, or `"gateway"` accounts. Use `"gateway"` for organization-managed Claude gateway deployments |
...
| `managedMcpServers` | array | Managed only | HTTP/SSE MCP servers provided by the organization to all users. Each entry is an object with `name`, `url`, and optionally `description` and `authentication` fields. Command-based entries are skipped — only HTTP/SSE servers are supported. Servers defined here are auto-loaded without requiring `enableAllProjectMcpServers` or `enabledMcpjsonServers` opt-in (v2.1.259) |
...
| `"fable"` | Claude Fable 5 — long-horizon reasoning model. Anthropic API only (v2.1.170+). Fable 5 includes 1M context by default; the `[1m]` suffix is auto-stripped, so `fable[1m]` is redundant (v2.1.173). **As of v2.1.257, the default Fable model is Fable 5.1 (`claude-fable-5-1`)** — $10/$50 per Mtok with $0.25/Mtok cache reads; supports extended thinking, effort levels, and prompt caching. Use `ANTHROPIC_DEFAULT_FABLE_MODEL` to pin a specific Fable model ID |
...
| `maxEffortLevel` | string or object | - | Cap the maximum effort level that users can select. Accepts a string (`"high"`, `"xhigh"`, `"max"`, etc.) to apply the same cap to all models, or an object keyed by model alias/provider to set per-model caps. Users can still choose any effort level **at or below** the cap; this prevents runaway token spend on extremely high reasoning levels. Example: `"xhigh"` or `{"opus": "xhigh", "fable": "high"}` (v2.1.267) |
...
| `keybindingFlavor` | string | - | **No effect as of v2.1.261** — Bash/GNU readline-style word editing is now the default behavior. Previously accepted `"readline"` to enable Ctrl+W word-deletion to whitespace boundary (v2.1.236–v2.1.260) |
...
| `timeFormat` | string | - | Timestamp display format. Accepts `"12h"` (12-hour with AM/PM), `"24h"` (24-hour local), `"24h-utc"` (24-hour UTC), or a custom strftime-style pattern (e.g., `"%H:%M:%S"`). When unset, uses the system locale default. Pairs with `timeZone` (v2.1.257) |
| `timeZone` | string | - | Timezone for timestamp display. Accepts IANA timezone names (e.g., `"America/New_York"`, `"Asia/Karachi"`, `"UTC"`). When unset, uses the system timezone. Pairs with `timeFormat` (v2.1.257) |
...
| `permissionExplainerEnabled` | boolean | - | **Removed in v2.1.257.** Previously showed an AI-generated natural-language explanation of why a permission is being requested. No longer read or honored |
...
| `SELF_HOSTED_RUNNER_CLIENT_LABEL` | Override the label used when registering a self-hosted runner with `claude self-hosted-runner`. Alternative to the `--client-label` CLI flag — useful in CI environments where flags are harder to inject than environment variables (v2.1.248) |
...
| `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` | Apply the subagent model to every subagent spawn, ignoring explicit model overrides in agent definitions and tool calls. Unlike `CLAUDE_CODE_SUBAGENT_MODEL` (which is a default that can be overridden), this variable forces the model on all subagents regardless of their own model configuration. Use to enforce a cost ceiling across an entire agentic workflow (v2.1.257) |
