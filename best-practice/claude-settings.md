...
![Last Updated](https://img.shields.io/badge/Last_Updated-Aug%2013%2C%202026%2010%3A48%20AM%20PKT-white?style=flat&labelColor=555) ![Version](https://img.shields.io/badge/Claude_Code-v2.1.229-blue?style=flat&labelColor=555)<br>
[![Implemented](https://img.shields.io/badge/Implemented-2ea44f?style=flat)](../.claude/settings.json)

A comprehensive guide to all available configuration options in Claude Code's `settings.json` files. As of v2.1.229, Claude Code exposes **129+ settings** and **317+ environment variables** (use the `"env"` field in `settings.json` to avoid wrapper scripts).
...
| `crossSessionInbound` | string | `"accept"` | Controls how inbound messages from other Claude Code sessions are handled. `"accept"` processes them immediately (default), `"hold"` queues them for review, `"refuse"` discards them. Configure via user or managed settings; not read from project settings (v2.1.220) |
...
| `dialogExpiry` | string | `"5m"` | Auto-delivery timeout for remote dialog messages when no client is connected. Values: `"60s"`, `"5m"`, `"10m"`, `"never"`. Dialogs that exceed this timeout are auto-continued or dismissed. Pairs with `askUserQuestionTimeout` for client-side behavior (v2.1.220) |
...
> **Starting August 14, 2026:** Auto mode becomes the default permission mode for new sessions on Pro, Max, and Team plans. A default you set yourself stays in place unless you accept the one-time switch prompt; a managed default is unchanged. Set `disableAutoMode: "disable"` to opt out.
...
**Marketplace source types:** `github`, `git`, `directory`, `settings`, `url`, `npm`, `file`, `archive`, `command`. Use `source: 'settings'` to declare a small set of plugins inline without setting up a hosted marketplace repository. Use `source: 'archive'` for zip-based plugin installation with SHA-256 pinning (v2.1.224). Use `source: 'command'` to derive plugins by running a shell command that outputs the marketplace manifest. Note: `hostPattern` is a *matcher* field for `blockedMarketplaces`, not a source type.
...
| `CLAUDE_CODE_MAX_SUBAGENTS_PER_SESSION` | **Removed as of v2.1.224.** The 200-subagent-per-session spawn cap was removed; long-running sessions no longer refuse new subagent spawns. Concurrency and depth limits (`CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS`, `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH`) still apply *(in v2.1.212 changelog; cap removed in v2.1.224)* |
...
| `ANTHROPIC_PROFILE` | Named authentication profile to use when multiple credential profiles are configured. Selects which stored credential set Claude Code authenticates with on startup (on official env-vars page) |
| `ANTHROPIC_FEDERATION_RULE_ID` | Workload identity federation rule ID used to authenticate with the Anthropic API via a federation provider. Required when using OIDC or SAML-based identity federation (on official env-vars page) |
| `ANTHROPIC_ORGANIZATION_ID` | Organization ID for scoping authentication and usage to a specific Anthropic organization. When set, requests are attributed to the named organization (on official env-vars page) |
| `CLAUDE_AX_STARTUP_QUIET_MS` | Milliseconds to suppress the screen-reader startup announcement after Claude Code launches. Use to avoid the announcement overlapping with terminal output during initialization (on official env-vars page) |
| `CLAUDE_CODE_WORKFLOW_PREFIX_STAGGER_MS` | Milliseconds to stagger fan-out between batches of workflow agent launches. Reduces thundering-herd API load when a workflow spawns many agents simultaneously (v2.1.229) |
| `CLAUDE_CODE_DISABLE_UNKNOWN_MODEL_WINDOW_ENFORCEMENT` | Set to `1` to restore the pre-v2.1.223 auto-compact behavior for unrecognized model IDs. By default, Claude Code refuses to run if it cannot determine the context window for the configured model; this variable bypasses that check (v2.1.223) |
