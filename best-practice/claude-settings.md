...
![Last Updated](https://img.shields.io/badge/Last_Updated-Sep%2013%2C%202026%2010%3A45%20AM%20PKT-white?style=flat&labelColor=555) ![Version](https://img.shields.io/badge/Claude_Code-v2.1.270-blue?style=flat&labelColor=555)<br>
[![Implemented](https://img.shields.io/badge/Implemented-2ea44f?style=flat)](../.claude/settings.json)

A comprehensive guide to all available configuration options in Claude Code's `settings.json` files. As of v2.1.270, Claude Code exposes **160+ settings** and **320+ environment variables** (use the `"env"` field in `settings.json` to avoid wrapper scripts).
...
**Exception — stricter-value-from-any-scope keys:** The following keys honor the stricter (more restrictive) value from any settings file, even lower-priority scopes: `disableClaudeAiConnectors` (`true` from any scope wins), `enableArtifact` (`false` from any scope wins), `isolatePeerMachines` (`true` from any scope wins), `maxEffortLevel` (lower cap from any scope wins, v2.1.267), `crossSessionInbound` (stricter value from project/local wins), `syncClaudeAiSkills` (`false` from any scope wins), `remoteControlAtStartup` (`false` from project/local applies), and `useAutoModeDuringPlan` (`false` from any scope wins).
...
| `managedSourcesBehavior` | string | `"first-wins"` | **(Managed only)** Controls how multiple deployed managed sources (server-managed, MDM, file) are combined. `"first-wins"` (default): only the highest-priority managed source applies; others are ignored. `"merge"`: all managed sources are composed — lower-priority sources apply under the winning source, so each source can tighten policy independently. Use `"merge"` when deploying from multiple independent managed sources (e.g., a platform team + a security team) |
| `sshHostAllowlist` | string[] | - | **(Managed only)** Allowlist of SSH hosts that remote background sessions are permitted to connect to. Each entry is a hostname or CIDR pattern |
| `disableDesktopLocalSessions` | boolean | - | **(Managed only)** Set to `true` to prevent Claude Code desktop from starting local interactive sessions. Only allows remote/cloud sessions when enabled |
| `gatewayInternalNetworks` | string[] | - | **(Managed only)** List of network CIDR ranges or hostnames that are treated as internal organization networks for Claude Code Gateway `/login` routing. Allows users on internal networks to authenticate without going through the public gateway endpoint (v2.1.268) |
...
| `enableWorkflows` | boolean | - | Turn dynamic workflows on or off against your plan's default. Use when you want to enable workflows on a plan where they're off by default, or control access per-project. Complements `disableWorkflows` |
| `isolatePeerMachines` | boolean | `false` | Prompt for confirmation before Claude messages one of your sessions on another machine via Remote Control. Set to `true` to require approval for cross-machine session messages. **Managed-precedence exception:** `true` from any scope applies even against a managed `false` |
| `bashOutputMaxChars` | number | - | Maximum number of characters to capture from Bash tool output (up to 128,000). When set, truncates Bash stdout/stderr to this limit. Useful in managed environments to prevent runaway output from filling context (v2.1.261) |
| `taskOutputMaxChars` | number | - | Maximum number of characters to capture from background task output (up to 128,000). When set, truncates task stdout/stderr to this limit (v2.1.261) |
...
| `skipAutoPermissionPrompt` | boolean | Skip the one-time auto-mode entry prompt that appears before activating auto mode for the first time in a session. Honored from user or managed settings only (v2.1.257) |
...
| `"bypassPermissions"` | Skip all permission checks (dangerous). All path-based prompts are skipped — writes to `.git`, `.config/git`, `.claude`, `.vscode`, `.idea`, `.husky`, `.cargo`, `.devcontainer`, `.yarn`, and `.mvn` no longer prompt (**v2.1.121** exempted `.claude/commands/`, `.claude/agents/`, `.claude/skills/`, and `.claude/worktrees/`; **v2.1.126** removed all remaining path-based prompts). Three classes still prompt regardless: (1) removals targeting the filesystem root or home directory (`rm -rf /`, `rm -rf ~`, including commands using `$(...)`, backtick substitution, or `<(...)` process substitution), (2) tools marked `requiresUserInteraction` in their MCP server manifest, and (3) organization-set-to-ask connector tools. Explicit `permissions.ask` rules still prompt — only the built-in permission checks are bypassed. **Scope restriction (v2.1.257):** `defaultMode: "bypassPermissions"` is ignored when set in project (`.claude/settings.json`) or local settings; use `~/.claude/settings.json` or managed settings |
...
| `allowedMcpServers` | array | Any | Allowlist with name/command/URL matching. When set in user settings, restricts user-added servers only; managed servers are unaffected |
| `deniedMcpServers` | array | Any | Blocklist with name/command/URL matching |
| `managedMcpServers` | object[] | Managed only | Provide remote HTTP/SSE MCP servers to every user alongside the servers they add themselves. Each entry specifies `type` (`"http"` or `"sse"`), `url`, and optional authentication. Requires v2.1.259+ |
...
| `sandbox.ripgrep` | string | - | **(User or managed)** Absolute path to a custom `ripgrep` binary to use inside the sandbox. When set, Claude Code uses this binary for file search operations rather than the bundled or system ripgrep. Example: `/opt/admin/rg` |
...
| `"fable-5-1"` | Claude Fable 5.1 (`claude-fable-5-1`) — updated Fable model with 1M context by default. Introduced v2.1.257 |
...
| `maxEffortLevel` | string | - | Cap the maximum effort level for every model or per model, on every provider. Accepts the same values as `effortLevel` (`"low"`, `"medium"`, `"high"`, `"xhigh"`). When set, users and subagent definitions cannot exceed this cap. **Managed-precedence exception:** the lowest cap from any scope wins — a lower `maxEffortLevel` in project settings applies even against a higher value in managed settings. Useful for cost control in managed deployments (v2.1.267) |
...
| `keybindingFlavor` | string | - | **REMOVED in v2.1.268.** Bash/GNU readline-style Ctrl+W is now the default word-deletion behavior; the setting was removed. Previously set to `"readline"` for readline-style behavior (introduced v2.1.236) |
| `timeFormat` | string | - | Display format for timestamps in the UI. Values: `"12h"` (12-hour with AM/PM), `"24h"` (24-hour), `"utc"` (UTC), or a `strftime`-style format string. Pairs with `timeZone` (v2.1.257) |
| `timeZone` | string | - | Timezone for timestamp display. Accepts IANA timezone names (e.g., `"Asia/Karachi"`, `"America/New_York"`, `"UTC"`). Pairs with `timeFormat` (v2.1.257) |
| `bashEditDiffEnabled` | boolean | `true` | Display file change diffs when the Bash tool handles file edits directly (e.g., via `sed`, `awk`, or shell redirection). When `true`, Claude shows a diff of the changes made by Bash file-editing operations (v2.1.270) |
| `promptSuggestionEnabled` | boolean | `true` | Show grayed-out prompt suggestions in the input box. Set to `false` to hide the suggestion hints |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `copyOnSelect` | boolean | `true` | Automatically copy text you select with the mouse in fullscreen rendering and agent view. Set to `false` to disable auto-copy-on-select |
...
| `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` | Force the model used for all subagents, overriding agent definitions and explicit model parameters. Unlike `CLAUDE_CODE_SUBAGENT_MODEL` (which is a default), this variable cannot be overridden. Use with caution in environments where all subagents must use a specific model (v2.1.257) |
...
| `OTEL_METRICS_INCLUDE_REPOSITORY` | Set to `1` to include VCS repository attributes (repository name, branch) as labels on all OpenTelemetry metric data points. Useful for breaking down metrics by repository in multi-repo environments (v2.1.269) |
...
| `CLAUDE_CODE_WEBFETCH_DEADLINE_MS` | Hard deadline in milliseconds for WebFetch requests. Default is 300,000ms (5 minutes) — requests that don't complete within this time are cancelled with an error. Set to `0` to disable the deadline. Use when fetching from slow or large pages that exceed the default timeout (v2.1.268) |
...
| `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS` | Maximum number of agents that the Workflow tool can run concurrently. Accepts a value from `1` to `256`. Use to raise (or lower) the default concurrent agent limit for large or small workflows (v2.1.269) |
| `CLAUDE_CODE_GATEWAY_MODEL_DISCOVERY_TIMEOUT_MS` | Timeout in milliseconds for Claude Code Gateway model discovery at session start. Default is 3000ms (3s). Increase if your gateway is slow to respond during model enumeration (v2.1.269) |
