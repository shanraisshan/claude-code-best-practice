...
A comprehensive guide to all available configuration options in Claude Code's `settings.json` files. As of v2.1.268, Claude Code exposes **220+ settings** and **350+ environment variables** (use the `"env"` field in `settings.json` to avoid wrapper scripts).
...
| `ultracode` | boolean | - | When `true`, the harness authors and runs a workflow for every substantive task by default, maximizing thoroughness regardless of token cost. Can be set in any `settings.json` file or via the SDK `settings` option. Also activatable as session-only via `/effort ultracode` or `--settings` without persisting (v2.1.154) |
...
| `askUserQuestionTimeout` | string | `"never"` | How long to wait before an unanswered AskUserQuestion dialog auto-continues without the user. Values: `"60s"`, `"5m"`, `"10m"`, `"never"` (no auto-continue — the default). Set via `/config` as **Question auto-continue timeout**. Honored from user settings (`~/.claude/settings.json`) and managed settings; the `/config` UI writes to user settings. Pairs with the `CLAUDE_AFK_TIMEOUT_MS` env var; the env var applies only when this setting is set to a duration. (v2.1.200) |
...
| `bashOutputMaxChars` | number | `30000` | Maximum inline characters Claude shows for a bash command result before truncating (clamped 4000–128000). Raise up to `128000` (128K) for commands that emit large structured output (e.g., `jq` on a big JSON file, test suite reports). When set, overrides the `BASH_MAX_OUTPUT_LENGTH` env var (v2.1.261) |
| `taskOutputMaxChars` | number | `32000` | Maximum inline characters Claude shows for a background-task (agent) result before truncating (clamped 4000–128000). Raise up to `128000` (128K) for tasks with large output. When set, overrides the `TASK_MAX_OUTPUT_LENGTH` env var (v2.1.261) |
...
| `permissions.blockReadsOutsideWorkingDirectories` | boolean | Block file reads outside the working directory (and `additionalDirectories`) in all permission modes, including `bypassPermissions`. When `true`, Read, Glob, and similar tools cannot access paths outside the allowed directory set. Can be set from any scope (v2.1.257) |
...
| `allowedMcpServers` | array | Any file | Allowlist with name/command/URL matching. **v2.1.259:** managed servers are exempt from the allowlist — they load even if `allowedMcpServers` does not list them |
| `deniedMcpServers` | array | Any file | Blocklist with matching |
| `managedMcpServers` | object | Managed only | Organization-deployed remote MCP servers (HTTP/SSE) pushed to all users. Keys are server names; each value specifies `type` (`"http"` or `"sse"`), `url`, and optional `headers`. Managed servers load regardless of `allowedMcpServers`; they cannot be blocked by user or project settings. Use for centrally managed tools (databases, internal APIs) (v2.1.259) |
...
| `sandbox.ignoreViolations` | object | `{}` | Map of command patterns to path arrays — suppress violation warnings for specific paths accessed by specific commands |
...
| `"best"` | Resolves to Fable 5 where available, otherwise falls back to Opus. Use when you want the most capable model without hardcoding a specific alias |
...
| `modelPicker` | array | - | Curate and order the `/model` picker with labeled, ordered rows. Overrides the default model list when set. **Does not merge across settings files** — the highest-precedence file that defines it supplies the entire list. Each entry uses `model` (model ID or alias) and optional `label` and `options` array for provider overrides. Example: `[{"model": "opus", "label": "Opus 5 (reasoning)"}, {"model": "sonnet", "label": "Sonnet 5 (balanced)"}]` (v2.1.242+) |
