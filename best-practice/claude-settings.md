...
![Last Updated](https://img.shields.io/badge/Last_Updated-Aug%2011%2C%202026%2010%3A45%20AM%20PKT-white?style=flat&labelColor=555) ![Version](https://img.shields.io/badge/Claude_Code-v2.1.227-blue?style=flat&labelColor=555)<br>
[![Implemented](https://img.shields.io/badge/Implemented-2ea44f?style=flat)](../.claude/settings.json)

A comprehensive guide to all available configuration options in Claude Code's `settings.json` files. As of v2.1.227, Claude Code exposes **129+ settings** and **313 environment variables** (use the `"env"` field in `settings.json` to avoid wrapper scripts).
...
| `tui` | string | `"auto"` | Rendering mode: `"auto"` (smart default), `"classic"` (minimal legacy output), or `"fullscreen"` (alt-screen flicker-free rendering). Set via `/tui fullscreen` or `/tui classic` (v2.1.110) |
...
| `dialogExpiry` | string | `"5m"` | Deadline for dialogs forwarded to remote clients (Remote Control, SDK host). When the deadline passes, Claude Code cancels and continues with a no-action default. Does **not** govern permission prompts or `AskUserQuestion` dialogs (use `askUserQuestionTimeout` for those). Values: `"60s"`, `"5m"`, `"10m"`, `"never"`. Override via the `CLAUDE_CODE_USER_DIALOG_TIMEOUT_MS` env var. Only honored from user, managed, and `--settings` sources (v2.1.224) |
| `crossSessionInbound` | string | - | Controls how inbound cross-session messages are handled. Values: `"accept"` (auto-deliver), `"hold"` (queue for approval), `"refuse"` (reject silently). Precedence: managed → `--settings` → user (first match wins); project/local settings apply only when stricter than the trusted sources' value on the `accept` < `hold` < `refuse` scale. Cross-session messages sent to a bypass-permissions session are held for approval by default unless explicitly set to `"accept"` from a trusted source (v2.1.224) |
...
| `sandbox.credentials` | object | — | Fine-grained control over which credential files and environment variables are blocked from sandboxed subprocess environments. Object with two arrays: `files` (array of credential file entries — see sub-keys below) and `envVars` (array of credential env var entries — see sub-keys below). An individual invalid entry is stripped with a warning; the valid subset is enforced. (v2.1.187; per-entry object shape since v2.1.191; per-entry `mode` and `injectHosts` since v2.1.199; `extract`, `onExtractNoMatch`, `decode`, `maskClaims` on `envVars[]` since v2.1.224) |
...
| `sandbox.credentials.files[].decode` | string | — | Set to `"jwt"` to enable JWT-aware masking. Claude Code finds JWT candidates using a built-in pattern (or your `extract` pattern when also set), verifies each is a JWT, and replaces it with a structurally valid fake token so code that decodes the token keeps working. Use `maskClaims` to mask only named top-level payload claims instead of replacing the whole token. When no candidate verifies or no claim matches, `onExtractNoMatch` governs the outcome. Cannot be combined with `extract` alone — both can be set to restrict where JWT detection runs. Requires `mode: "mask"`. Only honored from user settings, managed settings, or `--settings` (v2.1.224) |
| `sandbox.credentials.files[].maskClaims` | array | — | List of top-level JWT payload claim names to mask individually instead of replacing the entire token. Requires `decode: "jwt"`. Claims not listed remain readable inside the sandbox (v2.1.224) |
| `sandbox.credentials.envVars[].name` | string | — | Name of the environment variable to protect |
| `sandbox.credentials.envVars[].mode` | string | `"deny"` | `"deny"` (default, strips the var entirely) or `"mask"` (substitutes a per-session sentinel value, only honored from user settings/managed/`--settings`; `deny` takes precedence when the same var appears with both modes) |
| `sandbox.credentials.envVars[].injectHosts` | array | — | Hostnames that receive the real credential value via the TLS-termination proxy even when `mode` is `"mask"`. Requires `sandbox.network.tlsTerminate` (v2.1.199) |
| `sandbox.credentials.envVars[].extract` | string | — | Regex with a single capture group used to locate credential values within the variable for masking. Claude Code applies it across the value and replaces only the text captured by group 1 of each match. Required when `mode` is `"mask"` and the value has embedded structure (e.g., a connection string or config file). Cannot be combined with `decode`. (v2.1.224) |
| `sandbox.credentials.envVars[].onExtractNoMatch` | string | `"warn"` | Behavior when the `extract` pattern or `decode: "jwt"` verification finds nothing to mask: `"warn"` (default, warns and passes the variable through unmasked), `"deny"` (unsets the variable), or `"error"` (aborts sandbox setup) (v2.1.224) |
| `sandbox.credentials.envVars[].decode` | string | — | Set to `"jwt"` to enable JWT-aware masking. Claude Code verifies the value is a JWT and replaces it with a structurally valid fake token so code that decodes the token inside the sandbox keeps working. Use `maskClaims` to mask only named top-level payload claims. Cannot be combined with `extract`. Only honored from user settings, managed settings, or `--settings` (v2.1.224) |
| `sandbox.credentials.envVars[].maskClaims` | array | — | List of top-level JWT payload claim names to mask individually instead of replacing the entire token. Requires `decode: "jwt"`. Claims not listed remain readable inside the sandbox (v2.1.224) |
| `sandbox.credentials.awsPairs` | array | — | Groups custom-named AWS credential environment variables so the sandbox proxy can re-sign SigV4 requests using them. Each entry has `accessKeyIdVar` and `secretAccessKeyVar` (both required — named `mask` entries that mask whole values) and optional `sessionTokenVar`. The proxy re-signs requests on the hosts in the access key ID entry's `injectHosts`. Naming any of the conventional AWS vars (`AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_SESSION_TOKEN`) replaces the automatic pairing for those vars. Only honored from user settings, managed settings, or `--settings`. Requires `mode: "mask"` on each referenced entry (v2.1.224) |
| `sandbox.credentials.sigv4` | object | — | Per-form relaxation for the three AWS request forms the proxy cannot re-sign. Setting a form's key to `"passthrough"` forwards the request with its placeholder-derived signature so the calling tool receives AWS's own error instead of a proxy error. Keys: `streaming` (aws-chunked streaming uploads), `presigned` (presigned URLs), `sigv4a` (SigV4A asymmetric signatures). Only honored from user settings, managed settings, or `--settings` (v2.1.224) |
...
| `CLAUDE_CODE_USER_DIALOG_TIMEOUT_MS` | Override the `dialogExpiry` deadline (in milliseconds) for dialogs forwarded to remote clients (Remote Control, SDK host). Takes precedence over the `dialogExpiry` setting. Does **not** affect `AskUserQuestion` dialogs (use `CLAUDE_AFK_TIMEOUT_MS` for those) (v2.1.224) |
...
| `CLAUDE_CODE_DISABLE_UNKNOWN_MODEL_WINDOW_ENFORCEMENT` | Set to `1` to restore the pre-v2.1.219 behavior where sessions on unrecognized model IDs can grow past the assumed context window without auto-compaction enforcing the limit. By default, Claude Code enforces context window limits for all models, including unrecognized ones, using a conservative assumed window size (v2.1.219) |
...
| `CLAUDE_CODE_MAX_SUBAGENTS_PER_SESSION` | **REMOVED cap in v2.1.224.** The 200-subagent-per-session spawn limit has been removed; long-running sessions no longer refuse new agents. Concurrency and depth limits (`CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS`, `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH`) still apply *(in v2.1.212 changelog, not yet on official env-vars page)* |
