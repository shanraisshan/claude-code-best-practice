...
![Last Updated](https://img.shields.io/badge/Last_Updated-Sep%2015%2C%202026%2010%3A49%20AM%20PKT-white?style=flat&labelColor=555) ![Version](https://img.shields.io/badge/Claude_Code-v2.1.272-blue?style=flat&labelColor=555)<br>
[![Implemented](https://img.shields.io/badge/Implemented-2ea44f?style=flat)](../.claude/settings.json)

A comprehensive guide to all available configuration options in Claude Code's `settings.json` files. As of v2.1.272, Claude Code exposes **145+ settings** and **320+ environment variables** (use the `"env"` field in `settings.json` to avoid wrapper scripts).
...
| `managedSourcesBehavior` | string | `"all"` | **(Managed only)** Controls which sources can contribute managed settings. `"all"` (default): all configured managed sources (server-managed, MDM, file, policyHelper) apply. `"managed-settings-file-only"`: only the file-based managed settings source is honored; server-managed and MDM sources are ignored. Use to restrict policy delivery to a known-good local file (v2.1.261) |
| `disableDesktopLocalSessions` | boolean | - | **(Managed only)** Set to `true` to prevent users from starting local Claude Code sessions in the desktop app. When set, the desktop app requires a remote environment for all sessions. Only honored from managed settings (v2.1.265) |
| `sshHostAllowlist` | array | - | **(Managed only)** List of SSH host patterns (glob-style) that users are permitted to connect to via the desktop SSH session picker. When set, the picker rejects connections to hosts not matching any entry. Use to restrict remote session targets in managed environments (v2.1.265) |
| `gatewayInternalNetworks` | array | - | **(Managed only)** Array of IPv4 CIDR blocks belonging to the organization's network. When the user tries to log in to a Claude apps gateway (`forceLoginMethod: "gateway"`) from an IP in one of these ranges, `/login` succeeds even if the gateway's auth endpoint is on the org's internal network rather than a public address. Use when the Claude gateway is accessible only from corporate VPN (v2.1.268) |
...
| `enableWorkflows` | boolean | - | Turn workflows on or off **against your plan's default behavior** — distinct from `disableWorkflows`. When your plan's default enables workflows, set to `false` here to disable them for this settings scope; when your plan's default disables them, set to `true` to enable. Can be set at any scope |
| `isolatePeerMachines` | boolean | - | When `true`, Claude asks before sending messages to your sessions on another machine via Remote Control. **Managed-precedence exception:** `true` from any scope wins (any scope can strengthen the isolation, only a lower-scope `false` can loosen it). Appears in `/config` as **Confirm messages to your other machines** |
...
| `bashOutputMaxChars` | number | - | Maximum characters of inline Bash tool output shown in the conversation before the full output is spilled to a temp file (up to 128K chars). When unset, the default truncation threshold applies. Use to tune how much command output appears inline versus in a linked file (v2.1.261) |
| `taskOutputMaxChars` | number | - | Maximum characters of inline background-task output shown before the full output is spilled to a temp file. Mirrors `bashOutputMaxChars` for background task results (v2.1.261) |
...
| `permissions.blockReadsOutsideWorkingDirectories` | boolean | `false` | When `true`, file tools (`Read`, `Edit`, `Write`) refuse to access paths outside the working directory and any `additionalDirectories` **in every permission mode**, including `bypassPermissions`. Auto mode offers to enable this restriction automatically on the first attempt to read outside the working directories (v2.1.260+) |
| `skipAutoPermissionPrompt` | boolean | - | When `true`, skips the one-time confirmation notice shown when entering auto mode for the first time. Only honored from user or managed settings. Useful in environments where auto mode is pre-configured and the onboarding prompt would be disruptive |
...
| `managedMcpServers` | array | Managed only | Provide an array of MCP server definitions to every user in the organization. Each entry follows the same shape as `mcpServers` (name, type, command/url, env, headers, etc.). **Managed only** — user, project, and local settings cannot define this key. When a `managed-mcp.json` cannot be read or parsed, Claude Code blocks all user/project/plugin MCP servers and starts with only the managed list (fail-closed, v2.1.272) |
...
| `sandbox.allowUnsandboxedCommands` | boolean | `true` | Allow the `dangerouslyDisableSandbox` escape hatch. When set to `false`, the escape hatch is disabled and most commands must run sandboxed (or be in `excludedCommands`). **Note (v2.1.260):** commands typed directly at the `!` bash-mode prompt always run outside the sandbox even when this is `false` — the restriction applies to Claude-initiated bash commands, not user-typed ones. Useful for enterprise policies that require strict sandboxing on Claude-authored commands |
...
| `sandbox.ripgrep` | string | - | Path to a custom ripgrep binary used inside the sandbox for the Grep tool. Use when your sandbox environment does not have `rg` on `PATH` or requires a specific binary. Only honored from user and managed settings |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `maxEffortLevel` | string | - | Cap the effort level allowed in this scope. Accepts `"low"`, `"medium"`, `"high"`, `"xhigh"`. When set, any effort above this ceiling is silently clamped down — `/effort xhigh` on a session where a managed setting sets `maxEffortLevel: "high"` runs at `high`. **Managed-precedence exception:** the lowest ceiling from any settings scope wins; a stricter cap from project or local settings applies even against a more permissive managed value (v2.1.267) |
...
| `keybindingFlavor` | string | - | **Deprecated and has no effect as of v2.1.261.** Word-editing keys now unconditionally follow Bash/readline behavior; this key is accepted but ignored. Previously used to set `"readline"` for Ctrl+W word-deletion (v2.1.236; deprecated v2.1.261) |
...
| `promptSuggestionEnabled` | boolean | `true` | Show grayed-out prompt suggestions in the input area. Set to `false` to hide suggestions. Equivalent to the `CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION` env var |
| `bashEditDiffEnabled` | boolean | - | When `true`, appends a diff of files a Bash command modified to the Bash tool result, making it easier to see what shell commands changed on disk. Set via `/config`. *(in v2.1.269 changelog, not yet on official settings page — unverified)* |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `copyOnSelect` | boolean | `false` | Automatically copy selected text in fullscreen mode to the system clipboard. When `true`, text selected with the mouse is immediately copied without requiring an explicit copy command |
...
| `permissionExplainerEnabled` | boolean | `true` | **Removed in v2.1.257.** Previously showed an AI-generated natural-language explanation of why a permission is being requested alongside the permission prompt. The setting is accepted but ignored; permission prompts no longer include AI-generated explanations |
...
| `CLAUDE_CODE_WORKFLOW_MAX_CONCURRENT_AGENTS` | Maximum number of agents that a single dynamic workflow run can execute concurrently. Accepts an integer from 1 to 256. Use to throttle parallel fan-out when downstream services have capacity limits (v2.1.269) |
| `CLAUDE_CODE_BG_TASKS_REPORT_RUNNING` | Set to `0` to restore the pre-v2.1.269 "waiting for your input" indicator while background agents are still running, instead of the current "background agents running" status line. Use when the new status display causes issues with terminal output parsing (v2.1.269) |
| `CLAUDE_CODE_GATEWAY_MODEL_DISCOVERY_TIMEOUT_MS` | Timeout in milliseconds for the LLM gateway `/v1/models` model discovery request. Default: `3000` (3 seconds). Increase if your gateway is slow to respond to the model-list endpoint (v2.1.269) |
| `OTEL_METRICS_INCLUDE_REPOSITORY` | Set to `1` to tag OpenTelemetry metrics and events with `vcs.*` resource attributes. Commit-related events additionally include `vcs.ref.head.*` fields when `OTEL_LOG_TOOL_DETAILS` is also set (v2.1.269) |
| `CLAUDE_CODE_WEBFETCH_DEADLINE_MS` | Hard deadline in milliseconds for WebFetch requests. The request is aborted if it does not complete within this limit. Default: `300000` (300 seconds); set to `0` to disable the deadline. Distinct from `CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS` (which controls caching, not timeouts) (v2.1.268) |
| `ANTHROPIC_UNIX_SOCKET` | Path to a Unix domain socket for a local LLM proxy. When set, API requests are routed through this socket instead of the network. Organization-level policy routing is applied correctly through this path (v2.1.272) |
...
- [Claude Code Settings Documentation](https://code.claude.com/docs/en/settings) — settings files, precedence, and hierarchy only (not a key list; use Settings Reference above for individual keys)
