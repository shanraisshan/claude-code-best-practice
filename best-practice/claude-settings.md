...
![Last Updated](https://img.shields.io/badge/Last_Updated-Sep%2012%2C%202026%2010%3A47%20AM%20PKT-white?style=flat&labelColor=555) ![Version](https://img.shields.io/badge/Claude_Code-v2.1.269-blue?style=flat&labelColor=555)<br>
[![Implemented](https://img.shields.io/badge/Implemented-2ea44f?style=flat)](../.claude/settings.json)

A comprehensive guide to all available configuration options in Claude Code's `settings.json` files. As of v2.1.269, Claude Code exposes **145+ settings** and **315+ environment variables** (use the `"env"` field in `settings.json` to avoid wrapper scripts).
...
| `bashOutputMaxChars` | number | - | Maximum number of characters from a successful Bash command's output that Claude receives inline. Output beyond this limit is truncated. Raising the cap (up to ~128K) lets Claude see more of long-running command results without manual scrolling. See [output limits](https://code.claude.com/docs/en/tools-reference#output-limits) |
...
| `isolatePeerMachines` | boolean | - | When `true`, Claude Code prompts for approval before messaging one of your sessions running on a different machine via cross-session messaging. Lets you gate cross-machine session-to-session traffic while still allowing same-machine peer sessions to communicate freely. Any settings scope |
...
| `feedbackDrafts` | boolean | `true` | When `true`, Claude Code queues bug-report drafts in the background when it encounters surprising errors. Set to `false` to disable background draft queuing. Gates the `SendFeedback` tool. Appears in `/config` as **Feedback drafts**. **Only honored from user and managed settings** — project and local settings values are ignored. (v2.1.247) |
...
| `sandbox.ripgrep` | string | - | Absolute path to a custom `ripgrep` (`rg`) binary used by the Grep tool inside the sandbox. When set, this binary is used instead of any `rg` found on `PATH` in the sandboxed environment. Honored from user settings and managed settings only |
...
| `maxEffortLevel` | number | - | Cap the maximum effort level across every model and provider. Values follow the effort-level ordinal scale (e.g., `3` for high, `4` for xhigh). When set, users and subagents cannot exceed this effort ceiling even via `/effort` or `--settings`. Settable from any settings scope. Pair with `effortLevel` to both set a default and cap the maximum |
...
| `promptSuggestionEnabled` | boolean | `true` | Show grayed-out prompt suggestions in the input box when the prompt is empty. Set to `false` to hide the suggestions. Any settings scope |
