...
![Last Updated](https://img.shields.io/badge/Last_Updated-Jul%2023%2C%202026%2010%3A45%20AM%20PKT-white?style=flat&labelColor=555) ![Version](https://img.shields.io/badge/Claude_Code-v2.1.218-blue?style=flat&labelColor=555)<br>
...
| `sandbox.filesystem.disabled` | boolean | `false` | Skip filesystem isolation while keeping network egress control. When `true`, sandboxed commands have unrestricted read/write access to the host filesystem; network isolation stays intact. **Scope restriction**: only honored from user settings (`~/.claude/settings.json`), managed settings, or `--settings` CLI flag — ignored in `.claude/settings.json` and `.claude/settings.local.json` to prevent projects from disabling filesystem security. When managed settings configure `sandbox.filesystem` or list any `sandbox.credentials.files`, only managed settings can set `filesystem.disabled`. Requires v2.1.216+ |
...
| `emojiCompletionEnabled` | boolean | `true` | Enable emoji shortcode autocomplete in the prompt input. When `true`, typing `:heart:` suggests ❤️ and similar emoji shortcuts. Set to `false` to disable. Appears in `/config` as **Emoji completion** (v2.1.217) |
...
| `footerLinksRegexes` | object[] | - | Render clickable badges in the footer row based on regex matches in turn output. Each entry is an object with: `type` (`"regex"`), `pattern` (regex string with named capture groups), `url` (URL template — `{name}` placeholders filled from capture group names), and optional `label` (badge display text, also supports `{name}` placeholders). **Scope restriction**: read from user settings, `--settings` flag, and managed settings only — not from project or local settings. See official docs for URL scheme allowlist and badge count limits. Example: `[{"type": "regex", "pattern": "\\b(?<key>PROJ-\\d+)\\b", "url": "https://issues.example.com/browse/{key}", "label": "{key}"}]` (v2.1.176) |
