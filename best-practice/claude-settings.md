...
![Last Updated](https://img.shields.io/badge/Last_Updated-Aug%2024%2C%202026%2010%3A57%20AM%20PKT-white?style=flat&labelColor=555) ![Version](https://img.shields.io/badge/Claude_Code-v2.1.241-blue?style=flat&labelColor=555)<br>
[![Implemented](https://img.shields.io/badge/Implemented-2ea44f?style=flat)](../.claude/settings.json)

A comprehensive guide to all available configuration options in Claude Code's `settings.json` files. As of v2.1.241, Claude Code exposes **133+ settings** and **313 environment variables** (use the `"env"` field in `settings.json` to avoid wrapper scripts).
...
| `skipWebFetchPreflight` | boolean | `false` | Skip the WebFetch domain safety check that sends each requested hostname to `api.anthropic.com` before fetching. Set to `true` in environments that block outbound traffic to Anthropic, such as Bedrock, Vertex AI, or Foundry deployments with restrictive egress. Any file scope |
...
| `crossSessionInbound` | string | `"accept"` | Whether Claude Code delivers messages sent from other Claude Code sessions (via Remote Control or peer-session messaging). Values: `"accept"` (deliver all), `"hold"` (queue until user resumes), `"refuse"` (drop all). **Scope exception:** a stricter value (`accept` < `hold` < `refuse`) from project or local settings is honored over managed, `--settings`, and user values — a project/local value that is not stricter is ignored |
| `dialogExpiry` | number | - | How many seconds Claude Code waits for [Remote Control](https://code.claude.com/docs/en/remote-control) to answer a forwarded dialog (permission prompt, AskUserQuestion, etc.) before auto-dismissing. When unset, dialogs wait indefinitely. Only honored from user and managed settings |
| `isolatePeerMachines` | boolean | `false` | When `true`, Claude Code asks for confirmation before sending messages to one of your sessions on another machine via Remote Control or peer-session cross-session messaging. **Scope exception:** `true` from any scope (including project settings) is honored even against a managed `false` |
| `syncClaudeAiSkills` | boolean | `true` | When `true` (default), Claude Code downloads skills enabled on your claude.ai account at session start. Set to `false` to skip syncing, which prevents external skill content from being loaded. **Scope exception:** `false` from any scope (user, local, or `--settings`) is honored over a managed `true`; a `false` in shared project settings (`.claude/settings.json`) is ignored |
...
| `sandbox.ignoreViolations` | object | `{}` | Map of command patterns to path arrays — silence violation reports for paths a command is expected to probe (e.g., when a tool probes paths that always fail in a sandboxed environment). Any file scope |
...
| `outputStyle` | string | `"default"` | Adjust Claude's role, tone, and output format. Built-in values: `"default"`, `"Explanatory"` (educational, longer responses), `"Concise"` (brief responses, added v2.1.237). Requires `/clear` or a session restart to apply — part of the system prompt |
...
| `keybindingFlavor` | string | - | Change Ctrl+W delete behavior. Set to `"readline"` to delete back to the previous whitespace boundary (Bash-style word delete) instead of the default character-by-character delete (v2.1.238) |
| `spellcheck` | string | - | Spell-check words in the input prompt using an installed checker. Set to `"aspell"`, `"hunspell"`, or `"ispell"` to underline misspelled words as you type. Only honored from user and managed settings |
...
| `teammateDefaultModel` | string | `null` | **REMOVED in v2.1.234.** Previously: default model for [agent-team](https://code.claude.com/docs/en/agent-teams) teammates when the lead dispatches them. Teammates now always follow the lead's model. Listed under "Global config settings" on the official settings page |
...
| `CLAUDE_AX_PREPARK_MS` | Milliseconds to wait before writing text to the screen reader buffer (default: `50`, capped at `5000`). Increase if your screen reader misses the first characters of a response. Only takes effect in screen-reader mode. Requires v2.1.233+ |
...
| `ANTHROPIC_DEFAULT_MODEL` | Default model for new sessions when no `model` setting or `ANTHROPIC_MODEL` variable overrides it. Unlike `ANTHROPIC_MODEL` (which always wins), this variable applies only when no settings file sets `model`. Requires v2.1.236+ |
