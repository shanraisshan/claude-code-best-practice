...
![Last Updated](https://img.shields.io/badge/Last_Updated-Aug%2014%2C%202026%2011%3A04%20AM%20PKT-white?style=flat&labelColor=555) ![Version](https://img.shields.io/badge/Claude_Code-v2.1.232-blue?style=flat&labelColor=555)<br>
[![Implemented](https://img.shields.io/badge/Implemented-2ea44f?style=flat)](../.claude/settings.json)

A comprehensive guide to all available configuration options in Claude Code's `settings.json` files. As of v2.1.232, Claude Code exposes **127+ settings** and **346 environment variables** (use the `"env"` field in `settings.json` to avoid wrapper scripts).
...
| `crossSessionInbound` | string | - | Controls how this session treats inbound [cross-session messages](https://code.claude.com/docs/en/cross-session-messaging#control-inbound-messages) from other Claude Code sessions. Values: `"accept"` (delivers to Claude), `"hold"` (shows a notice without delivering), `"refuse"` (drops messages). When unset, Claude Code decides per message based on the two sessions' permission-mode classes. Scope: user, managed, `--settings` only; project/local apply only when stricter on the `accept` < `hold` < `refuse` ladder. Appears in `/config` as **Messages from your other sessions** (v2.1.224+) |
| `isolatePeerMachines` | boolean | `false` | Require explicit approval before a `SendMessage` call reaches sessions on other machines. Prompts for approval even when the session is in `bypassPermissions` mode. `true` from **any** scope applies (v2.1.224+) |
...
| `promptSuggestionEnabled` | boolean | `true` | Show grayed-out prompt predictions (ghost text) as you type. Set to `false` to disable. The `CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION` env var takes precedence when set |
...
| `advisorModel` | string | - | Model for the server-side advisor tool. Accepts a model alias (`opus`, `sonnet`, `fable`) or a full model ID. When unset, the advisor uses the session model. Requires v2.1.98+. **v2.1.232+:** Fable 5 is restored as a valid advisor model for organizations with Fable access. (Between v2.1.210 and v2.1.231, `"fable"` did not attach an advisor.) |
...
| `askUserQuestionTimeout` | string | `"never"` | How long to wait before an unanswered AskUserQuestion dialog auto-continues without the user. Values: `"60s"`, `"5m"`, `"10m"`, `"never"` (no auto-continue — the default). Set via `/config` as **Question auto-continue timeout** (writes to user settings). Pairs with the `CLAUDE_AFK_TIMEOUT_MS` env var; the env var applies only when this setting is set to a duration. **Not read from project or local settings** — only honored from user settings (`~/.claude/settings.json`) and managed settings. (v2.1.200) |
| `dialogExpiry` | string | `"5m"` | Sets the deadline for dialogs that Claude Code forwards to remote clients (Remote Control or SDK hosts) and for approval dialogs for held cross-session messages. Values: `"60s"`, `"5m"`, `"10m"`, `"never"`. When no answer arrives before the deadline, Claude Code cancels the dialog and continues with its no-action default. Not used for permission prompts or `AskUserQuestion` questions (those use their own flows). Overridden by the `CLAUDE_CODE_USER_DIALOG_TIMEOUT_MS` env var. Scope: user, managed, `--settings` only — project and local settings values are ignored. Appears in `/config` as **Dialog expiry** (v2.1.224+) |
...
| `strictKnownMarketplaces` | array / boolean | Managed only | Admin allowlist of permitted marketplace sources. `undefined` = no restrictions; `[]` (empty array) = total lockdown including the official Anthropic marketplace; a non-empty array = only those sources are allowed. `true` is also accepted and locks down to a default admin-chosen set. This is NOT an "official-only" flag — the allowlist controls what the admin permits |
...
| `disableCommandPluginSources` | boolean | Managed only | Controls the `command` plugin source, which installs a plugin by running a marketplace-declared command on the user's machine. `true` blocks command-sourced plugins entirely — never runs the command, doesn't install/update those plugins, and stops loading already-installed ones. `false` allows command-sourced plugins explicitly. Unset follows `allowManagedHooksOnly`. Works in conjunction with `allowManagedHooksOnly` and `disableSideloadFlags` (v2.1.229+) |

**Marketplace source types:** `github`, `git`, `directory`, `settings`, `url`, `npm`, `file`, `archive`, `command` (v2.1.229 — runs a marketplace-declared command to install/locate the plugin dir; supports `mode: "link"`), `hostPattern` (matches by hostname regex — e.g. `{ "source": "hostPattern", "hostPattern": "^github\\.example\\.com$" }`), `pathPattern` (matches by URL path regex). Use `source: 'settings'` to declare plugins inline. Use `source: 'archive'` for zip-based installation with SHA-256 pinning (v2.1.224). Note: `hostPattern` and `pathPattern` are valid source types **and** matcher fields for `blockedMarketplaces`/`strictKnownMarketplaces`.
...
| `subagentStatusLine` | object | - | Custom command that rewrites rows in the subagent task display. Same `type`/`command` structure as `statusLine`. Subject to the same `disableAllHooks` and `allowManagedHooksOnly` gates. See [Subagent status lines](https://code.claude.com/docs/en/statusline#subagent-status-lines) |
...
| `diffTool` | string | `"auto"` | Controls where file diffs are displayed when an IDE is connected. Values: `"auto"` (uses IDE diff view when available, otherwise built-in) or `"terminal"` (always uses the built-in terminal diff view). This is not a command path — it is an enum that selects the display destination |
...
| `fast_mode` | Boolean — whether fast mode is enabled for this session |
| `prompt_id` | UUID of the prompt currently being processed. Matches the `prompt.id` field on OpenTelemetry spans. Present only after the first user input arrives |
...
| `ANTHROPIC_FEDERATION_RULE_ID` | Federation rule ID for Workload Identity Federation. Identifies which federation rule governs the token exchange |
| `ANTHROPIC_ORGANIZATION_ID` | Organization ID for Workload Identity Federation. Required when the federation rule is scoped to a specific organization |
| `ANTHROPIC_PROFILE` | Named authentication profile from `ant auth login`. When set, Claude Code uses this profile's stored credentials instead of the default. Takes precedence over keychain-stored credentials |
...
| `CLAUDE_CODE_MESSAGING_SOCKET` | Read-only. Path to the Unix domain socket for inbound cross-session messages when the session is bound to an inbox socket. Available in hooks and Bash tool subprocesses (v2.1.224+) |
| `CLAUDE_CODE_MESSAGING_TOKEN` | Read-only. Per-session authentication token for the cross-session messaging inbox. Companion to `CLAUDE_CODE_MESSAGING_SOCKET` (v2.1.224+) |
...
| `CLAUDE_CODE_USER_DIALOG_TIMEOUT_MS` | Overrides the `dialogExpiry` setting in milliseconds. Sets the deadline for dialogs forwarded to remote clients (Remote Control or SDK hosts) and for held cross-session message approval dialogs. When set, takes precedence over `dialogExpiry`. Not used for permission prompts or `AskUserQuestion` questions (v2.1.224+) |
...
| `CLAUDE_CODE_DISABLE_UNKNOWN_MODEL_WINDOW_ENFORCEMENT` | Set to `1` to skip context-window enforcement for unknown model IDs. When routing through a custom base URL with a model name that Claude Code doesn't recognize, this prevents context-window-exceeded errors that would otherwise block the session (v2.1.223+) |
...
| `CLAUDE_CODE_WORKFLOW_PREFIX_STAGGER_MS` | Delay in milliseconds between prefixed workflow agent starts to stagger concurrent launches. Set to `0` to disable staggering and start all agents simultaneously (v2.1.229+) |
...
| `CLAUDE_SUBAGENT_BG_SHELL_MAX_MS` | Maximum duration in ms for shell commands run inside background subagents (default: `3600000` / 1 hour). Shell calls that exceed this are killed |
...
| `MCP_SDK_GENERATION` | Pin the MCP client runtime version. Set to `"v1"` or `"v2"` to force a specific MCP SDK generation instead of the auto-detected version |
...
| `CLAUDE_AX_STARTUP_QUIET_MS` | Hold time in milliseconds before rendering begins in screen-reader mode (default: `3000` / 3 seconds). Gives screen readers time to orient before the initial UI paints (v2.1.217+) |
