...
![Last Updated](https://img.shields.io/badge/Last_Updated-Sep%2003%2C%202026%2010%3A51%20AM%20PKT-white?style=flat&labelColor=555) ![Version](https://img.shields.io/badge/Claude_Code-v2.1.259-blue?style=flat&labelColor=555)<br>
[![Implemented](https://img.shields.io/badge/Implemented-2ea44f?style=flat)](../.claude/settings.json)

A comprehensive guide to all available configuration options in Claude Code's `settings.json` files. As of v2.1.259, Claude Code exposes **150+ settings** and **315+ environment variables** (use the `"env"` field in `settings.json` to avoid wrapper scripts).
...
| `enableWorkflows` | boolean | `false` | Set to `true` to explicitly enable [dynamic workflows](https://code.claude.com/docs/en/workflows). Complement to `disableWorkflows` — useful when managed settings disable workflows org-wide but you want to re-enable for specific users or projects |
| `isolatePeerMachines` | boolean | `false` | Require explicit approval before accepting cross-machine messages from your other Claude sessions via [Remote Control](https://code.claude.com/docs/en/remote-control). When `true`, inbound messages from peer machines show a prompt rather than auto-accepting based on `crossSessionInbound` |
| `syncClaudeAiSkills` | boolean | - | Download and sync [skills from claude.ai](https://code.claude.com/docs/en/skills-guide). When `true`, Claude Code downloads your claude.ai skill configurations at startup and merges them into the session. Only honored from user settings, local settings, and managed settings (not project settings). Pairs with `CLAUDE_CODE_SYNC_SKILLS` env var |
...
| `advisorModel` | string | - | Model for the server-side advisor tool. Accepts a model alias (`opus`, `sonnet`) or a full model ID. When unset, the advisor uses the session model. Requires v2.1.98+. **v2.1.210+:** Setting `"fable"` (previously Fable 5, now Fable 5.1) is not available in the advisor picker — use `"opus"` or `"sonnet"` instead |
...
| `feedbackDrafts` | string | `"notify"` | Controls background bug-report draft queuing. Values: `"notify"` (default — queue drafts and show a notification), `"quiet"` (queue drafts silently, no notification), `"off"` (disable draft queuing entirely). Gates the `SendFeedback` tool. Appears in `/config` as **Feedback drafts**. Can be set in managed settings for org-wide control (v2.1.247) |
...
| `permissions.defaultMode` | string | Default permission mode. Valid values: `"default"`, `"manual"` (alias for `"default"`, v2.1.200), `"acceptEdits"`, `"dontAsk"`, `"bypassPermissions"`, `"auto"`, `"plan"`. In Remote environments, only `acceptEdits` and `plan` are honored (v2.1.70+). **Note (v2.1.142):** `"auto"` is ignored when set in project (`.claude/settings.json`) or local settings — a repository cannot grant itself auto mode; use `~/.claude/settings.json` instead. **Note (v2.1.257):** `"bypassPermissions"` is now also ignored in project and local settings — same restriction as `"auto"`. Only user settings (`~/.claude/settings.json`) and managed settings can activate bypass mode |
| `permissions.disableBypassPermissionsMode` | string | Prevent bypass mode activation |
| `permissions.blockReadsOutsideWorkingDirectories` | boolean | When `true`, Claude Code shows a one-time confirmation prompt before the first file read outside the current working directories. Introduced in v2.1.257 as a softer boundary than full filesystem isolation. *(in v2.1.257/259 changelog, not yet on official settings page)* |
...
| `skipAutoPermissionPrompt` | boolean | Skip the first-time informational notice shown when entering auto mode for the first time. When `true`, the mode activates immediately without the welcome explanation. Only honored from user settings (`~/.claude/settings.json`) and managed settings |
...
| `allowedMcpServers` | array | Any | Allowlist with name/command/URL matching. **Note (v2.1.259):** This key now governs only servers users add themselves; managed servers load regardless of this list |
| `deniedMcpServers` | array | Any | Blocklist with matching |
| `managedMcpServers` | array | Managed only | Org-wide MCP server delivery. Contains `httpSse` or `stdio` entries (same fields as `mcpServers` but pushed from managed settings to every user automatically). Servers in this list are loaded alongside user MCP servers and cannot be overridden by `deniedMcpServers`. *(in v2.1.259 changelog, not yet on official settings page)* |
...
| `sandbox.ripgrep` | string | - | Absolute path to a custom `ripgrep` binary for use inside the sandbox. Override the auto-detected `ripgrep` when your sandbox environment restricts `PATH` access. Only honored from user settings and managed settings |
...
| `"fable"` | **Claude Fable 5.1** (`claude-fable-5-1`) — long-horizon reasoning model, now the default Fable model (v2.1.257). Anthropic API only (v2.1.170+). Fable 5.1 pricing: $10/$50 per Mtok with $0.25/Mtok cache reads. 1M context by default; the `[1m]` suffix is auto-stripped, so `fable[1m]` is redundant (v2.1.173). Gateways without Fable 5.1 support keep resolving the `fable` alias to Fable 5 |
...
| `spinnerTipsOverride` | object | - | Custom spinner tips. Supports two formats: (1) simple: `{tips: string[], excludeDefault?: boolean}` — when `excludeDefault: true`, only custom tips show; when `false`/absent, custom tips merge with built-in tips; `excludeDefault: true` also suppresses time-based spinner tips (v2.1.121). (2) structured entries (v2.1.247): each entry in `tips` can be a string or an object with `{id, text, cooldownSessions?, priority?, tipsFile?, label?}` — `id` uniquely identifies the tip, `cooldownSessions` controls how many sessions before the tip reappears, `priority` sets display priority, `tipsFile` points to an external tips file, `label` groups tips by organization category. Allows orgs to rotate custom tips alongside built-ins with granular control |
...
| `promptSuggestionEnabled` | boolean | `true` | Show predicted follow-up prompts after responses. Set to `false` to disable next-prompt suggestions (v2.1.121+). The `CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION` env var is the equivalent environment override |
| `subagentStatusLine` | string | - | Custom status line command for subagents, separate from the main session `statusLine`. When set, subagents use this command for their status line display; when unset, subagents inherit the main session's `statusLine` setting |
| `timeFormat` | string | - | Time display format for turn-end clocks and transcript timestamps. Values: `"12h"` (12-hour with AM/PM), `"24h"` (24-hour local time), `"24h-utc"` (24-hour UTC), or a custom `strftime` pattern string (e.g., `"%H:%M:%S"`). When unset, uses the system locale default. *(in v2.1.257 changelog, not yet on official settings page)* |
| `timeZone` | string | - | Timezone for timestamp display. Accepts standard IANA timezone identifiers (e.g., `"America/New_York"`, `"Asia/Karachi"`, `"UTC"`). Affects the turn-end clock and transcript view timestamps across all interfaces. When unset, uses the system timezone. *(in v2.1.257 changelog, not yet on official settings page)* |
...
| `permissionExplainerEnabled` | boolean | `true` | **Removed in v2.1.257.** Previously showed an AI-generated natural-language explanation of why a permission was being requested. No longer read or honored — permission prompts always show the raw tool call |
...
| `sshHostAllowlist` | array | **(Managed only)** Restrict Desktop SSH connections to this list of allowed hosts. When set, users can only connect to hosts explicitly listed here; the SSH dropdown only shows approved connections |
| `disableDesktopLocalSessions` | boolean | **(Managed only)** Disable local Claude Code Desktop sessions. When `true`, users cannot start terminal/IDE sessions from Desktop and must use remote environments instead |
| `managedSourcesBehavior` | string | **(Managed only)** Controls how multiple managed settings sources are combined. Values: `"compose"` (merge all managed sources together, with per-key highest-source wins), `"highest-priority"` (only the highest-precedence managed source is used, others ignored). Default behavior is single highest-priority source. The `"compose"` value corresponds to the admin-source union behavior documented in the Settings Hierarchy section |
...
| `ANTHROPIC_DEFAULT_FABLE_MODEL` | Override Fable model alias (e.g., `claude-fable-5-1`) |
...
| `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` | Force a specific model for all subagents, overriding per-subagent model selections. Set to a model alias or model ID (e.g., `haiku`, `claude-haiku-4-5-20251001`) *(in changelog, not on official env-vars page — unverified)* |
