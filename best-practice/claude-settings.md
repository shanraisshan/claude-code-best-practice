...
![Last Updated](https://img.shields.io/badge/Last_Updated-Aug%2012%2C%202026%2010%3A45%20AM%20PKT-white?style=flat&labelColor=555) ![Version](https://img.shields.io/badge/Claude_Code-v2.1.228-blue?style=flat&labelColor=555)<br>
[![Implemented](https://img.shields.io/badge/Implemented-2ea44f?style=flat)](../.claude/settings.json)

A comprehensive guide to all available configuration options in Claude Code's `settings.json` files. As of v2.1.228, Claude Code exposes **127+ settings** and **335 environment variables** (use the `"env"` field in `settings.json` to avoid wrapper scripts).
...
| `crossSessionInbound` | string | per-message default | Controls how cross-session messages (`SendMessage`) addressed to a permission-bypassed session are handled. Values: `"accept"` (deliver immediately), `"hold"` (queue for approval), `"refuse"` (discard). Scope rule: read from managed → `--settings` → user settings, first value wins; a project/local value applies only when stricter on the `accept < hold < refuse` ladder. When no trusted source sets it, a project/local `hold` or `refuse` still applies. Use in managed settings to enforce a floor policy across all sessions (v2.1.224) |
| `dialogExpiry` | string | `"5m"` | Deadline for dialogs forwarded to a remote client (Remote Control or SDK host) and for held cross-session message approval. Accepts `"60s"`, `"5m"`, `"10m"`, `"never"`. **Does not** govern permission prompts or `AskUserQuestion` (those use `askUserQuestionTimeout`). Read from user, managed, and `--settings` only — project and local settings are ignored. Override at runtime with `CLAUDE_CODE_USER_DIALOG_TIMEOUT_MS`. (v2.1.224) |
...
| `Write` | `Write(path pattern)` | `Write(*.md)`, `Write(./docs/**)` — **path form is never consulted in any list** (allow, deny, or ask); accepted at parse time but evaluation always uses `Edit(path)` instead. A bare `Write` (no path) in a deny rule **does** block the tool globally. Use `Edit(path)` wherever you need path-level write control (see note below) |
| `NotebookEdit` | `NotebookEdit(pattern)` | `NotebookEdit(*)` — **path form is never consulted in any list**; use `Edit(path)` for path-level notebook write control. A bare `NotebookEdit` deny blocks the tool globally (see note below) |
...
> **v2.1.210 — path forms are never consulted in any list:** `Write(path)`, `NotebookEdit(path)`, `MultiEdit(path)`, and `Glob(path)` are accepted at parse time in **all** rule lists (allow, deny, and ask) but are **never consulted** — permission evaluation only checks `Edit(path)` and `Read(path)`. Claude Code prints a startup warning and recommends the correct alternatives. **Exception:** a `Glob(path)` rule passed via `--allowedTools` is honored. **Bare tool-name rules (no path) still work** — a deny rule for bare `Write` or `NotebookEdit` (without parentheses) blocks the tool globally everywhere. Use `Edit(path)` in place of `Write(path)`, `NotebookEdit(path)`, or `MultiEdit(path)`, and `Read(path)` in place of `Glob(path)`.
>
> **v2.1.228 — Read-deny now blocks Write too:** A `Read` deny rule also blocks the `Edit` **and `Write`** tools on the same path, including creating a new file there. `NotebookEdit` is not covered by Read-deny, so add an `Edit` deny rule for paths no tool should change. Requires v2.1.208 for edit blocking and **v2.1.228** for write blocking.
...
| `sandbox.credentials.files[].decode` | string | — | `"jwt"` — verifies the value is a JWT and substitutes a structurally valid fake token so in-sandbox decoders keep working. Cannot be combined with `extract`. (v2.1.224) |
| `sandbox.credentials.files[].maskClaims` | array | — | Companion to `decode: "jwt"` — array of top-level payload claim names to mask, leaving all other claims readable. (v2.1.224) |
...
| `sandbox.credentials.files[].onExtractNoMatch` | string | `"warn"` | Behavior when the `extract` regex matches nothing: `"warn"` (log a warning, allow read), `"deny"` (block the read), or `"error"` (abort the sandboxed command). **Escalation:** On macOS with filesystem isolation active, `deny` is treated as `error`. Claude Code also treats `deny` as `error` when a `filesystem.allowRead` entry re-opens the file's path. (v2.1.224) |
| `sandbox.credentials.files[].maskDuplicates` | boolean | `false` | When `true`, also masks verbatim copies of the extracted (or decoded) credential value found **outside the matched spans within the same file**. Matches raw substrings — reserve for long, high-entropy secrets to avoid unintended replacements. (v2.1.224) |
...
| `sandbox.credentials.envVars[].extract` | string | — | Same as `files[].extract` but for env var values — regex with a single capture group that locates the credential within the value. (v2.1.224) |
| `sandbox.credentials.envVars[].decode` | string | — | `"jwt"` — same JWT handling as `files[].decode` but for env var values. Cannot be combined with `extract`. (v2.1.224) |
| `sandbox.credentials.envVars[].maskClaims` | array | — | Array of top-level JWT payload claim names to mask when `decode: "jwt"` is set on an env var entry. (v2.1.224) |
| `sandbox.credentials.envVars[].onExtractNoMatch` | string | `"warn"` | Same values as `files[].onExtractNoMatch` (`"warn"` / `"deny"` / `"error"`). When `"deny"`, the variable is **unset inside the sandbox** (rather than making a file unreadable, as in the file case). (v2.1.224) |

**Also add `filesystem.disabled` managed-pinning:** When managed settings configure `sandbox.filesystem` at all, or list any `sandbox.credentials.files` entry with `"mode": "deny"`, only managed settings can subsequently modify those keys. Additionally, when `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB` is set, `filesystem.disabled` is ignored from all sources including managed settings.
...
| `CLAUDE_CODE_USER_DIALOG_TIMEOUT_MS` | Runtime override for `dialogExpiry` — how many milliseconds before a dialog forwarded to a Remote Control or SDK client auto-expires. When set, takes precedence over the `dialogExpiry` settings key. Applies to held cross-session message approval and forwarded remote dialogs. Does not affect `AskUserQuestion` or permission prompts (those use `CLAUDE_AFK_TIMEOUT_MS`). (v2.1.224) |
