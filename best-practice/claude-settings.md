...
![Last Updated](https://img.shields.io/badge/Last_Updated-Aug%2021%2C%202026%2010%3A46%20AM%20PKT-white?style=flat&labelColor=555) ![Version](https://img.shields.io/badge/Claude_Code-v2.1.238-blue?style=flat&labelColor=555)<br>
[![Implemented](https://img.shields.io/badge/Implemented-2ea44f?style=flat)](../.claude/settings.json)

A comprehensive guide to all available configuration options in Claude Code's `settings.json` files. As of v2.1.238, Claude Code exposes **228+ settings** and **335 environment variables** (use the `"env"` field in `settings.json` to avoid wrapper scripts).
...
| `crossSessionInbound` | string | `"on"` | Controls whether this session receives cross-session messages from other Claude sessions via Remote Control. Values: `"on"` (default — accept from any session), `"contacts"` (only from sessions in your contacts list), `"off"` (block all inbound cross-session messages). Only honored from user and managed settings (v2.1.224) |
| `dialogExpiry` | string | - | Duration after which a pending Remote Control permission dialog auto-expires if the user does not respond. Accepts duration strings such as `"5m"` or `"1h"`. When unset, dialogs never expire. Only honored from user and managed settings (v2.1.224) |
...
| `sandbox.ripgrep` | string | - | Absolute path to the `rg` (ripgrep) binary used for the Grep tool when running inside the sandbox. Overrides automatic `PATH` detection. Useful in managed environments where the ripgrep binary is not on the standard `PATH` within the sandbox (v2.1.224) |
...
| `sandbox.credentials.files[].extract` | string | — | Regex with a single capture group used to locate credential values within the file for masking. Required when `mode` is `"mask"` and the file is not a well-known format. Example: `"oauth_token:\\s*(\\S+)"` (v2.1.224) |
...
| `sandbox.credentials.files[].maskDuplicates` | boolean | `false` | When `true`, also mask occurrences of the extracted credential value that appear in other files and env vars read during the same sandboxed command *(in JSON schema, not on official settings page — unverified)* |
| `sandbox.credentials.files[].decode` | string | — | Credential encoding to decode before masking. Currently supports `"jwt"` — Claude Code decodes the JWT and also masks the individual claim values in addition to the raw token string. When unset, the raw extracted value is masked as-is (v2.1.224) |
| `sandbox.credentials.files[].maskClaims` | array | — | List of JWT claim names whose values should be masked in addition to the full token when `decode: "jwt"` is set. Example: `["access_token", "refresh_token"]` (v2.1.224) |
| `sandbox.credentials.files[].awsPairs` | boolean | `false` | When `true`, extract and mask the `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` key-value pair from the credential file in addition to the raw token value (v2.1.224) |
| `sandbox.credentials.files[].sigv4` | boolean | `false` | When `true`, mask AWS SigV4 `Authorization` header values in outbound requests so signed requests cannot be replayed from sandbox output (v2.1.224) |
...
| `additionalMarketplaces` | object | Project | Alias for `extraKnownMarketplaces` — identical semantics. Use either key to declare custom plugin marketplaces; the two are merged if both are present (v2.1.232) |
...
| `allowedMarketplaces` | boolean \| array | Managed only | Alias for `strictKnownMarketplaces` with extended syntax. When `true`, only the official marketplace is allowed (same as `strictKnownMarketplaces: true`); when an array of marketplace name strings, only those named marketplaces plus the official one are permitted (v2.1.232) |
...
**Marketplace source types:** `github`, `git`, `directory`, `settings`, `url`, `npm`, `file`, `archive`, `command`. Use `source: 'settings'` to declare a small set of plugins inline without setting up a hosted marketplace repository. Use `source: 'archive'` for zip-based plugin installation with SHA-256 pinning (v2.1.224). Use `source: 'command'` to invoke a shell command that writes a marketplace manifest to stdout — enables dynamically generated or internally hosted marketplaces (v2.1.229). Note: `hostPattern` is a *matcher* field for `blockedMarketplaces`, not a source type.

**Marketplace `headersHelper` (v2.1.238):** Each marketplace config entry accepts an optional `headersHelper` field — a path to a script invoked before each marketplace request to generate per-request authentication headers. The script receives `CLAUDE_CODE_MCP_SERVER_URL` and `CLAUDE_CODE_MCP_SERVER_NAME` as environment variables and must write `Name: Value` pairs to stdout, one header per line. Use for marketplaces that require token-based or signed authentication.
...
| `keybindingFlavor` | string | `"default"` | Keyboard shortcut scheme. Values: `"default"` (Claude Code built-in shortcuts), `"vscode"` (VS Code-compatible shortcuts). Set via `/config` as **Keybinding flavor** (v2.1.238) |
| `spellcheck` | boolean | `true` | Enable spell-check in the prompt input box. Set to `false` to disable spell-check highlighting (v2.1.235) |
...
| `teammateDefaultModel` | string | `null` | **REMOVED in v2.1.234.** Previously set the default model for [agent-team](https://code.claude.com/docs/en/agent-teams) teammates when the lead dispatches them. Use the `model` field in subagent definitions or the `CLAUDE_CODE_SUBAGENT_MODEL` env var instead |
...
| `ANTHROPIC_DEFAULT_MODEL` | Override the built-in default model selection. Accepts aliases (`sonnet`, `opus`, `haiku`) or full model IDs. Lower precedence than `ANTHROPIC_MODEL` — sets the fallback default when no other model is specified. Use to configure a baseline model for an environment without locking out per-session overrides *(in v2.1.225 changelog, not yet on official env-vars page)* |
...
| `CLAUDE_CODE_TOOL_MEMORY_LIMIT` | Maximum memory in bytes that a single tool result may occupy in the context window before being summarized or truncated. Useful for capping large file reads or web fetch responses in memory-constrained sessions *(in v2.1.227 changelog, not yet on official env-vars page)* |
| `CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS` | Time-to-live in milliseconds for WebFetch cached responses. When set, fetched page content is reused from the in-session cache for the specified duration instead of re-fetching. Reduces token spend and latency for frequently fetched URLs *(in v2.1.228 changelog, not yet on official env-vars page)* |
...
| `CLAUDE_CODE_ENABLE_TASKS` | Controls whether sessions use the structured Task tools (`TaskCreate`, `TaskUpdate`, `TaskGet`, `TaskList`) or the legacy `TodoWrite` tool. As of v2.1.142, Task tools were the default in all modes; as of v2.1.233, todo tools are removed by default on newer models. Set to `0` to revert to `TodoWrite`; set to `1` to force-enable Task tools on models where they are off by default |
| `CLAUDE_CODE_ENABLE_TODO_TOOLS` | Set to `1` to force-enable the legacy `TodoWrite`/`TodoRead` tools. As of v2.1.233, these tools are off by default on newer models that use Task tools instead. Useful when migrating scripts or workflows that depend on `TodoWrite` (v2.1.233) |
...
| `CLAUDE_CODE_PROJECT_DIR_NAME` | Override the name of the Claude project directory within a repository. Defaults to `.claude`. Set to a custom directory name to use a non-standard project settings location (e.g., `CLAUDE_CODE_PROJECT_DIR_NAME=.ai`) *(in v2.1.226 changelog, not yet on official env-vars page)* |
...
| `CLAUDE_CODE_MAX_SUBAGENTS_PER_SESSION` | **Cap removed in v2.1.224** — this variable is now a no-op. Previously capped subagent spawning at 200 per session. Use `CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS` to limit simultaneous subagents instead *(in v2.1.212 changelog, cap removed v2.1.224, not yet on official env-vars page)* |
...
| `CLAUDE_CODE_GOAL_CHECKIN_MINUTES` | Interval in minutes at which Claude Code prompts itself to check in on progress toward the session goal. Set to `0` to disable goal check-ins. Useful for long agentic sessions where periodic re-alignment is desired *(in v2.1.231 changelog, not yet on official env-vars page)* |
| `CLAUDE_CODE_WORKFLOW_PREFIX_STAGGER_MS` | Stagger delay in milliseconds between individual agent starts within a dynamic workflow. When set, agents in a workflow phase do not all launch simultaneously — each is delayed by this interval relative to the previous, reducing burst load on the API and model (v2.1.236) |
